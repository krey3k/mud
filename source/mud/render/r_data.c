...
static byte (*masks)[256];
static char (*masknames)[32];
...
static void R_InitTextures(void)
{
    ...
    for(int i = numlumps - 1; i >= 0; i--)
        if(!strncasecmp(lumpinfo[i]->name, "PNAMES", 6))
        {
            if(numpnameslumps == maxpnameslumps)
            {
                maxpnameslumps++;
                pnameslumps =
                I_Realloc(pnameslumps, maxpnameslumps * sizeof(pnameslump_t));
            }
            ...
            pnameslumps[numpnameslumps].name_p =
            (char*)pnameslumps[numpnameslumps].names + 4;
    ...
    for(int i = 0; i < numtextures1 + numtextures2; i++, directory++)
    {
        ...
        ((size_t)SHORT(mtexture->patchcount) - 1) * sizeof(texpatch_t),
        PU_STATIC, 0);
    ...
    // clean up malloc-ing to use sizeof
    texturetranslation =
    Z_Malloc(((size_t)numtextures + 1) * sizeof(*texturetranslation), PU_STATIC, NULL);
...
static void R_InitBrightmaps(void)
{
    ...
    if(FREEDOOM && W_GetNumLumps("BRGHTMPS") == 1)
        return;
        ...
        if(M_StringCompare(lumpinfo[i]->name, "BRGHTMPS"))
        {
            ...
            while(SC_GetString())
                ...
                else if(SC_Compare("TEXTURE"))
                {
                    ...
                    if(SC_Compare("TEXTURE") || SC_Compare("SPRITE") ||
                    SC_Compare("FLAT") || SC_Compare("STATE"))
                    {
                        SC_UnGet();
                        *sc_String = '\0';
                    }
                    ...
                    if(texture >= 0)
                    {
                        ...
                        else if(!*sc_String || SC_Compare("0") ||
                        SC_Compare("DOOM|DOOM2") || SC_Compare("DOOM1|DOOM2") ||
                        (game.mission == doom && !SC_Compare("2") && !SC_Compare("DOOM2")) ||
                        (game.mission != doom && !SC_Compare("1") &&
                        !SC_Compare("DOOM") && !SC_Compare("DOOM1")))
                            for(int j = 0; j < nummasks; j++)
                                if(M_StringCompare(maskname, masknames[j]))
                                {
    ...
    if(r_brightmaps && numbrightmaps > 0)
    {
        ...
        C_Output("Brightmaps have been applied to %s texture%s.", temp,
        (numbrightmaps == 1 ? "" : "s"));
        free(temp);
    }
}
...
static void R_InitFlats(void)
{
    ...
    // Create translation table for global animation.
    flattranslation =
    Z_Malloc(((size_t)numflats + 1) * sizeof(*flattranslation), PU_STATIC, NULL);
...
static void R_InitSpriteLumps(void)
{
    ...
    newspritetopoffset =
    Z_Malloc(numspritelumps * sizeof(*newspritetopoffset), PU_STATIC, NULL);
    ...
    for(int i = 0; i < nummobjtypes; i++)
    {
        ...
        for(int j = 0; j < numspritelumps; j++)
        {
            ...
            if(M_StringStartsWith(lump->name, temp) && lump->wadfile->type == PWAD &&
    ...
    M_StringCopy(berserk,
    M_StringReplaceFirst(powerups[pw_strength], " power-up", ""), sizeof(berserk));
    ...
    if(FREEDOOM)
    {
        ...
        weaponinfo[wp_supershotgun].name =
        M_StringDuplicate("double-barreled shotgun");

        M_StringCopy(weaponinfo[wp_missile].ammoname, "missile",
        sizeof(weaponinfo[0].ammoname));
        M_StringCopy(weaponinfo[wp_missile].ammoplural, "missiles",
        sizeof(weaponinfo[0].ammoplural));
        M_StringCopy(weaponinfo[wp_plasma].ammoname, "polaric recharge",
        sizeof(weaponinfo[0].ammoname));
        M_StringCopy(weaponinfo[wp_plasma].ammoplural, "polaric recharges",
        sizeof(weaponinfo[0].ammoplural));
        M_StringCopy(weaponinfo[wp_bfg].ammoname, "polaric recharge",
        sizeof(weaponinfo[0].ammoname));
        M_StringCopy(weaponinfo[wp_bfg].ammoplural, "polaric recharges",
        sizeof(weaponinfo[0].ammoplural));

        M_StringCopy(
        mobjinfo[MT_MISC0].name1, "light armor vest", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC0].plural1, "light armor vests",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC1].name1, "heavy armor vest", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC1].plural1, "heavy armor vests",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC2].name1, "1% health bonus", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC2].plural1, "1% health bonuses",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC3].name1, "1% armor bonus", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC3].plural1, "1% armor bonuses",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC4].name1, "blue passcard", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC4].plural1, "blue passcards",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC5].name1, "red passcard", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC5].plural1, "red passcards",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC6].name1, "yellow passcard", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC6].plural1, "yellow passcards",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC7].name1, "yellow skeleton key",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC7].plural1, "yellow skeleton keys",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC8].name1, "red skeleton key", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC8].plural1, "red skeleton keys",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC9].name1, "blue skeleton key",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC9].plural1, "blue skeleton keys",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC10].name1, "small health pack",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC10].plural1, "small health packs",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC11].name1, "large health pack",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC11].plural1, "large health packs",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC12].name1, "overdrive sphere",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC12].plural1, "overdrive spheres",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_INV].name1, "vanguard device", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_INV].plural1, "vanguard devices",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC13].name1, "strength symbiote",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC13].plural1, "strength symbiotes",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_INS].name1, "invisibility cloak", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_INS].plural1, "invisibility cloaks",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC14].name1, "rescue operations suit",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC14].plural1, "rescue operations suits",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MISC15].name1, "area survey map", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC15].plural1, "area survey maps",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC16].name1, "low-light goggles",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC16].plural1, "low-light goggles",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_MEGA].name1, "negentropic surge", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MEGA].plural1, "negentropic surges",
        sizeof(mobjinfo[0].plural1));
        ...
        M_StringCopy(
        mobjinfo[MT_MISC17].name1, "box of ammo", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC17].plural1, "boxes of ammo",
        sizeof(mobjinfo[0].plural1));
        ...
        M_StringCopy(mobjinfo[MT_MISC19].name1, "crate of missiles",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC19].plural1, "crates of missiles",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC20].name1, "small polaric recharge",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC20].plural1, "small polaric recharges",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC21].name1, "large polaric recharge",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC21].plural1, "large polaric recharges",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC25].name1, "SKAG 1337", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_MISC25].plural1, "SKAG 1337s", sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_CHAINGUN].name1, "minigun", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_CHAINGUN].plural1, "miniguns", sizeof(mobjinfo[0].plural1));
        ...
        M_StringCopy(mobjinfo[MT_MISC27].name1, "missile launcher",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC27].plural1, "missile launchers",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_MISC28].name1, "polaric energy cannon",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_MISC28].plural1, "polaric energy cannons",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_SHOTGUN].name1, "pump-action shotgun",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_SHOTGUN].plural1, "pump-action shotguns",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_SUPERSHOTGUN].name1, "double-barreled shotgun",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_SUPERSHOTGUN].plural1,
        "double-barreled shotguns", sizeof(mobjinfo[0].plural1));

        M_StringCopy(mobjinfo[MT_POSSESSED].name1, "zombie", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_POSSESSED].plural1, "zombies", sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_SHOTGUY].name1, "shotgun zombie", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_SHOTGUY].plural1, "shotgun zombies",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_VILE].name1, "necromancer", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_VILE].plural1, "necromancers", sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_UNDEAD].name1, "octaminator", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_UNDEAD].plural1, "octaminators",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_FATSO].name1, "combat slug", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_FATSO].plural1, "combat slugs", sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_CHAINGUY].name1, "minigun zombie",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_CHAINGUY].plural1, "minigun zombies",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_TROOP].name1, "serpentipede", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_TROOP].plural1, "serpentipedes",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_SERGEANT].name1, "flesh worm", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_SERGEANT].plural1, "flesh worms",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_SHADOWS].name1, "stealth worm", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_SHADOWS].plural1, "stealth worms",
        sizeof(mobjinfo[0].plural1));
        ...
        M_StringCopy(
        mobjinfo[MT_BRUISER].name1, "pain bringer", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_BRUISER].plural1, "pain bringers",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_KNIGHT].name1, "pain lord", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_KNIGHT].plural1, "pain lords", sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_SKULL].name1, "hatchling", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_SKULL].plural1, "hatchlings", sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_SPIDER].name1, "large technospider",
        sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_SPIDER].plural1, "large technospiders",
        sizeof(mobjinfo[0].plural1));
        M_StringCopy(mobjinfo[MT_BABY].name1, "technospider", sizeof(mobjinfo[0].name1));
        M_StringCopy(
        mobjinfo[MT_BABY].plural1, "technospiders", sizeof(mobjinfo[0].plural1));
        M_StringCopy(
        mobjinfo[MT_CYBORG].name1, "assault tripod", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_CYBORG].plural1, "assault tripods",
        sizeof(mobjinfo[0].plural1));
        ...
        M_StringCopy(
        mobjinfo[MT_WOLFSS].name1, "Spanish sailor", sizeof(mobjinfo[0].name1));
        M_StringCopy(mobjinfo[MT_WOLFSS].plural1, "Spanish sailors",
        sizeof(mobjinfo[0].plural1));
    }
}
...
static void R_InitColormaps(void)
{
    ...
    if(W_CheckNumForName("C_START") >= 0 && W_CheckNumForName("C_END") >= 0)
    {
        ...
        if(W_CheckNumForName("CC_START") >= 0 && W_CheckNumForName("CC_END") >= 0)
        {
            ...
            colormaps =
            I_Realloc(colormaps, (numcolormaps + numcolormaps2) * sizeof(*colormaps));
    ...
    dc_colormap[1] = dc_nextcolormap[1] = colormaps[0] =
    W_CacheLumpName("COLORMAP");

    if(numcolormaps == 1)
        C_Output(
        "The " BOLD("COLORMAP") " lump in the %s " BOLD("%s") " is being used.",
        (colormapwad->type == IWAD ? "IWAD" : "PWAD"), colormapwad->path);
    else
    {
        wadfile_t* othercolormapwad = lumpinfo[firstcolormaplump]->wadfile;

        if(D_IsResourceWAD(othercolormapwad->path))
            C_Output("The " BOLD("COLORMAP") " lump in the %s " BOLD(
                     "%s") " is being used.",
            (colormapwad->type == IWAD ? "IWAD" : "PWAD"), colormapwad->path);
        else if(M_StringCompare(colormapwad->path, othercolormapwad->path))
            C_Output(
            "The " BOLD("COLORMAP") " lump and %i more in the %s " BOLD("%s") " are being used.",
            numcolormaps - 1, (colormapwad->type == IWAD ? "IWAD" : "PWAD"),
            colormapwad->path);
        else
            C_Output(
            "The " BOLD("COLORMAP") " lump in the %s " BOLD(
            "%s") " and %i more in the %s " BOLD("%s") " are being used.",
            (colormapwad->type == IWAD ? "IWAD" : "PWAD"), colormapwad->path,
            numcolormaps - 1, (othercolormapwad->type == IWAD ? "IWAD" : "PWAD"),
            othercolormapwad->path);
    }
...
int R_CheckTextureNumForName(const char* name)
{
    ...
    // Guard against uninitialized texture system
    ...
    if(*name != '-')
    {
        ...
        while(i >= 0 && i < numtextures && strncasecmp(textures[i]->name, name, 8))
            i = textures[i]->next;
        
        // Bounds check - if chain led to invalid index, texture not found
        if(i >= numtextures)
            i = -1;
    }
...
{
    bool* hitlist =
    calloc(MAX(MAX(numsectors, numflats), MAX(numsides, numtextures)), sizeof(bool));

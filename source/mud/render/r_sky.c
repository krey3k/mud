...
static void InitSkyDefs(void)
{
    ...
    array_foreach(sky,
    skydefs->skies) if(skytexture == R_CheckTextureNumForName(sky->skytex.name))
    {
...
void R_InitSkyMap(void)
{
    ...
    skytexture               = P_GetMapSky1Texture(game.episode, game.map);
    canfreelook = ((freelook || keyboardfreelook || keyboardfreelook2 ||
                   mousefreelook != -1 || controllerfreelook || autotilt ||
                   (weaponrecoil && r_screensize == r_screensize_max)) &&
    !nofreelook);
    ...
    if(!skytexture)
    {
        ...
        else
        {
            ...
            case 5:
                ...
                R_CheckTextureNumForName("SKY5_ZD") != -1 ? "SKY5_ZD" : "SKY5");
                break;

            case 6:
                skytexture = R_TextureNumForName(
                R_CheckTextureNumForName("SKY6_ZD") != -1 ? "SKY6_ZD" : "SKY6");
                break;
            }
        }
    }
    ...
    if(skyheight > 128 && skyheight < V_VANILLAHEIGHT)
        skytexturemid = -54 * FRACUNIT * skyheight / SKYSTRETCH_HEIGHT;
    else if(skyheight > V_VANILLAHEIGHT)
        skytexturemid = (V_VANILLAHEIGHT - skyheight) * FRACUNIT * skyheight / SKYSTRETCH_HEIGHT;
    else
        skytexturemid = V_VANILLAHEIGHT / 2 * FRACUNIT;

    if(canfreelook)
        skyiscale = (fixed_t)(((uint64_t)video.screen_width * V_VANILLAHEIGHT * FRACUNIT) /
                    ((uint64_t)render.view_width * video.screen_height)) *
    ...
    skyscrolldelta =
    (vanilla ? 0 : (int)(P_GetMapSky1ScrollDelta(game.episode, game.map) * FRACUNIT));

...
fixed_t r_centerxfrac;
fixed_t r_centeryfrac;
fixed_t r_projection;
...
// flattening the arc to a flat r_projection plane.
...
// of 'tx' value, used by R_ProjectSprite for sprite r_projection.
fixed_t fovtx;
...
static void R_InitTables(void)
{
    ...
    for(int i = 0; i < FINEANGLES / 2; i++)
        finetangent[i] =
        (fixed_t)(tan(((double)i - FINEANGLES / 4 + 0.5) * M_PI * 2 / FINEANGLES) * FRACUNIT);
...
static void R_InitPointToAngle(void)
{
    ...
    for(int i = 0; i <= SLOPERANGE; i++)
        tantoangle[i] =
        (angle_t)(0xFFFFFFFF * atan((i + 0.5) / SLOPERANGE) / (M_PI * 2));
}
...
{
    // Calc focallength so field of view angles covers render.screen_width.
    const fixed_t focallength = FixedDiv(r_centerxfrac, fovscale);
    ...
    for(int i = 0; i < FINEANGLES / 2; i++)
    {
        ...
        if(tangent > fovscale)
            ...
            viewangletox[i] = render.view_width + 1;
        else
            viewangletox[i] = BETWEEN(-1,
            (r_centerxfrac - FixedMul(tangent, focallength) + FRACUNIT - 1) >> FRACBITS,
            render.view_width + 1);
    }

    // Scan viewangletox[] to generate xtoviewangle[]:
    //  xtoviewangle will give the smallest view angle that maps to x.
    for(int i, x = 0; x <= render.view_width; x++)
    {
        ...
        linearskyangle[x] = (render.view_width / 2 - x) *
        ((render.screen_width << FRACBITS) / render.view_width) * (ANG90 / (render.nonwide_width << FRACBITS));
    }
    ...
    for(int i = 0; i < FINEANGLES / 2; i++)
        if(viewangletox[i] == -1)
            viewangletox[i] = 0;
        else if(viewangletox[i] == render.view_width + 1)
            viewangletox[i]--;
...
{
    const unsigned int wfovscale =
    FixedMul(render.screen_width / render.scale, FixedDiv(FRACUNIT, fovscale)) + 1;
...
void R_ExecuteSetViewSize(void)
{
    ...
    if(setblocks == 11)
    {
        v_viewwidth    = video.screen_width;
        v_viewheight   = video.screen_height;
        v_viewwindowx  = 0;
        v_viewwindowy  = 0;

        render.view_width    = render.screen_width;
        render.view_height   = render.screen_height;
        render.view_window_x  = 0;
        render.view_window_y  = 0;
        pspritescale = FixedMul(FixedDiv(render.nonwide_width, render.vanilla_width), render.scale * FRACUNIT);
    }
    else
    {
        v_viewwidth    = setblocks * video.screen_width / 10;
        v_viewheight   = ((setblocks * (video.screen_height - V_SBARHEIGHT) / 10) & ~7);
        v_viewwindowx  = (video.screen_width - v_viewwidth) / 2;
        v_viewwindowy  = (video.screen_height - V_SBARHEIGHT - v_viewheight) / 2;

        render.view_width    = setblocks * render.screen_width / 10;
        render.view_height   = ((setblocks * (render.screen_height - render.sbar_height) / 10) & ~7);
        render.view_window_x  = (render.screen_width - render.view_width) / 2;
        render.view_window_y  = (render.screen_height - render.sbar_height - render.view_height) / 2;
        pspritescale = FixedMul(FixedDiv(setblocks * render.nonwide_width / 10, render.vanilla_width), render.scale * FRACUNIT);
    }

    r_centerx     = render.view_width / 2;
    r_centerxfrac = r_centerx << FRACBITS;
    fovscale    = finetangent[FINEANGLES / 4 +
    ((menuactive && !helpscreen && menuspin ? r_fov_max : r_fov) + render.wide_fov_delta) * FINEANGLES / 360 / 2];
    fovtx = (vid_aspectratio >= vid_aspectratio_21_9 && r_fov >= 90 ? 4 : 2);
    r_projection     = FixedDiv(r_centerxfrac, fovscale);
    r_viewheightfrac = render.view_height << (FRACBITS + 2);
    ...
    for(int i = 0; i < render.view_width; i++)
        viewheightarray[i] = render.view_height;

    // planes
    num = FixedMul(FixedDiv(FRACUNIT, fovscale), render.view_width * FRACUNIT / 2);

    for(int i = 0; i < render.view_height; i++)
        for(int j = 0; j < LOOKDIRS; j++)
            yslopes[j][i] = FixedDiv(num,
            ABS(((i - (render.view_height / 2 + (j - LOOKDIRMAX) * 2 * setblocks / 10)) << FRACBITS) +
            FRACUNIT / 2));
    ...
    for(int i = 0; i < LIGHTLEVELS; i++)
    {
        ...
        {
            const int level =
            BETWEEN(0, start - j * render.screen_width / render.scale / (render.view_width * 2), NUMCOLORMAPS - 1) * 256;
    ...
    {
        const int start =
        ((OLDLIGHTLEVELS - LIGHTBRIGHT - i) * 2) * NUMCOLORMAPS / OLDLIGHTLEVELS;
...
void R_InitColumnFunctions(void)
{
    if(r_textures)
    {
        skycolfunc = (canmodify && !transferredsky &&
        (game.mode != commercial || game.map < 21) && !canfreelook ?
        &R_DrawFlippedSkyColumn :
        &R_DrawWallColumn);
        ...
        if(r_ditheredlighting)
        {
            if(r_detail == r_detail_low)
            {
                ...
                tl50segcolfunc  = (r_textures_translucency ?
                 &R_DrawLowResDitheredTranslucent50Column :
                 &R_DrawLowResDitheredColumn);
                tl50bmapsegcolfunc = (r_textures_translucency ?
                &R_DrawLowResDitheredTranslucent50ColumnWithBrightmap :
                &R_DrawLowResDitheredColumnWithBrightmap);
            ...
            else
            {
                ...
                tl50segcolfunc = (r_textures_translucency ? &R_DrawDitheredTranslucent50Column :
                                                            &R_DrawDitheredColumn);
                tl50bmapsegcolfunc = (r_textures_translucency ?
                &R_DrawDitheredTranslucent50ColumnWithBrightmap :
                &R_DrawDitheredColumnWithBrightmap);
        ...
        else
        {
            ...
            tl50segcolfunc =
            (r_textures_translucency ? &R_DrawTranslucent50Column : &R_DrawColumn);
            tl50bmapsegcolfunc =
            (r_textures_translucency ? &R_DrawTranslucent50ColumnWithBrightmap :
                                       &R_DrawColumnWithBrightmap);
            spanfunc    = &R_DrawSpan;
            altspanfunc = &R_DrawSpan;
        ...
        bloodcolfunc = (r_sprites_translucency ? &R_DrawTranslucentBloodColumn :
                                                 &R_DrawTranslatedColumn);
        bloodsplatcolfunc =
        (r_bloodsplats_translucency ? &R_DrawBloodSplatColumn : &R_DrawSolidBloodSplatColumn);
    ...
    else
    {
        ...
        if(r_ditheredlighting)
        {
            if(r_detail == r_detail_low)
            {
                ...
                tl50segcolfunc     = (r_textures_translucency ?
                    &R_DrawLowResDitheredTranslucent50SolidColorColumn :
                    &R_DrawLowResDitheredSolidColorColumn);
                tl50bmapsegcolfunc = (r_textures_translucency ?
                &R_DrawLowResDitheredTranslucent50SolidColorColumn :
                &R_DrawLowResDitheredSolidColorColumn);
                spanfunc           = &R_DrawLowResDitheredSolidColorSpan;
                tlcolfunc =
                (r_sprites_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                          &R_DrawLowResDitheredSolidColorColumn);
                tl50colfunc =
                (r_sprites_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                          &R_DrawLowResDitheredSolidColorColumn);
            ...
            else
            {
                ...
                tl50segcolfunc     = (r_textures_translucency ?
                    &R_DrawDitheredTranslucent50SolidColorColumn :
                    &R_DrawDitheredSolidColorColumn);
                tl50bmapsegcolfunc = (r_textures_translucency ?
                &R_DrawDitheredTranslucent50SolidColorColumn :
                &R_DrawDitheredSolidColorColumn);
        ...
        else
        {
            ...
            tl50segcolfunc = (r_textures_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                                        &R_DrawSolidColorColumn);
            tl50bmapsegcolfunc =
            (r_textures_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                       &R_DrawSolidColorColumn);
            spanfunc = &R_DrawSolidColorSpan;
            tlcolfunc = (r_sprites_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                                  &R_DrawSolidColorColumn);
            tl50colfunc = (r_sprites_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                                    &R_DrawSolidColorColumn);
            tl33colfunc = (r_sprites_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                                    &R_DrawSolidColorColumn);
        ...
        bloodcolfunc = (r_sprites_translucency ? &R_DrawTranslucent50SolidColorColumn :
                                                 &R_DrawSolidColorColumn);
        bloodsplatcolfunc =
        (r_bloodsplats_translucency ? &R_DrawBloodSplatColumn : &R_DrawSolidBloodSplatColumn);
...
{
    TracyCZoneN(zone_setup, "R_SetupFrame", 1);
    int colormap = 0;
    ...
    && !menuactive && !consoleactive && !paused)
    {
        ...
        viewz = viewplayer->oldviewz +
        FixedMul(viewplayer->viewz - viewplayer->oldviewz, fractionaltic);
        viewangle = R_InterpolateAngle(mo->oldangle, mo->angle);

        if(canfreelook)
            pitch = (viewplayer->oldlookdir +
                    (int)((viewplayer->lookdir - viewplayer->oldlookdir) *
                    FIXED2DOUBLE(fractionaltic))) /
            MLOOKUNIT;

        if(weaponrecoil)
            pitch = BETWEEN(-LOOKDIRMAX / render.scale,
            pitch + viewplayer->oldrecoil +
            FixedMul(viewplayer->recoil - viewplayer->oldrecoil, fractionaltic),
            LOOKDIRMAX / render.scale);
    }
    else
    {
        viewx     = mo->x;
        ...
        if(weaponrecoil)
            pitch = BETWEEN(-LOOKDIRMAX / render.scale, pitch + viewplayer->recoil, LOOKDIRMAX / render.scale);
    }
    ...
    if(automapactive)
    {
        TracyCZoneEnd(zone_setup)
        return;
    }

    r_centery = render.view_height / 2;

    if(pitch)
        r_centery += pitch * 2 * render.scale * (r_screensize + 3) / 10;

    extralight = (viewplayer->extralight << 2) + r_extralighting / 3;

    r_centeryfrac = r_centery << FRACBITS;
    yslope      = yslopes[LOOKDIRMAX + pitch * render.scale];
    ...
    if(mo->subsector->sector->heightsec)
    {
        ...
        colormap = (viewz < s->interpfloorheight ?
        s->bottommap :
...
{
    TracyCZoneN(tracy_zone, "R_RenderPlayerView", 1);
...
// Needs fixing due due to HUD render split and discard pixels
    if(r_homindicator)
        V_FillRect(0, v_viewwindowx, v_viewwindowy, v_viewwidth, v_viewheight,
        (game.stats.maptime % 20 < 9 ? nearestred : nearestblack), 0, false, false, NULL, NULL);
...
//
// R_ResizeMainBuffers
// Allocates or reallocates main rendering buffers for the current scale.
// Called by R_ResizeRenderState when buffer sizes need to increase.
//
// Buffer sizes:
//   xtoviewangle, linearskyangle: r_alloc_max_width + 1 elements
//
void R_ResizeMainBuffers(void)
{
    // Free existing buffers
    if(xtoviewangle) free(xtoviewangle);
    if(linearskyangle) free(linearskyangle);

    // Allocate new buffers (max_width + 1 for off-by-one access)
    xtoviewangle = calloc(r_alloc_max_width + 1, sizeof(angle_t));
    linearskyangle = calloc(r_alloc_max_width + 1, sizeof(angle_t));

    // Verify allocations succeeded
    if(!xtoviewangle || !linearskyangle)
        I_Error("R_ResizeMainBuffers: Failed to allocate view angle buffers");
}

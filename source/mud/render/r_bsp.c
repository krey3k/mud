...
//
// R_ResizeClipSegs
// Allocates or reallocates clip segment buffers for the current scale.
// Called during init and by R_ResizeRenderState when buffer sizes need to increase.
//
// Buffer sizes:
//   solidcol: r_alloc_max_width elements
//
void R_ResizeClipSegs(void)
{
    if(solidcol) free(solidcol);
    solidcol = calloc(r_alloc_max_width, sizeof(*solidcol));

    // Verify allocation succeeded
    if(!solidcol)
        I_Error("R_ResizeClipSegs: Failed to allocate clip segment buffer");
}
...
{
    memset(solidcol, 0, render.max_width);
}
...
static void R_RecalcLineFlags(line_t* line)
{
    ...
    line->r_validcount = game.time;

    if(!twosided || backsector->interpceilingheight <= frontsector->interpfloorheight ||
    ...
    backsector->interpfloorheight != frontsector->interpfloorheight ||
    curline->sidedef->midtexture || backsector->floorxoffset != frontsector->floorxoffset ||
...
static void R_InterpolateSector(sector_t* sector)
{
    ...
    if(vid_capfps != TICRATE)
    {
        ...
        sector->oldfloorgametime == game.time - 1)
        ...
        sector->oldceilinggametime == game.time - 1)
        ...
        if(heightsec)
        {
            ...
            heightsec->oldfloorgametime == game.time - 1)
            ...
            heightsec->oldceilinggametime == game.time - 1)
        ...
        if(sector->oldflooroffsetgametime == game.time - 1)
        {
        ...
        if(sector->oldceilingoffsetgametime == game.time - 1)
        {
...
{
    if(vid_capfps != TICRATE && side->oldgametime == game.time - 1)
    {
...
//
sector_t*
R_FakeFlat(sector_t* sec, sector_t* tempsec, int* floorlightlevel, int* ceilinglightlevel, const bool back)
{
    const sector_t* s = sec->heightsec;

    if(floorlightlevel)
        *floorlightlevel =
        (sec->floorlightsec ? sec->floorlightsec->lightlevel : sec->lightlevel);

    if(ceilinglightlevel)
        *ceilinglightlevel =
        (sec->ceilinglightsec ? sec->ceilinglightsec->lightlevel : sec->lightlevel);
    ...
    if(s)
    {
        ...
        *tempsec = *sec;
        ...
        (tempsec->interpceilingheight = s->interpfloorheight - 1), !back))
        {
            ...
            if(floorlightlevel)
                *floorlightlevel =
                (s->floorlightsec ? s->floorlightsec->lightlevel : s->lightlevel);

            if(ceilinglightlevel)
                *ceilinglightlevel =
                (s->ceilinglightsec ? s->ceilinglightsec->lightlevel : s->lightlevel);
        }
...
{
    TracyCZoneN(tracy_zone, "R_AddLine", 1);
    int x1;
    int x2;
    angle_t angle1;
    angle_t angle2;
    ...
    if((linedef = curline->linedef)->r_validcount != game.time)
...
{
    TracyCZoneN(zone_bbox, "R_CheckBBox", 1);
    const byte checkcoord[12][4] = { { 3, 0, 2, 1 }, { 3, 0, 2, 0 }, { 3, 1, 2, 0 },
...
{
    TracyCZoneN(tracy_zone, "R_Subsector", 1);
    ...
    && (frontsector->floorsky & PL_SKYFLAT) ?
    frontsector->floorsky :
    ...
    (heightsec ? heightsec->floorrotation : frontsector->floorrotation)) :
    NULL);

    ceilingplane = (frontsector->interpceilingheight > viewz ||
    frontsector->ceilingpic == skyflatnum || (heightsec && heightsec->floorpic == skyflatnum) ?
    R_FindPlane(frontsector->interpceilingheight, // killough 03/08/98
    (frontsector->ceilingpic == skyflatnum        // killough 10/98
    && (frontsector->ceilingsky & PL_SKYFLAT) ?
    frontsector->ceilingsky :
    ...
    (ceilinglightsec ? ceilinglightsec->colormap :
                       (heightsec ? heightsec->colormap : frontsector->colormap)),
    (heightsec ? heightsec->ceilingrotation : frontsector->ceilingrotation)) :
    NULL);
    ...
    if(sector->validcount != validcount && !automapactive)
    {
        ...
        R_AddSprites(sector,
        (sector->heightsec ? (ceilinglightlevel + floorlightlevel) / 2 : floorlightlevel));
...
{
    TracyCZoneN(tracy_zone, "R_RenderBSPNode", 1);
    ...
    while(true)
    {
        ...
        while(!(bspnum & NF_SUBSECTOR))
        {
            ...
            bsp             = nodes + bspnum;
            side            = (((int64_t)viewy - bsp->y) * bsp->dx +
            ((int64_t)bsp->x - viewx) * bsp->dy >
            0);
            bspstack[sp]    = bspnum;
            sidestack[sp++] = side;

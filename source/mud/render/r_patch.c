...
bool R_CheckIfPatch(const int lump)
{
    ...
    if(size >= 13)
    {
        ...
        if(magic[0] != 0x89 || magic[1] != 'P' || magic[2] != 'N' || magic[3] != 'G')
        {
            ...
            if((result = (width > 0 && width <= 16384 && width < size / 4 &&
...
static void CreatePatch(int patchnum)
{
    ...
    {
        C_Warning(1, "The " BOLD("%.8s") " patch couldn't be created.",
    ...
    {
        oldcolumn =
        (const column_t*)((const byte*)oldpatch + LONG(oldpatch->columnoffset[x]));
...
static void CreateTextureCompositePatch(const int id)
{
    ...
    for(int i = 0; i < texture->patchcount; i++)
    {
        ...
        for(int x = 0; x < SHORT(oldpatch->width); x++)
        {
            ...
            while(oldcolumn->topdelta != 0xFF)
            {
                ...
                numpoststotal++;
                oldcolumn =
                (const column_t*)((const byte*)oldcolumn + oldcolumn->length + 4);
            }
        }
    ...
    datasize = pixeldatasize + columnsdatasize + postsdatasize;
    compositepatch->data =
    Z_Calloc(1, datasize, PU_STATIC, (void**)&compositepatch->data);

    // set out pixel, column, and post pointers into our data array
    compositepatch->pixels = compositepatch->data;
    compositepatch->columns =
    (rcolumn_t*)((unsigned char*)compositepatch->pixels + pixeldatasize);
    compositepatch->posts =
    (rpost_t*)((unsigned char*)compositepatch->columns + columnsdatasize);

    for(int x = 0, numpostsusedsofar = 0; x < texture->width; x++)
    {
        // setup the column's data
        compositepatch->columns[x].pixels =
        &compositepatch->pixels[x * compositepatch->height];
    ...
    for(int i = 0; i < texture->patchcount; i++)
    {
        ...
        for(int x = 0; x < SHORT(oldpatch->width); x++)
        {
            ...
            while(oldcolumn->topdelta != 0xFF)
            {
                ...
                rpost_t* post =
                &compositepatch->columns[tx].posts[countsincolumn[tx].postsused];
                ...
                (id == STEP2 && modifiedgame) || id == TEKWALL1)
                    oy = 0;
                else if(countsincolumn[tx].patches > 1)
                {
                    if(!i)
                        for(int y = 0; y < count; y++)
                        {
                            ...
                            compositepatch->pixels[tx * compositepatch->height + ty] =
    ...
    for(int x = 0; x < texture->width; x++)
    {
        ...
        while(i < column->numposts - 1)
        {
            ...
            {
                const int length = post1->length + post2->length -

...
#define BASEYCENTER ((render.vanilla_height / 2) / render.scale)
...
static const fixed_t floatbobdiffs[64] = { 205560, 205560, 203576, 199640,
    193776, 186048, 176528, 165304, 152496, 138216, 122600, 105808, 87992,
    69336, 50008, 30200, 10096, -10096, -30200, -50008, -69336, -87992, -105808,
    -122600, -138216, -152496, -165304, -176528, -186048, -193776, -199640,
    -203576, -205560, -205560, -203576, -199640, -193776, -186048, -176528,
    -165304, -152496, -138216, -122600, -105808, -88000, -69336, -50008, -30200,
    -10096, 10096, 30200, 50008, 69336, 87992, 105808, 122600, 138216, 152496,
    165304, 176528, 186048, 193776, 199640, 203576 };
...
#define R_SpriteNameHash(s) \
    ((unsigned int)((s)[0] - ((s)[1] * 3 - (s)[3] * 2 - (s)[2]) * 2))
        ...
        if((j = hash[R_SpriteNameHash(spritename) % numspritelumps].index) >= 0)
        {
            ...
            do
            {
                   ...
                   (lump->name[2] ^ spritename[2]) | (lump->name[3] ^ spritename[3])))
                {
                    ...
                    if(lump->name[6])
                        R_InstallSpriteLump(
                        lumpnum, lump->name[6] - 'A', lump->name[7], true);
                }
                ...
                for(int frame = 0; frame < maxframe; frame++)
                    ...
                    case 1:
                        ...
                        for(int rot = 0; rot < 16; rot++)
                            if(sprtemp[frame].lump[rot] == -1)
                                I_Error("R_InitSprites: Frame %c of sprite "
                                        "%.8s is missing rotations",
                                frame + 'A', sprnames[i]);
...
{
    for(int i = 0; i < render.max_width; i++)
        negonearray[i] = -1;
...
static vissprite_t* R_NewVisSprite(void)
{
    ...
    {
        num_vissprite_alloc =
        (num_vissprite_alloc ? num_vissprite_alloc * 2 : MAXVISSPRITES);
...
static void inline R_BlastSpriteColumn(const rcolumn_t* column)
{
    ...
    while(dc_numposts--)
    {
        ...
        if((dc_yh = MIN((int)((topscreen + (int64_t)spryscale * post->length - 256) >> FRACBITS),
            dc_floorclip)) >= 0)
            if((dc_yl = MAX(dc_ceilingclip,
                (int)((topscreen + FRACUNIT + 512) >> FRACBITS))) <= dc_yh)
            {
                dc_texturefrac = dc_texturemid - (topdelta << FRACBITS) +
                FixedMul((dc_yl - r_centery) << FRACBITS, dc_iscale);
...
static void inline R_BlastPlayerSpriteColumn(const rcolumn_t* column)
{
    ...
    while(dc_numposts--)
    {
        ...
        if((dc_yh = MIN((int)((topscreen + (int64_t)pspritescale * post->length) >> FRACBITS),
            render.view_height - 1)) >= 0)
...
static void R_DrawVisSprite(const vissprite_t* vis)
{
    ...
    if(translation && (r_corpses_color || !(flags & MF_CORPSE)))
    {
        ...
        dc_translation =
        &translationtables[(translation >> (MF_TRANSLATIONSHIFT - 8)) - 256];
    }
    else
    {
        colfunc = vis->colfunc;
    ...
    sprtopscreen = (int64_t)r_centeryfrac - FixedMul(dc_texturemid, spryscale);
    baseclip = (vis->footclip ? (int)(sprtopscreen + vis->footclip) >> FRACBITS : render.view_height);
    fuzz1pos = 0;

    if((percolumnlighting = (r_percolumnlighting && !vis->fullbright &&
    ...
    for(dc_x = vis->x1; dc_x <= x2; dc_x++, frac += xiscale)
    {
        ...
        if((dc_numposts = column->numposts))
        {
            ...
            if(percolumnlighting)
            {
                ...
                (vis->flipped ? pcl_patchoffset - frac : frac - pcl_patchoffset);
                ...
                lightnum =
                ((floorlightlevel + ceilinglightlevel) >> (LIGHTSEGSHIFT + 1)) + extralight;

                dc_colormap[0] =
                scalelight[BETWEEN(0, lightnum - 2, LIGHTLEVELS - 1)][pcl_lightindex];
                dc_nextcolormap[0] =
                scalelight[BETWEEN(0, lightnum + 2, LIGHTLEVELS - 1)][pcl_lightindex];
...
static void R_DrawVisSpriteWithShadow(const vissprite_t* vis)
{
    ...
    shadowtopscreen = (int64_t)r_centeryfrac - FixedMul(vis->shadowpos, spryscale);
    ...
    for(dc_x = vis->x1; dc_x <= x2; dc_x++, frac += xiscale)
    {
        ...
        if((dc_numposts = column->numposts))
        {
            ...
            while(dc_numposts--)
            {
                ...
                const int64_t topscreen = shadowtopscreen + spryscale * post->topdelta;

                if((dc_yh = MIN((((topscreen + spryscale * post->length) >> FRACBITS) / 10 + shadowshift),
                    dc_floorclip)) >= 0)
                    if((dc_yl = MAX(dc_ceilingclip,
                        ((topscreen + FRACUNIT) >> FRACBITS) / 10 + shadowshift)) <= dc_yh)
                        shadowcolfunc();
            }
...
static void R_DrawPlayerVisSprite(const vissprite_t* vis)
{
    ...
    dc_texturemid      = vis->texturemid;
    sprtopscreen = (int64_t)r_centeryfrac - FixedMul(dc_texturemid, pspritescale);
...
static void R_DrawVisSplat(const vissplat_t* vis)
{
    ...
    dc_sectorcolormap = vis->sectorcolormap;
    splattopscreen    = r_centeryfrac - FixedMul(vis->texturemid, spryscale);
    ...
    for(dc_x = vis->x1; dc_x <= x2; dc_x++, frac += xiscale)
    {
        ...
        if(column->numposts)
        {
            ...
            if((dc_yh = MIN((topscreen + spryscale * post->length) >> FRACBITS,
                clipbot[dc_x] - 1)) >= 0)
...
{
    TracyCZoneN(tracy_zone, "R_ProjectSprite", 1);
    fixed_t tx;
    fixed_t xscale;
    int x1;
    int x2;
    ...
    if((int64_t)(ABS((tx = FixedMul(tr_x, viewsin) - FixedMul(tr_y, viewcos))) >>
       fovtx) > ((int64_t)tz << 2))
    {
        TracyCZoneEnd(tracy_zone)
        return;
    ...
    if(((flags2 = thing->flags2) & MF2_FLOATBOB) && r_floatbob)
        fz += floatbobdiffs[((thing->floatbob + game.stats.maptime) & 63)];

    if(sprframe->rotate)
    {
        // choose a different rotation based on player view
        const angle_t ang =
        R_PointToAngle(fx, fy) - thing->angle + (angle_t)(ANG45 / 2) * 9;
    ...
    gzt    = fz + topoffset;
    xscale = FixedDiv(r_projection, tz);

    // killough 04/09/98: clip things which are out of view due to height
    if(fz > (int64_t)viewz + FixedDiv(r_viewheightfrac, xscale) ||
    gzt < (int64_t)viewz - FixedDiv(r_viewheightfrac - render.view_height, xscale))
    {
        TracyCZoneEnd(tracy_zone)
        return;
    }
    ...
    // off the right side?
    if((x1 = (r_centerxfrac + FixedMul(tx, xscale)) >> FRACBITS) >= render.view_width)
    {
        TracyCZoneEnd(tracy_zone)
        return;
    }
    ...
    // off the left side?
    if((x2 = ((r_centerxfrac + FixedMul(tx + width, xscale) - FRACUNIT / 2) >> FRACBITS)) < 0)
    {
        TracyCZoneEnd(tracy_zone)
        return;
    }
    ...
    if((heightsec = thing->subsector->sector->heightsec))
    {
        ...
        if(phs)
        {
            if(viewz < phs->interpfloorheight ? fz >= heightsec->interpfloorheight :
                                                gzt < heightsec->interpfloorheight)
            {
                TracyCZoneEnd(tracy_zone)
                return;
            }
    ...
    vis->x2    = MIN(x2, render.view_width - 1);
    vis->patch = lump;
...
static void R_ProjectBloodSplat(const bloodsplat_t* splat)
{
    ...
    // off the left side?
    if((x2 = ((r_centerxfrac + FixedMul(tx + width, xscale)) >> FRACBITS) - 1) < 0)
        return;
    ...
    {
        vis->colormap       = (viewplayer->fixedcolormap == 1 ?
              fixedcolormap :
...
{
    TracyCZoneN(zone_addsprites, "R_AddSprites", 1);
    ...
    {
        spritelights =
        scalelight[BETWEEN(0, ((lightlevel - 2) >> LIGHTSEGSHIFT) + extralight, LIGHTLEVELS - 1)];
...
{
    TracyCZoneN(tracy_zone, "R_AddNearbySprites", 1);
...
{
    TracyCZoneN(tracy_zone, "R_DrawNearbySprites", 1);
...
static void R_DrawPlayerSprite(const pspdef_t* psp, bool invisibility, bool altered)
{
    ...
    // calculate edges of the shape
    tx = psp->sx - render.vanilla_width / render.scale / 2 * FRACUNIT -
    (!r_fixspriteoffsets || (altered && !vanilla) ? spriteoffset[lump] :
                                                    newspriteoffset[lump]);
    x1 = (r_centerxfrac + FRACUNIT / 2 + FixedMul(tx, pspritescale)) >> FRACBITS;
    x2 =
    ((r_centerxfrac + FRACUNIT / 2 + FixedMul(tx + spritewidth[lump], pspritescale)) >> FRACBITS) - 1;

    // store information in a vissprite
    vis->texturemid = (BASEYCENTER << FRACBITS) + FRACUNIT / 4 -
    (psp->sy + ABS(viewplayer->bounce) - spritetopoffset[lump]);
    ...
    if(interpolatesprites)
    {
        ...
        if(lump == psp_inter.lump && !skippsprinterp)
        {
            ...
            vis->x2         = MIN(vis->x1 + deltax, render.view_width);
            vis->texturemid = psp_inter.texturemid +
            FixedMul(vis->texturemid - psp_inter.texturemid, fractionaltic);
        }
        else
        {
            psp_inter.x1         = vis->x1;
    ...
    vis->texturemid += FixedMul(((r_centery - render.view_height / 2) << FRACBITS), pspriteiscale);
    ...
    else
    {
        ...
        if(r_sprites_translucency)
        {
            if(!r_textures)
            {
                vis->colfunc  = (psp == &viewplayer->psprites[1] ?
                 &R_DrawTranslucent50SolidColorColumn :
                 &R_DrawSolidColorColumn);
                vis->colormap = NULL;
            }
            else if(spr == SPR_SHT2)
                vis->colfunc = ((frame & FF_FRAMEMASK) && (frame & FF_FULLBRIGHT) &&
                (!altered || state->translucent) ?
                tlredwhitecolfunc1 :
                basecolfunc);
            else if(muzzleflash && spr >= SPR_SHTG && spr <= SPR_BFGF &&
            (!altered || state->translucent))
            {
                void (*colfuncs[])(void) = { NULL, NULL,
        ...
        else
        {
            ...
            {
                const int lightnum =
                ((sec->floorlightsec ? sec->floorlightsec : sec)->lightlevel >> OLDLIGHTSEGSHIFT) +
                extralight;

                vis->colormap =
                psprscalelight[MIN(lightnum, OLDLIGHTLEVELS - 1)][MIN(lightnum + 16, OLDMAXLIGHTSCALE - 1)];
            }
...
// V_FillRect
//
static void R_FillRect(int screen,
int x,
int y,
int width,
int height,
...
const byte* tinttab2)
{
    byte* dest = &r_screens[screen][y * render.screen_width + x];
...
{
    TracyCZoneN(zone_psprites, "R_DrawPlayerSprites", 1);
    ...
    if(!weaponstate)
    {
        TracyCZoneEnd(zone_psprites)
        return;
    }

    altered = (weaponinfo[viewplayer->readyweapon].altered ||
    weaponstate->dehacked || !r_fixspriteoffsets);
    ...
    if(invisibility && (invisibility > STARTFLASHING || (invisibility & FLASHONTIC)))
    {
        ...
        R_FillRect(1, render.view_window_x, render.view_window_y, render.view_width, render.view_height, PINK, 0,
...
static void R_DrawBloodSplatSprite(const vissplat_t* splat)
{
    ...
    for(drawseg_t* ds = ds_p; ds-- > drawsegs;)
    {
        ...
        if(ds->maxscale < scale ||
        (ds->minscale < scale && !R_PointOnSegSide(gx, gy, ds->curline)))
            continue;
        else
        {
...
{
    TracyCZoneN(zone_sort, "R_SortVisSprites", 1);
    ...
    TracyCZoneEnd(zone_sort)
}

static void R_DrawSprite(const vissprite_t* spr)
{
    TracyCZoneN(tracy_zone, "R_DrawSprite", 1);
    const int x1        = spr->x1;
    const int x2        = spr->x2;
    ...
    if(spr->heightsec) // only things in specially marked sectors
    {
        ...
        if(mh > spr->gz && (h = r_centeryfrac - FixedMul((mh -= viewz), scale)) >= 0 &&
        (h >>= FRACBITS) < render.view_height)
        {
        ...
        (h = r_centeryfrac - FixedMul(mh - viewz, scale)) >= 0 && (h >>= FRACBITS) < render.view_height)
        {
...
{
    TracyCZoneN(tracy_zone, "R_DrawMasked", 1);
    if(consoleactive || paused || freeze)
    {
        M_Fuzz1Seed(game.stats.maptime);
        M_Fuzz2Seed(game.stats.maptime);
    }
...
// Allocates or reallocates sprite rendering buffers for the current scale.
// Called by R_ResizeRenderState when buffer sizes need to increase.
//
// Buffer sizes:
//   negonearray, viewheightarray, cliptop, clipbot: r_alloc_max_width elements
//
void R_ResizeThingsBuffers(void)
{
    // Free existing buffers
    if(negonearray) free(negonearray);
    if(viewheightarray) free(viewheightarray);
    if(cliptop) free(cliptop);
    if(clipbot) free(clipbot);

    // Allocate new buffers based on current allocation sizes
    negonearray = calloc(r_alloc_max_width, sizeof(int));
    viewheightarray = calloc(r_alloc_max_width, sizeof(int));
    cliptop = calloc(r_alloc_max_width, sizeof(int));
    clipbot = calloc(r_alloc_max_width, sizeof(int));

    // Verify allocations succeeded
    if(!negonearray || !viewheightarray || !cliptop || !clipbot)
        I_Error("R_ResizeThingsBuffers: Failed to allocate sprite buffers");
}

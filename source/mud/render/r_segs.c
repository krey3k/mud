...
static void R_FixWiggle(sector_t* sector)
{
    ...
    int height =
    MAX(1, (sector->interpceilingheight - sector->interpfloorheight) >> FRACBITS);
    ...
    if(height != sector->cachedheight)
    {
        ...
        const scalevalues_t scalevalues[] = { { 2048 * FRACUNIT, 12 },
            { 1024 * FRACUNIT, 12 }, { 1024 * FRACUNIT, 11 },
...
{
    return scalelight[BETWEEN(0,
    (lightlevel >> LIGHTSEGSHIFT) + extralight + curline->fakecontrast, LIGHTLEVELS - 1)];
}

static void R_BlastMaskedSegColumn(const rcolumn_t* column)
{
    TracyCZoneN(tracy_zone, "R_BlastMaskedSegColumn", 1);
    ...
    while(dc_numposts--)
    {
        ...
        if((dc_yh = MIN((int)((topscreen + (int64_t)spryscale * post->length) >> FRACBITS),
            dc_floorclip)) >= 0)
...
{
    TracyCZoneN(tracy_zone, "R_RenderMaskedSegRange", 1);
    int texnum;
    fixed_t texheight;
    ...
    if(fixedcolormap)
    {
        ...
        dc_sectorcolormap =
        (frontsector->colormap && viewplayer->fixedcolormap != INVERSECOLORMAP ?
    ...
    else
    {
        ...
        dc_sectorcolormap =
        (frontsector->colormap ? colormaps[frontsector->colormap] : fullcolormap);
    }
    ...
    if(curline->linedef->flags & ML_DONTPEGBOTTOM)
        ...
        dc_texturemid =
        MIN(frontsector->interpceilingheight, backsector->interpceilingheight) -
    ...
    for(dc_x = x1; dc_x <= x2; dc_x++, spryscale += rw_scalestep)
        if(maskedtexturecol[dc_x] != INT_MAX)
        {
            ...
            R_GetPatchColumnWrapped(patch, maskedtexturecol[dc_x]);
            ...
            if((dc_numposts = column->numposts))
            {
                ...
                //
                // sprtopscreen = r_centeryfrac - FixedMul(dc_texturemid, spryscale);
                //
                // This code fixes it, by using double-precision intermediate
                // arithmetic and by skipping the drawing of 2s normals whose
                // mapping to screen coordinates is totally out of range:
                const int64_t t = ((int64_t)r_centeryfrac << FRACBITS) -
                (int64_t)dc_texturemid * spryscale;

                // skip if the texture is out of screen's range
                if(t + (int64_t)texheight * spryscale < 0 ||
                t > (int64_t)render.screen_height << FRACBITS * 2)
                    continue;
                ...
                {
                    const int index =
                    MIN(spryscale >> LIGHTSCALESHIFT, MAXLIGHTSCALE - 1);
...
static void R_RenderSegLoop(void)
{
    ...
    for(; rw_x < rw_stopx; rw_x++)
    {
        ...
        if(segtextured)
        {
            ...
            const angle_t angle = MIN((rw_centerangle + xtoviewangle[rw_x]) >> ANGLETOFINESHIFT,
            FINEANGLES / 2 - 1);

            texturecolumn =
            (rw_offset - FixedMul(finetangent[angle], rw_distance)) >> FRACBITS;
        ...
        if(midtexture && yh >= yl)
        {
            ...
            ceilingclip[rw_x] = render.view_height;
            floorclip[rw_x]   = -1;
        }
        else
        {
            ...
            if(toptexture)
            {
                ...
                if(mid >= yl)
                {
                    ...
                    else
                    {
                        ...
                        dc_texturemid = rw_toptexturemid + (dc_yl - r_centery + 1) * SPARKLEFIX;
            ...
            {
                // bottom wall
                const int mid = MAX((int)((pixlow + heightunit - 1) >> heightbits),
                ceilingclip[rw_x] + 1);
...
// Returns the texture mapping scale for the current line (horizontal span) at
// the given angle. rw_distance must be calculated first.
//
static fixed_t R_ScaleFromGlobalAngle(angle_t visangle)
{
    const int angle = ANG90 + visangle;
    const int den = FixedMul(rw_distance, finesine[angle >> ANGLETOFINESHIFT]);
    const fixed_t num = FixedMul(r_projection,
    finesine[(angle + viewangle - rw_normalangle) >> ANGLETOFINESHIFT]);
...
{
    TracyCZoneN(tracy_zone, "R_StoreWallRange", 1);
    int64_t dx, dy;
    int64_t dx1, dy1;
    int64_t len;
    int worldtop;
    ...
    if((rw_stopx - start) * sizeof(*lastopening) + (lastopening - openings) > r_alloc_max_screen_area)
    {
        TracyCZoneEnd(tracy_zone)
        return;
    }
    ...
    (!frontsector->heightsec || viewz > frontsector->heightsec->interpfloorheight) && r_liquid_bob)
    ...
    if(!backsector)
    {
        ...
        else
        {
            ...
            midbrightmap =
            (usebrightmaps && !nobrightmap[midtexture] ? brightmap[midtexture] : NULL);
            rw_midtexturemid = ((linedef->flags & ML_DONTPEGBOTTOM) ?
                               frontsector->interpfloorheight + height - viewz :
                               worldtop) +
    ...
    else
    {
        ...
        !sidedef->midtexture && r_liquid_bob)
        {
        ...
        markfloor =
        (worldlow != worldbottom || backsector->floorpic != frontsector->floorpic ||
        ...
        if(worldhigh < worldtop)
        {
            ...
            else
            {
                ...
                toptexheight =
                ((linedef->r_flags & RF_TOP_TILE) ? 0 : (height >> FRACBITS));
                topbrightmap =
                (usebrightmaps && !nobrightmap[toptexture] ? brightmap[toptexture] : NULL);
                rw_toptexturemid = ((linedef->flags & ML_DONTPEGTOP) ?
                                   worldtop :
                                   backsector->interpceilingheight + height - viewz) +
                FixedMod(sidedef->rowoffset, height);
            }
        }
        ...
        frontsector->interpfloorheight != backsector->interpfloorheight)
        {
            ...
            else
            {
                ...
                bottomtexheight =
                ((linedef->r_flags & RF_BOT_TILE) ? 0 : (height >> FRACBITS));
                bottombrightmap = (usebrightmaps && !nobrightmap[bottomtexture] ?
                brightmap[bottomtexture] :
                NULL);
                rw_bottomtexturemid =
                ((linedef->flags & ML_DONTPEGBOTTOM) ? worldtop : worldlow - liquidoffset) +
                FixedMod(sidedef->rowoffset, height);
    ...
    topfrac = ((int64_t)r_centeryfrac >> invhgtbits) -
    (((int64_t)worldtop * rw_scale) >> FRACBITS);

    bottomstep = -FixedMul(rw_scalestep, (worldbottom >>= invhgtbits));
    bottomfrac = ((int64_t)r_centeryfrac >> invhgtbits) -
    ...
    if(backsector)
    {
        ...
        {
            pixhigh = ((int64_t)r_centeryfrac >> invhgtbits) -
        ...
        {
            pixlow = ((int64_t)r_centeryfrac >> invhgtbits) -
    ...
    if(markfloor)
    {
        ...
        if(floorplane)
        {
            ...
            // check being made, so we have to override it and split them anyway
            // if that is a possibility, otherwise the floor marking would
            // overwrite the ceiling marking, resulting in HOM.
    ...
    if((ds_p->silhouette & SIL_TOP) && !ds_p->sprtopclip)
    {
        ...
        sizeof(*lastopening) * ((size_t)rw_stopx - start));

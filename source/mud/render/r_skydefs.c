...
static bool ParseSkyTex(cJSON* json, skytex_t* out)
{
    ...
    out->scalex =
    (scalex->valuedouble ? (fixed_t)(1.0 / scalex->valuedouble * FRACUNIT) : FRACUNIT);
    out->scaley =
    (scaley->valuedouble ? (fixed_t)(1.0 / scaley->valuedouble * FRACUNIT) : FRACUNIT);
    return true;
}

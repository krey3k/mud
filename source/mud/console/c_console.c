...
void C_Input(const char* string, ...)
{
    ...
    if(numconsolestrings >= (int)consolestringsmax)
        console = I_Realloc(
        console, (consolestringsmax += CONSOLESTRINGSMAX) * sizeof(*console));
...
void C_IntegerCVAROutputNoRepeat(const char* cvar, const int value)
{
    ...
    if(numconsolestrings && M_StringStartsWith(console[numconsolestrings - 1].string, cvar))
        M_StringCopy(console[numconsolestrings - 1].string, buffer,
        sizeof(console[0].string));
    else
        C_Input(buffer);
...
void C_Output(const char* string, ...)
{
    ...
    M_StringCopy(console[numconsolestrings].string, buffer, sizeof(console[0].string));
    console[numconsolestrings].string[0] =
    toupper(console[numconsolestrings].string[0]);
    console[numconsolestrings].indent       = 0;
    console[numconsolestrings].wrap         = 0;
...
void C_TabbedOutput(const int tabs[MAXTABS], const char* string, ...)
{
    ...
    console[numconsolestrings].indent =
    (tabs[2] ? tabs[2] : (tabs[1] ? tabs[1] : tabs[0])) - 10;
    console[numconsolestrings++].wrap = 0;
    outputhistory                     = -1;
...
void C_Warning(const int minwarninglevel, const char* string, ...)
{
    ...
    if(numconsolestrings > 0 && console[i].stringtype == warningstring &&
    M_StringCompare(console[i].string, buffer))
        console[i].count++;
    else
    {
        if(numconsolestrings >= (int)consolestringsmax)
            console = I_Realloc(console,
            (consolestringsmax += CONSOLESTRINGSMAX) * sizeof(*console));

        M_StringCopy(
        console[numconsolestrings].string, buffer, sizeof(console[0].string));
...
void C_AddConsoleDivider(void)
{
    if(!numconsolestrings || console[numconsolestrings - 1].stringtype != dividerstring)
    {
        ...
        M_StringCopy(console[numconsolestrings].string, DIVIDERSTRING,
        sizeof(console[0].string));
        console[numconsolestrings++].stringtype = dividerstring;
    }
}

const kern_t altkern[] = { { ' ', ' ', -1 }, { ' ', 'J', -1 }, { ' ', 'T', -1 },
    { '!', ' ', 2 }, { '"', '+', -1 }, { '"', ',', -2 }, { '"', '.', -2 },
    { '"', '4', -1 }, { '"', 'a', -1 }, { '"', 'c', -1 }, { '"', 'd', -1 },
    { '"', 'e', -1 }, { '"', 'g', -1 }, { '"', 'J', -2 }, { '"', 'j', -2 },
    { '"', 'o', -1 }, { '"', 'q', -1 }, { '"', 's', -1 }, { '(', '(', -1 },
    ...
    { '\\', '\\', -2 }, { '\\', 'T', -1 }, { '\\', 't', -1 }, { '\\', 'V', -1 },
    { '\\', 'v', -1 }, { '\t', '-', 1 }, { '\t', '4', -1 }, { '_', 'f', -1 },
    { '_', 'T', -1 }, { '_', 't', -1 }, { '_', 'V', -1 }, { '_', 'v', -1 },
    { 'a', '"', -1 }, { 'a', '\\', -1 }, { 'a', 'j', -2 }, { 'b', '"', -1 },
    { 'b', ',', -1 }, { 'b', ';', -1 }, { 'b', '\\', -1 }, { 'b', 'j', -2 },
    { 'c', '"', -1 }, { 'c', ',', -1 }, { 'c', ';', -1 }, { 'c', '\\', -1 },
    { 'c', 'j', -2 }, { 'D', 'j', -1 }, { 'd', 'j', -2 }, { 'e', '"', -1 },
    { 'e', ')', -1 }, { 'e', ',', -1 }, { 'e', ';', -1 }, { 'e', '\\', -1 },
    { 'e', '_', -1 }, { 'e', 'j', -2 }, { 'F', ' ', -1 }, { 'f', ' ', -1 },
    { 'F', ',', -1 }, { 'f', ',', -1 }, { 'F', '.', -1 }, { 'f', '.', -1 },
    { 'F', ';', -1 }, { 'f', ';', -1 }, { 'f', '_', -1 }, { 'F', '_', -2 },
    ...
    { 's', ',', -1 }, { 's', ';', -1 }, { 's', 'j', -2 }, { 'T', ' ', -1 },
    { 'T', ',', -1 }, { 'T', '.', -1 }, { 'T', ';', -1 }, { 'T', 'a', -1 },
    { 'T', 'e', -1 }, { 't', 'j', -2 }, { 'T', 'o', -1 }, { 't', 't', -1 },
    { 'u', 'j', -2 }, { 'V', ',', -1 }, { 'v', ',', -1 }, { 'V', '.', -1 },
    { 'v', '4', -1 }, { 'V', ';', -1 }, { 'v', ';', -1 }, { 'v', '_', -1 },
    { 'V', 'a', -1 }, { 'v', 'a', -1 }, { 'v', 'j', -2 }, { 'w', 'j', -2 },
    { 'x', '7', -1 }, { 'x', 'j', -2 }, { 'Y', ',', -1 }, { 'Y', '.', -1 },
    { 'Y', ';', -1 }, { 'z', 'j', -2 }, { '\0', '\0', 0 } };
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        prevletter == '[' || prevletter == '\t')
        {
            ...
            else
            {
                ...
                width += SHORT(
                (c >= 0 && c < CONSOLEFONTSIZE ? consolefont[c] : unknownchar)->width);
            }
        }
        else
        {
...
static int C_OverlayWidth(const char* text, const bool monospaced)
{
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        if(letter == ' ')
            ...
            width += (monospaced ? zerowidth :
                                   SHORT(consolefont[letter - CONSOLEFONTSTART]->width));
        else if(letter >= CONSOLEFONTSTART)
            width +=
            (SHORT(consolefont[letter - CONSOLEFONTSTART]->width) - (letter == ','));
    }
...
{
    scrollbarfacestart = CONSOLESCROLLBARHEIGHT *
    MAX(0,
    (outputhistory == -1 ? numconsolestrings - CONSOLEBLANKLINES - CONSOLELINES :
                           outputhistory - CONSOLEBLANKLINES)) /
    numconsolestrings;
    scrollbarfaceend = scrollbarfacestart + CONSOLESCROLLBARHEIGHT -
    CONSOLESCROLLBARHEIGHT *
    ...
    {
        const int offset = (CONSOLEHEIGHT - consoleheight) * video.screen_width;
        const int gripstart =
        (scrollbarfacestart + (scrollbarfaceend - scrollbarfacestart) / 2 - 2) * video.screen_width - offset;
        const int trackend = MAX(0, CONSOLESCROLLBARHEIGHT * video.screen_width - offset);

        // draw scrollbar track
        for(int y = 0; y < trackend; y += video.screen_width)
        ...
        {
            const int gripend = gripstart + 6 * video.screen_width;

            // init scrollbar grip
            for(int y = gripstart; y < gripend; y += 2 * video.screen_width)
                if(y >= 0)
                    for(int x = CONSOLESCROLLBARX + 1;
                    x < CONSOLESCROLLBARX + CONSOLESCROLLBARWIDTH - 1; x++)
                        tempscreen[y + x] = v_screens[0][y + x];

            // draw scrollbar face
            const int end = scrollbarfaceend * video.screen_width - offset;

            for(int y = scrollbarfacestart * video.screen_width - offset; y < end; y += video.screen_width)
                if(y >= 0)
                    for(int x = CONSOLESCROLLBARX;
                    x < CONSOLESCROLLBARX + CONSOLESCROLLBARWIDTH; x++)
                        v_screens[0][y + x] = consolescrollbarfacecolor;
            ...
            for(int y = gripstart; y < gripend; y += 2 * video.screen_width)
                if(y >= 0)
                    for(int x = CONSOLESCROLLBARX + 1;
                    x < CONSOLESCROLLBARX + CONSOLESCROLLBARWIDTH - 1; x++)
                        v_screens[0][y + x] = tempscreen[y + x];
        }
        else
        {
            const int end = scrollbarfaceend * video.screen_width - offset;
        ...
        if(scrollbarfaceend >= 0)
            for(int x = CONSOLESCROLLBARX; x < CONSOLESCROLLBARX + CONSOLESCROLLBARWIDTH; x++)
            {
                byte* dot = *v_screens + scrollbarfaceend + x;
...
static void C_InitEdgeColors(void)
{
    ...
    !(consoleedgecolor1 >= nearestcolors[LIGHTGRAY1] &&
    consoleedgecolor1 <= nearestcolors[DARKGRAY4]) &&
...
void C_Init(void)
{
    ...
    for(int i = 0, j = CONSOLEFONTSTART; i < CONSOLEFONTSIZE; i++)
    {
        ...
        consolefont[i] =
        W_CacheLastLumpName(W_CheckNumForName(buffer) >= 0 ? buffer : "DRFON000");
    }
...
static void C_DrawBackground(void)
{
    ...
    const int height = (consoleheight + 5) * video.screen_width;
    static byte blurscreen[V_MAXSCREENAREA];

    // apply random noise to background
    M_BigSeed(666);

    for(int i = height; i >= 0; i--)
        v_screens[0][i] = colormaps[0][M_BigRandomInt(0, 7) * 256 + v_screens[0][i]];

    M_BigSeed((unsigned int)time(NULL));

    // blur background
    memcpy(blurscreen, v_screens[0], height);

    for(int y = 0; y <= height - video.screen_width; y += video.screen_width)
        for(int x = y; x <= y + video.screen_width - 2; x++)
            blurscreen[x] = tinttab50[(blurscreen[x + 1] << 8) + blurscreen[x]];

    for(int y = 0; y <= height - video.screen_width; y += video.screen_width)
        for(int x = y + video.screen_width - 2; x > y; x--)
            blurscreen[x] = tinttab50[(blurscreen[x - 1] << 8) + blurscreen[x]];

    for(int y = height - video.screen_width; y >= video.screen_width; y -= video.screen_width)
        for(int x = y + video.screen_width - 1; x >= y + 1; x--)
            blurscreen[x] =
            tinttab50[(blurscreen[x - video.screen_width - 1] << 8) + blurscreen[x]];

    for(int y = 0; y <= height - 2 * video.screen_width; y += video.screen_width)
        for(int x = y; x <= y + video.screen_width - 1; x++)
            blurscreen[x] =
            tinttab50[(blurscreen[x + video.screen_width] << 8) + blurscreen[x]];
    ...
    if(inverted)
        for(int i = 0; i < height; i++)
            v_screens[0][i] = consolebackcolor2[blurscreen[i]];
    else
        for(int i = 0; i < height; i++)
            v_screens[0][i] = consolebackcolor1[blurscreen[i]];

    // apply corrugated glass effect to background
    for(int y = consoleheight % 3; y <= height - 3 * video.screen_width; y += video.screen_width)
    {
        for(int x = y + 2; x < y + video.screen_width - 1; x += 3)
            v_screens[0][x] =
            colormaps[0][6 * 256 + v_screens[0][x + ((x % video.screen_width) ? -1 : 1)]];

        for(int x = (y += video.screen_width) + 1; x < y + video.screen_width - 1; x += 3)
            v_screens[0][x] =
            colormaps[0][6 * 256 + v_screens[0][x + ((x % video.screen_width) ? -1 : 1)]];

        for(int x = (y += video.screen_width); x < y + video.screen_width - 1; x += 3)
            v_screens[0][x] =
            colormaps[0][6 * 256 + v_screens[0][x + ((x % video.screen_width) ? -1 : 1)]];
    }
    ...
    {
        for(int i = height - 3 * video.screen_width; i < height; i++)
            v_screens[0][i] = tinttab60[consoleedgecolor1 + v_screens[0][i]];
    }
    else
    {
        ...
        for(int i = height - 3 * video.screen_width; i < height; i++)
            v_screens[0][i] = tinttab60[color1 + v_screens[0][i]];
    }

    // bevel left and right edges
    if(automapactive && am_backcolor == am_backcolor_default)
    {
        for(int i = 0; i < height - 3 * video.screen_width; i += video.screen_width)
            v_screens[0][i] = consoleautomapbevelcolor[v_screens[0][i + 1]];

        for(int i = MAX(0, height - 3 * video.screen_width); i < height; i += video.screen_width)
            v_screens[0][i] = consolebevelcolor1[v_screens[0][i + 1]];

        for(int i = 0; i < height - 3 * video.screen_width; i += video.screen_width)
            v_screens[0][i + video.screen_width - 1] =
            consoleautomapbevelcolor[v_screens[0][i + video.screen_width - 2]];

        for(int i = MAX(0, height - 3 * video.screen_width); i < height; i += video.screen_width)
            v_screens[0][i + video.screen_width - 1] =
            consolebevelcolor1[v_screens[0][i + video.screen_width - 2]];
    }
    else
        for(int i = 0; i < height; i += video.screen_width)
        {
            v_screens[0][i] = consolebevelcolor1[v_screens[0][i + 1]];
            v_screens[0][i + video.screen_width - 1] =
            consolebevelcolor1[v_screens[0][i + video.screen_width - 2]];
    ...
    // bevel bottom edge
    if(inverted)
        for(int i = height - video.screen_width + 1; i < height - 1; i++)
            v_screens[0][i] = consolebevelcolor2[v_screens[0][i]];
    else
        for(int i = height - video.screen_width + 1; i < height - 1; i++)
            v_screens[0][i] = consolebevelcolor1[v_screens[0][i]];

    // draw shadow
    for(int i = video.screen_width; i <= 4 * video.screen_width; i += video.screen_width)
        for(int j = height; j < height + i && j < video.screen_area; j++)
            v_screens[0][j] = colormaps[0][4 * 256 + v_screens[0][j]];
}

static int C_DrawConsoleText(int x,
int y,
char* text,
const int color1,
const int color2,
const int boldcolor,
const byte* tinttab,
const int tabs[MAXTABS],
const bool formatting,
const bool kerning,
const bool wrapped,
const int index,
unsigned char prevletter,
unsigned char prevletter2,
void consoletextfunc(const int, const int, const patch_t*, const int, const int, const int, const bool, const byte*))
{
    bool bold                 = false;
    bool bolder               = false;
    bool italics              = false;
    ...
    if(console[index].stringtype == warningstring || console[index].stringtype == playerwarningstring)
    {
        V_DrawConsoleTextPatch(
        x - 1, y, warning, WARNINGWIDTH, color1, color2, false, tinttab);
    ...
    {
        V_DrawConsoleTextPatch(
        x, y, altbuddha, altbuddhawidth, color1, color2, false, tinttab);
        return 0;
    }
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        else
        {
            ...
            if(letter == ' ' && formatting)
                ...
                x = (x > tabs[++tab] + CONSOLETEXTX - 10 ? x + spacewidth :
                                                           tabs[tab] + CONSOLETEXTX - 10);
            else if(letter == '(' && i < len - 3 && tolower(text[i + 1]) == 't' &&
            tolower(text[i + 2]) == 'm' && text[i + 3] == ')' && formatting)
            {
                patch = trademark;
            ...
            else if(letter == '(' && i < len - 2 &&
            tolower(text[i + 1]) == 'c' && text[i + 2] == ')' && formatting)
            {
                patch = copyright;
                i += 2;
            }
            else if(letter == '(' && i < len - 2 &&
            tolower(text[i + 1]) == 'r' && text[i + 2] == ')' && formatting)
            {
                patch = regomark;
                i += 2;
            }
            else if(letter == 'x' && isdigit(prevletter) &&
            (i == len - 1 || isdigit(nextletter)))
                patch = multiply;
            else if(letter == '-' &&
            (prevletter == ' ' || (prevletter == BOLDONCHAR && prevletter2 == ' ')) &&
            !isdigit(nextletter))
                patch = endash;
            else if(letter == '\n')
                break;
            ...
            if(patch)
            {
                ...
                consoletextfunc(
                x + (monospaced && width <= zerowidth ? (zerowidth - width) / 2 : 0),
                y, patch, width,
                (bold && italics ? (color1 == consolewarningcolor ? color1 : consolebolditalicscolor) :
                                   (bold ? boldcolor : color1)),
                color2,
                (italics && letter != '_' && letter != '-' && letter != '+' &&
                letter != ',' && letter != '/' && patch != unknownchar),
                (bolder ? NULL : tinttab));
                ...
                if(x >= CONSOLETEXTPIXELWIDTH && wrapped)
                {
                    for(int j = 1; j <= 3; j++)
                    {
                        ...
                        (bold && italics ? (color1 == consolewarningcolor ? color1 : consolebolditalicscolor) :
                                           (bold ? boldcolor : color1)),
                        color2, false, (bolder ? NULL : tinttab));
...
static void C_DrawOverlayText(byte* screen,
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        else
        {
            ...
            if(isdigit(letter) && monospaced)
            {
                ...
                x + (letter == '1') - (letter == '4'), y, patch, width - 1,
                color, shadowcolor, tinttab);
                x += zerowidth;
            }
            else
            {
...
static void C_DrawTimeStamp(int x, const int y, const int index, const int color)
{
    ...
    if(con_timestampformat == con_timestampformat_standard)
    {
        ...
        M_snprintf(buffer, sizeof(buffer), "%02i:%02i:%02i", timestamp.tm_hour,
    ...
    for(int i = (int)strlen(buffer) - 1; i >= 0; i--)
    {
        ...
        V_DrawConsoleTextPatch(x + (ch == '1') - (ch == '4'), y, patch, width,
        color, NOBACKGROUNDCOLOR, false, tinttab33);
    }
}
...
static byte C_GetOverlayTextColor(void)
{
    ...
    !automapactive && game.state == GS_LEVEL)
        return nearestblack;
...
{
    return ((viewplayer->fixedcolormap == INVERSECOLORMAP) != !r_textures && !r_hud_translucency ?
    -1 :
    nearestdarkgray);
}
...
{
    const int x     = video.screen_width - fpswidth - OVERLAYTEXTX;
    const int color = C_GetOverlayTextColor();
    int shadowcolor = C_GetOverlayTextShadowColor();
    const byte* tinttab =
    (r_hud_translucency ? (automapactive ? tinttab70 : tinttab50) : NULL);
    ...
    int graphx = video.screen_width - OVERLAYFPSGRAPHWIDTH - OVERLAYTEXTX;
    ...
    for(int i = 0; i < OVERLAYFPSGRAPHWIDTH; i++)
    {
        ...
        if(maxfpsseen > 0)
        {
            int offset = (fpshistory[(fpshistoryindex + i) % OVERLAYFPSGRAPHWIDTH] *
                         OVERLAYFPSGRAPHHEIGHT) /
            maxfpsseen;
    ...
    for(int yy = graphy; yy < graphy + OVERLAYFPSGRAPHHEIGHT + 1; yy++)
        for(int xx = graphx; xx < graphx + OVERLAYFPSGRAPHWIDTH; xx++)
            tempscreen[yy * video.screen_width + xx] = PINK;

    for(int i = 0; i < OVERLAYFPSGRAPHWIDTH; i++)
    {
        const int px = graphx + i;
        const int py = pyvals[i];
        byte* dest   = &tempscreen[py * video.screen_width + px];
        ...
        *dest = color;
        ...
        if(prevpx >= 0 && prevpy >= 0)
        {
            ...
            for(int j = 0; j < n; j++)
            {
                ...
                dest  = &tempscreen[y0 * video.screen_width + x0];
                *dest = color;
                dest += video.screen_width;
    ...
    if(tinttab)
    {
        for(int yy = graphy; yy < graphy + OVERLAYFPSGRAPHHEIGHT + 1; yy++)
            for(int xx = graphx; xx < graphx + OVERLAYFPSGRAPHWIDTH; xx++)
            {
                const int i    = yy * video.screen_width + xx;
                const byte dot = tempscreen[i];

                if(dot == BLUE1)
                {
                    byte* dest = &v_screens[0][i];
    ...
    V_DrawOverlayTextPatch(v_screens[0], video.screen_width, x,
    (graphy += OVERLAYFPSGRAPHHEIGHT + 2) + 3, fps, fpswidth, color, shadowcolor, tinttab);
    C_DrawOverlayText(v_screens[0], video.screen_width, x - C_OverlayWidth(temp, true) - 3,
...
void C_UpdateTimerOverlay(void)
{
    ...
    if(timeremaining != prevtime)
    {
        ...
        if(hours)
            M_snprintf(buffer, sizeof(buffer), "%i:%02i:%02i.%02i", hours,
            minutes, seconds % 60, millseconds / 10);
        else
            M_snprintf(buffer, sizeof(buffer), "%02i:%02i.%02i", minutes,
            seconds % 60, millseconds / 10);

        timerwidth = C_OverlayWidth(buffer, true);
    }

    C_DrawOverlayText(v_screens[0], video.screen_width, video.screen_width - timerwidth - OVERLAYTEXTX + 1,
    y, (r_hud_translucency ? (automapactive ? tinttab70 : tinttab50) : NULL),
    buffer, C_GetOverlayTextColor(), true, C_GetOverlayTextShadowColor());
}

void C_UpdatePlayerPositionOverlay(void)
{
    const int x            = video.screen_width - OVERLAYTEXTX + 1;
    int y                  = OVERLAYTEXTY;
    ...
    C_DrawOverlayText(v_screens[0], video.screen_width, x - C_OverlayWidth(angle, true),
    y, tinttab, angle, color, true, shadowcolor);
    C_DrawOverlayText(v_screens[0], video.screen_width, x - C_OverlayWidth(coordinates, true),
...
void C_UpdatePathOverlay(void)
{
    ...
    if(*distance)
    {
        ...
        C_DrawOverlayText(v_mapscreen, V_MAPWIDTH, x - width, y,
...
void C_UpdatePlayerStatsOverlay(void)
{
    ...
    if(game.stats.maptime != prevmaptime)
    {
        int seconds           = (prevmaptime = game.stats.maptime) / TICRATE;
    ...
    C_DrawOverlayText(v_mapscreen, V_MAPWIDTH, x - width, y, tinttab, buffer, color,
    true, shadowcolor);
    y += OVERLAYLINEHEIGHT + OVERLAYSPACING;
    ...
    if(game.stats.kills)
    {
        ...
        char* temp2 = commify(game.stats.kills);

        M_snprintf(kills, sizeof(kills), s_STSTR_KILLS, temp1, temp2);
        C_DrawOverlayText(v_mapscreen, V_MAPWIDTH, x - C_OverlayWidth(kills, false),
        y, tinttab, kills, color, false, shadowcolor);
        free(temp1);
        free(temp2);
    ...
    if(game.stats.items)
    {
        ...
        char* temp2 = commify(game.stats.items);

        M_snprintf(items, sizeof(items), s_STSTR_ITEMS, temp1, temp2);
        C_DrawOverlayText(v_mapscreen, V_MAPWIDTH, x - C_OverlayWidth(items, false),
        y, tinttab, items, color, false, shadowcolor);
        free(temp1);
        free(temp2);
    ...
    if(game.stats.secrets)
    {
        char secrets[32];
        char* temp1 = commify(viewplayer->secretcount);
        char* temp2 = commify(game.stats.secrets);

        M_snprintf(secrets, sizeof(secrets), s_STSTR_SECRETS, temp1, temp2);
        C_DrawOverlayText(v_mapscreen, V_MAPWIDTH, x - C_OverlayWidth(secrets, false),
        y, tinttab, secrets, color, false, shadowcolor);
        free(temp1);
        free(temp2);
    }
}
...
static bool IsCheatSequence(char* string)
{
    ...
    isdigit(string[length - 1]) && isdigit(string[length - 2]))
    {
        static char lump[7];

        if(game.mode == commercial)
            M_snprintf(lump, sizeof(lump), "MAP%c%c", string[length - 2],
            string[length - 1]);
        else
            M_snprintf(lump, sizeof(lump), "E%cM%c", string[length - 2],
            string[length - 1]);
...
void C_Drawer(void)
{
    ...
    (outputhistory == -1 ? numconsolestrings : outputhistory + CONSOLELINES) - 1;
    int len;
    ...
    if(consolewait < tics)
    {
        consolewait = tics + (game.state == GS_TITLESCREEN ? 4 : 8);

        if(consoledirection == 1)
        {
            if(consoleheight < CONSOLEHEIGHT)
            {
                const int consoledown[CONSOLEDOWNSIZE] = { 12, 29, 45, 60, 84,
                    97, 109, 120, 130, 139, 147, 154, 160, 165, 169, 173, 176,
                    179, 182, 184, 186, 188, 190, 192, 194, 194, 195, 195 };

                const int height =
                (game.state == GS_TITLESCREEN ? consoledown[consoleanim] * 2 + 5 :
        ...
        else
        {
            if(consoleheight)
            {
                const int consoleup[CONSOLEUPSIZE] = { 183, 167, 150, 133, 117,
                    100, 83, 67, 50, 33, 17, 0 };

                const int height =
                consoleup[consoleanim] * (game.state == GS_TITLESCREEN ? 2 : 1);
    ...
    (joy_rumble_barrels || joy_rumble_damage || joy_rumble_fall ||
    ...
    for(i = bottomline; i >= 0; i--)
    {
        ...
        {
            int yy = (y + 5 - (CONSOLEHEIGHT - consoleheight)) * video.screen_width;

            if(yy >= 0)
                for(int xx = yy + CONSOLETEXTX;
                xx < yy + CONSOLETEXTPIXELWIDTH + CONSOLETEXTX + 7; xx++)
                {
                    byte* dest = &v_screens[0][xx];

                    *dest = tinttab50[consoledividercolor + *dest];
                }

            if((yy += video.screen_width) >= 0)
            {
        ...
        else if(!(topofconsole = !((len = (int)strlen(console[i].string)))))
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    stringtype == playerwarningstring || !indent)
            ...
            if(stringtype == playermessagestring)
            {
                ...
                if(count > 1)
                {
                    ...
                    NOBACKGROUNDCOLOR, consoleplayermessagecolor, tinttab66, notabs,
                    true, true, false, i, '\0', '\0', &V_DrawConsoleTextPatch);
                    free(temp);
                }
                else
                ...
                if(con_timestamps)
                    C_DrawTimeStamp(video.screen_width - CONSOLETEXTX - CONSOLESCROLLBARWIDTH - 7,
                ...
                NOBACKGROUNDCOLOR, consoleboldcolor, tinttab66, console[i].tabs,
                true, true, false, i, '\0', '\0', &V_DrawConsoleTextPatch);
            else if(stringtype == inputstring || stringtype == cheatstring)
                C_DrawConsoleText(CONSOLETEXTX, y, text, consoleinputcolor,
                NOBACKGROUNDCOLOR, consoleboldcolor, tinttab75, notabs, true,
                true, false, i, '\0', '\0', &V_DrawConsoleTextPatch);
            else if(stringtype == warningstring)
            {
                ...
                if(count > 1)
                {
                    ...
                    M_snprintf(buffer, sizeof(buffer), "%s (%s)", text, temp);
                    C_DrawConsoleText(CONSOLETEXTX, y, buffer, consolewarningcolor,
                    NOBACKGROUNDCOLOR, consolewarningboldcolor, tinttab66, notabs,
                    true, true, false, i, '\0', '\0', &V_DrawConsoleTextPatch);
                    free(temp);
                }
                else
            ...
            else if(stringtype == playerwarningstring)
            {
                ...
                V_DrawConsoleHeaderPatch(CONSOLETEXTX,
                y + 4 - (CONSOLEHEIGHT - consoleheight), console[i].header,
                CONSOLETEXTPIXELWIDTH + 7, consoleedgecolor1,
                (luminance[consoleedgecolor1 >> 8] <= 128 ? nearestwhite : nearestblack));
            else
                V_DrawConsoleHeaderPatch(CONSOLETEXTX,
                y + 4 - (CONSOLEHEIGHT - consoleheight), console[i].header,
                CONSOLETEXTPIXELWIDTH + 7, (nearestcolors[con_edgecolor] << 8),
            ...
            if(wrap < len && i < bottomline)
            {
                ...
                C_DrawConsoleText(CONSOLETEXTX + console[i].indent,
                y + CONSOLELINEHEIGHT, trimwhitespace(temp), consolecolors[stringtype],
                NOBACKGROUNDCOLOR, consoleboldcolors[stringtype], tinttab66,
    ...
    if(consoleinput[0] != '\0')
    {
        ...
        if(partialinput[0] != '\0')
        {
            x += C_DrawConsoleText(x, CONSOLEINPUTY, partialinput,
            consoleinputcolor, NOBACKGROUNDCOLOR, NOBOLDCOLOR, NULL, notabs,
            false, true, false, 0, '\0', '\0', &V_DrawConsoleTextPatch);
        ...
        if(selectstart < caretpos)
        {
            ...
            if(partialinput[0] != '\0')
            {
                for(i = 1; i < CONSOLELINEHEIGHT - 1; i++)
                {
                    ...
                    if(yy >= 0)
                        v_screens[0][yy * video.screen_width + x - 1] = consoleselectedinputbackgroundcolor;
                }

                x += C_DrawConsoleText(x, CONSOLEINPUTY, partialinput,
                consoleselectedinputcolor, consoleselectedinputbackgroundcolor,
                NOBOLDCOLOR, NULL, notabs, false, true, false, 0, prevletter,
                prevletter2, &V_DrawConsoleSelectedTextPatch);
                ...
                for(i = 1; i < CONSOLELINEHEIGHT - 1; i++)
                {
                    ...
                    if(yy >= 0)
                        v_screens[0][yy * video.screen_width + x] = consoleselectedinputbackgroundcolor;
                }
            }
        }
    }
    ...
    if(consoleheight == CONSOLEHEIGHT && windowfocused && !messagetoprint)
    {
        ...
        if(showcaret)
        {
            byte* dest = &v_screens[0][CONSOLEINPUTY * video.screen_width + x];

            for(int yy = 0; yy < CONSOLELINEHEIGHT * video.screen_width; yy += video.screen_width)
            {
    ...
    if(consoleinput[0] != '\0')
    {
        ...
        if(caretpos < (len = (int)strlen(consoleinput)))
        {
            ...
            if(partialinput[0] != '\0')
                ...
                false, 0, prevletter, prevletter2, &V_DrawConsoleTextPatch);
        }
    }
        ...
        else
        {
            ...
            if(sscanf(input, "%127s %127[^\n]", cmd, parms) > 0)
            {
                ...
                (consolecmds[i].parameters || !*temp))
                {
                    ...
                    {
                        if(temp[0] != '\0')
                            C_Input((input[length - 1] == '%' ? "%s %s%" : "%s %s"),
                            cmd, parms);
                        else
...
bool C_Responder(event_t* ev)
{
    ...
    if(ev->type == ev_keydown)
    {
        ...
        case KEY_HOME:
            ...
            numconsolestrings > CONSOLELINES)
...
#else
                selectstart = (false ? caretpos : len);
#endif
        ...
        case KEY_TAB:
            ...
            if(consoleinput[0] != '\0' && caretpos == len)
            {
                ...
                for(i = len - 1; i >= 0; i--)
                    if((consoleinput[i] == ';' && M_StringStartsWith(consoleinput, "bind") &&
                       M_StringStartsWith(consoleinput, "unbind") &&
                       M_StringStartsWith(consoleinput, "alias")) ||
                    (consoleinput[i] == '"' && M_StringStartsWith(consoleinput, "bind")))
                        break;
                ...
                (scrolldirection == 1 &&
                (autocomplete == -1 || *autocompletelist[autocomplete].text)))
                {
                    static char output[255];

                    autocomplete += scrolldirection;
                    M_StringCopy(
                    output, autocompletelist[autocomplete].text, sizeof(output));

                    if(!M_StringCompare(output, input))
                    {
                        const int gametype = autocompletelist[autocomplete].game;
                        const int len2     = (int)strlen(output);
                        const int spaces2  = numspaces(output);
                        const bool endspace2 = (len2 > 0 && output[len2 - 1] == ' ');

                        if((gametype == DOOM1AND2 ||
                           (game.mission == doom && gametype == DOOM1ONLY) ||
                           (game.mission != doom && gametype == DOOM2ONLY)) &&
                        M_StringStartsWith(output, input) &&
                        input[strlen(input) - 1] != '+' &&
                        ((!spaces1 && (!spaces2 || (spaces2 == 1 && endspace2))) ||
                        (spaces1 == 1 && !endspace1 &&
                        (spaces2 == 1 || (spaces2 == 2 && endspace2))) ||
                        (spaces1 == 2 && !endspace1 &&
                        (spaces2 == 2 || (spaces2 == 3 && endspace2))) ||
                        (spaces1 == 3 && !endspace1)))
                        {
                            char* temp;

                            if(isuppercase(input))
                                temp = M_StringJoin(prefix,
                                M_StringReplaceFirst(uppercase(output), input, input), NULL);
                            else if(islowercase(input))
                                temp = M_StringJoin(prefix,
                                M_StringReplaceFirst(lowercase(output), input, input), NULL);
                            else
                                temp = M_StringJoin(prefix,
                                M_StringReplaceFirst(output, input, input), NULL);

                            C_AddToUndoHistory();
                            M_StringCopy(consoleinput, temp, sizeof(consoleinput));
                            caretpos = selectstart = selectend =
                            len2 + (int)strlen(prefix);
                            caretwait = I_GetTimeMS() + CARETBLINKTIME;
                            showcaret = true;
                            free(temp);
                            return true;
        ...
        case KEY_UPARROW:
#ifdef MUD_SOKOL_PORT
            if((modstate & KMOD_CTRL) && !topofconsole &&
            numconsolestrings > CONSOLELINES && scrollbardrawn)
#else
            if((false) && !topofconsole && numconsolestrings > CONSOLELINES && scrollbardrawn)
#endif
            {
                // scroll output up
                scrollspeed   = MIN(scrollspeed + 4, TICRATE * 8);
                outputhistory = (outputhistory == -1 ?
                numconsolestrings - (CONSOLELINES + 1) :
            ...
            else
            {
                ...
                for(i = (inputhistory == -1 ? numconsolestrings : inputhistory) - 1;
                i >= 0; i--)
...
#else
            if((false) && outputhistory != -1 && scrollbardrawn)
#endif
            {
                // scroll output down
                scrollspeed = MIN(scrollspeed + 4, TICRATE * 8);
            ...
            else
            {
                ...
                if(inputhistory != -1)
                {
                    for(i = inputhistory + 1; i < numconsolestrings; i++)
                        ...
                        {
                            inputhistory = i;
                            M_StringCopy(consoleinput, console[i].string,
                            sizeof(consoleinput));
                            break;
                        }
...
#else
            if((false) && selectstart < selectend)
#endif
            {
                char* temp = M_SubString(
                consoleinput, selectstart, (size_t)selectend - selectstart);
...
#ifdef MUD_SOKOL_PORT
                M_snprintf(buffer, sizeof(buffer), "%s%s%s", temp1,
                SDL_GetClipboardText(), temp2);
                M_StringCopy(buffer, M_StringReplaceFirst(buffer, "(null)", ""),
                sizeof(buffer));
                M_StringCopy(buffer, M_StringReplaceFirst(buffer, "(null)", ""),
                sizeof(buffer));
#endif
    ...
    else if(ev->type == ev_textinput)
    {
        ...
        if(ch >= CONSOLEFONTSTART && ch != keyboardconsole && ch != keyboardconsole2 &&
        C_TextWidth(consoleinput, false, true) +
        (ch == ' ' ? spacewidth : SHORT(consolefont[ch - CONSOLEFONTSTART]->width)) -
        (selectstart < selectend ?
        C_TextWidth((temp = M_SubString(consoleinput, selectstart, (size_t)selectend - selectstart)),
        false, true) :
        0) <=
        CONSOLEINPUTPIXELWIDTH)
        {
            C_AddToUndoHistory();
    ...
    else if(ev->type == ev_mouse)
    {
        ...
        {
            const int x = (ev->data2 - (vid_widescreen ? 0 : video.max_widescreen_delta)) * 2;
            const int y = ev->data3 * 2;

            // hide console
            if(y >= video.screen_height / 2 && game.state == GS_LEVEL)
                C_HideConsole();
            ...
            else if(len && y >= CONSOLEINPUTY - 2 && y < CONSOLEINPUTY + CONSOLELINEHEIGHT)
            {
                for(i = 0; i < len; i++)
                {
                    ...
                    if(x <= CONSOLEINPUTX + C_TextWidth(temp1, false, true) +
                    C_TextWidth(temp2, false, true) / 2)
                    {
                        free(temp1);
                        free(temp2);
                        break;
            ...
            else if(scrollbardrawn && x >= CONSOLESCROLLBARX &&
            x <= CONSOLESCROLLBARX + CONSOLESCROLLBARWIDTH)
            {
                // scroll output up
                ...
                y < CONSOLESCROLLBARHEIGHT - (CONSOLEHEIGHT - consoleheight))
                {
                    if((outputhistory += scrollspeed / TICRATE) + CONSOLELINES >= numconsolestrings)
                        outputhistory = -1;
                }
    ...
    else if(ev->type == ev_mousewheel && scrollbardrawn)
    {
        ...
        if(ev->data1 > 0)
        {
            if(!topofconsole && numconsolestrings > CONSOLELINES)
                outputhistory =
                (outputhistory == -1 ? numconsolestrings - (CONSOLELINES + 1) :
    ...
    {
        // hide console
        if(((controllerbuttons & controllerconsole) || (controllerbuttons & CONTROLLER_B)) &&
        ...
        // scroll output up
        else if((controllerthumbLY < 0 || controllerthumbRY < 0 ||
...
void C_PrintCompileDate(void)
{
    ...
    sscanf(__TIME__, "%02d:%02d:%*d", &hour, &minute) == 2)
    {
        ...
        const int month =
        (int)(strstr(shortmonthnames, shortmonthname) - shortmonthnames) / 3;

        C_Output("This %i-bit " ITALICS("%s") " app of " ITALICS(
                 "%s") " was built with love by %s at %i:%02i%s on %s, %s %i, "
                       "%i somewhere in %s.",
        8 * (int)sizeof(intptr_t), WINDOWS, DOOMRETRO_NAMEANDVERSIONSTRING,
        DOOMRETRO_CREATOR, (hour ? hour - 12 * (hour > 12) : 12), minute,
        (hour < 12 ? "(AM)" : "(PM)"), dayofweek(day, month + 1, year),
        monthnames[month], day, year, DOOMRETRO_HOMEOFCREATOR);
    }
#if defined(__GNUC__)
    ...
    if(_MSC_BUILD)
        C_Output("It was compiled using v%i.%02i.%i.%i of the " ITALICS(
                 "Microsoft C/C++ Optimizing Compiler."),
        _MSC_FULL_VER / 10000000, (_MSC_FULL_VER % 10000000) / 100000,
        _MSC_FULL_VER % 100000, _MSC_BUILD);
    else
        C_Output("It was compiled using v%i.%02i.%i of the " ITALICS(
                 "Microsoft C/C++ Optimizing Compiler."),
        _MSC_FULL_VER / 10000000, (_MSC_FULL_VER % 10000000) / 100000,

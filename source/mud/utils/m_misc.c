...
const char* daynames[7] = { "Sunday", "Monday", "Tuesday", "Wednesday",
    "Thursday", "Friday", "Saturday" };

const char* monthnames[12] = { "January", "February", "March", "April", "May",
    "June", "July", "August", "September", "October", "November", "December" };

// Create a directory
void M_MakeDirectory(const char* path)
{
    FS_MakeDir(path, FS_TRUE);
}

// Check if a file exists
bool M_FileExists(const char* filename)
{
    fs_file* fstream = FS_OpenFile(filename, FS_READ, FS_TRUE);

    if(fstream)
    {
        FS_CloseFile(fstream);
        return true;
    }
...
{
    fs_file_info info;
    if (FS_GetInfo(&info, folder, FS_TRUE) != FS_SUCCESS)
        return false;
...
{
    if(type == personal)
        return (playergender == playergender_male ?
        "he" :
...
const char* words[][2] = { { "agoniz", "agonis" }, { "airplane", "aeroplane" },
    { "analog", "analogue" }, { "armor", "armour" }, { "artifact", "artefact" },
    { "barreled", "barrelled" }, { "behavior", "behaviour" },
    { "caliber", "calibre" }, { "centered", "centred" }, { "centering", "centring" },
    { "center", "centre" }, { "color", "colour" }, { "defense", "defence" },
    { "dialog", "dialogue" }, { "disk", "disc" }, { "donut", "doughnut" },
    { "endeavor", "endeavour" }, { "favor", "favour" }, { "fiber", "fibre" },
    { "flavor", "flavour" }, { "gray", "grey" }, { "harbor", "harbour" },
    { "honor", "honour" }, { "humor", "humour" }, { "initializ", "initialis" },
    { "inquir", "enquir" }, { "jewelry", "jewellery" }, { "judgment", "judgement" },
    { "labor", "labour" }, { "license", "licence" }, { "liter", "litre" },
    { "meter", "metre" }, { "neighbor", "neighbour" }, { "offense", "offence" },
    { "organiz", "organis" }, { "practice", "practise" }, { "program", "programme" },
    { "realiz", "realis" }, { "randomiz", "randomis" }, { "recogniz", "recognis" },
    { "refueling", "refuelling" }, { "rumor", "rumour" }, { "savior", "saviour" },
    { "savor", "savour" }, { "skeptic", "sceptic" }, { "specializ", "specialis" },
    { "stabiliz", "stabilis" }, { "standardiz", "standardis" },
    { "synchroniz", "synchronis" }, { "theater", "theatre" }, { "traveled", "travelled" },
    { "traveling", "travelling" }, { "utiliz", "utilis" }, { "vapor", "vapour" },

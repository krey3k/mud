...
const int nmus[] = { mus_messag, mus_ddtblu, mus_doom, mus_shawn, mus_in_cit,
...
static void InitSfxModule(void)
{
    if(I_InitSound())
    {
        C_Output("Sound effects are playing at %i%% volume and a sample rate "
                 "of %.1fkHz over %d sound channels.",
        (int)((float)s_sfxvolume / s_sfxvolume_max * 100), SAMPLERATE / 1000.0f, s_channels);
    }
    else
    {
        C_Warning(1, "Sound effects couldn't be %s.",
...
static void InitMusicModule(void)
{
    if(!I_InitMusic())
    {
        C_Warning(1, "Music couldn't be %s.",
        (english == english_american ? "initialized" : "initialised"));
        nomusic = true;
    }
}
    ...
    if(!nomusic)
    {
        ...
        for(int i = mus_e4m1, j = 0; i <= mus_e4m9; i++, j++)
        {
            ...
            if(W_CheckNumForName(namebuf) == -1)
            {
                ...
                M_StringCopy(
                music->title1, s_music[spmus[j]].title1, sizeof(music->title1));
            }
        }
    }
}
...
{
    if(!s_randommusic)
        return ((game.episode - 1) * 9 + game.map);

    if(game.mode == commercial)
        return M_RandomIntNoRepeat(1, (game.mission == pack_nerve ? 9 : 32), game.map);
    else
        return M_RandomIntNoRepeat(1, 4 * 9, (game.episode - 1) * 9 + game.map);
}
...
static int S_GetMusicNum(void)
{
    ...
    return (game.mode == commercial ?
    (game.mission == pack_nerve ? nmus[map] : mus_runnin + map) :
    mus_e1m1 + map);
}
...
static bool S_AdjustSoundParms(const mobj_t* origin, int* vol, int* sep)
{
    ...
    if(adx)
        dist = FixedDiv(adx,
        finesine[(tantoangle[FixedDiv(ady, adx) >> DBITS] + ANG90) >> ANGLETOFINESHIFT]);
    ...
    if(s_stereo)
    {
        ...
        *sep = NORM_SEP -
        FixedMul(S_STEREO_SWING, finesine[(angle - viewangle) >> ANGLETOFINESHIFT]);
    }

    // volume calculation
    *vol = (dist < S_CLOSE_DIST ? s_sfxvolume :
                                  s_sfxvolume * (S_CLIPPING_DIST - dist) / S_ATTENUATOR);

    return (*vol > 0);
}

static void S_StartSoundAtVolume(mobj_t* origin, sfxnum_t sfxnum)
{
    sfxinfo_t* sfx = &s_sfx[sfxnum];
    int sep        = NORM_SEP;
    int cnum;
    int handle;
    int volume = s_sfxvolume;
    ...
    if((handle = I_StartSound(sfx, cnum, channels[cnum].handle, volume, sep)) != -1)
...
void S_StartSound(mobj_t* mobj, const sfxnum_t sfxnum)
{
    ...
    {
        mobj->madesound = true;
        S_StartSoundAtVolume(mobj, sfxnum);
    }
    else
        S_StartSoundAtVolume(NULL, sfxnum);
}

void S_StartSectorSound(degenmobj_t* degenmobj, const sfxnum_t sfxnum)
{
    S_StartSoundAtVolume((mobj_t*)degenmobj, sfxnum);
}
...
void S_ChangeMusic(const musicnum_t musicnum, const bool looping, const bool allowrestart, const bool mapstart)
{
    ...
    if(mapstart && (mapinfomusic = P_GetMapMusic(game.episode, S_GetMapNum())) > 0)
    {
    ...
    if(!(music->stream = I_RegisterSong(music->data, W_LumpLength(music->lumpnum))))
    {
        char* temp = uppercase(namebuf);
...
void S_ChangeMusInfoMusic(const int lumpnum, const bool looping)
{
    ...
    if(!(music->stream = I_RegisterSong(music->data, W_LumpLength(lumpnum))))
        return;
    ...
    if(W_CheckNumForName("MUSINFO") >= 0)
    {
        ...
        while(SC_GetString())
            if(inmap || SC_Compare(lumpname))
            {
                ...
                if(M_StrToInt(sc_String, (int*)&num) && num > 0 &&
                num < MAX_MUS_ENTRIES && SC_GetString())
                {
...
void MusInfoThinker(mobj_t* thing)
{
    ...
    thing->subsector->sector == viewplayer->mo->subsector->sector)
    {
        musinfo.lastmapthing = musinfo.mapthing;
        musinfo.mapthing     = thing;
        musinfo.tics         = (game.stats.maptime ? 30 : 0);
    }
}

...
bool P_TeleportMove(mobj_t* thing, const fixed_t x, const fixed_t y, const fixed_t z, const bool boss)
{
    ...
    const int allowmonstertelefrags = P_GetAllowMonsterTelefrags(game.episode, game.map);

    telefrag = (boss ? allowmonstertelefrags != 0 :
                       (thing->player || allowmonstertelefrags == 1));
...
static bool untouched(line_t* ld)
{
    fixed_t x, y;
    fixed_t bbox[4]        = { 0 };
...
static bool PIT_CheckLine(line_t* ld)
{
    ...
    if(!(tmthing->flags & (MF_MISSILE | MF_BOUNCES)))
    {
        ...
        if(!((tmthing->flags & MF_FRIEND) || tmthing->player ||
           (tmthing->flags2 & MF2_SPAWNEDBYPLAYER)) &&
        ((ld->flags & ML_BLOCKMONSTERS) ||
        ((ld->flags & ML_BLOCKLANDMONSTERS) && !(tmthing->flags & MF_FLOAT))) &&
...
static bool P_ProjectileImmune(const mobj_t* target, const mobj_t* source)
{
    ...
    && ((projectilegroup == PG_DEFAULT && source->type == target->type)
       ...
       || (projectilegroup != PG_DEFAULT &&
          projectilegroup == mobjinfo[source->type].projectilegroup)));
}
...
static bool PIT_CheckThing(mobj_t* thing)
{
    ...
    if(((corpse && type != MT_BARREL) || (flags & MF_DROPPED)) &&
    !thing->nudge && tmthing->z == tmthing->floorz &&
    ((tmflags & MF_SHOOTABLE) ||
    ((tmflags & MF_CORPSE) && (tmthing->momx || tmthing->momy))) &&
    r_corpses_nudge)
        if(P_ApproxDistance(thing->x - tmthing->x, thing->y - tmthing->y) < 16 * FRACUNIT)
        {
            ...
            thing->momy +=
            (!r ? M_RandomIntNoRepeat(-1, 1, 0) : M_RandomInt(-1, 1)) * FRACUNIT;
            thing->nudge = TICRATE;
    ...
    blockdist =
    ((flags & MF_SPECIAL) && thing->info->pickupradius ? thing->info->pickupradius :
                                                         thing->info->radius) +
    tmthing->radius;
    ...
    if(!thing->player && !corpse && (flags & MF_SHOOTABLE) &&
    (tmflags & MF_SHOOTABLE) && type != MT_BARREL && tmtype != MT_BARREL)
    {
    ...
    if((tmflags & MF_SKULLFLY) && ((flags & MF_SOLID) || infiniteheight || compat_nopassover))
    {
        ...
        ((M_Random() & 7) + 1) * tmthing->info->damage, true, false);
    ...
    if((tmflags & MF_MISSILE) || ((tmflags & MF_BOUNCES) && !(tmflags & MF_SOLID)))
    {
        ...
        if(tmthing->mbf21flags & MF_MBF21_RIP)
        {
            ...
            if(r_blood != r_blood_none)
            {
                if(thing->player)
                {
                    ...
                    !(viewplayer->cheats & CF_GODMODE))
                        P_SpawnBlood(tmthing->x, tmthing->y, tmthing->z,
                        shootangle, damage, tmthing);
                }
                else if(!(thing->flags & MF_NOBLOOD))
                    P_SpawnBlood(tmthing->x, tmthing->y, tmthing->z, shootangle,
                    damage, tmthing);
            }
            ...
            if(tmtype == MT_RIPPER)
            {
                ...
                SafeAdd(stat_shotssuccessful_calamityblade, 1);
            }
        ...
        if(type != MT_BARREL && type != MT_LAMP)
        {
            if(tmtype == MT_PLASMA)
            {
                ...
                SafeAdd(stat_shotssuccessful[wp_plasma], 1);
            }
            else if(tmtype == MT_ROCKET)
            {
                if(tmthing->nudge == 1)
                {
                    ...
                    SafeAdd(stat_shotssuccessful[wp_missile], 1);
                }
            ...
            else if(tmtype == MT_FLAME)
            {
                if(tmthing->nudge == 1)
                {
                    ...
                    stat_shotssuccessful_incinerator =
                    SafeAdd(stat_shotssuccessful_incinerator, 1);
                }
    ...
    if(flags & MF_SPECIAL)
    {
        ...
        joy_rumble_pickup)
        {
            I_ControllerRumble(PICKUP_RUMBLE_STRENGTH, PICKUP_RUMBLE_STRENGTH);
    ...
    return (!((flags & MF_SOLID) && !(flags & MF_NOCLIP) && !freeze && (tmflags & MF_SOLID)) ||
    unblocking);
}
    ...
    for(int bx = xl; bx <= xh; bx++)
        for(int by = yl; by <= yh; by++)
            if(!P_BlockThingsIterator(
               bx, by, &PIT_CheckThing, !(tmthing->mbf21flags & MF_MBF21_RIP)))
                return false;
...
bool P_IsInLiquid(mobj_t* thing)
{
    ...
    if(thing->player || (thing->flags & MF_SHOOTABLE))
        for(const struct msecnode_s* seclist = thing->touching_sectorlist;
...
bool P_TryMove(mobj_t* thing, const fixed_t x, const fixed_t y, const int dropoff)
{
    ...
    if(!(flags & MF_NOCLIP) && !freeze)
    {
        ...
        if((flags & MF_BOUNCES) && !(flags & (MF_MISSILE | MF_NOGRAVITY)) &&
    ...
    {
        const double dist =
        (double)P_ApproxDistance(x - oldx, y - oldy) / FRACUNIT / UNITSPERFOOT;
...
static bool PIT_ApplyTorque(line_t* ld)
{
    ...
    if(ld->backsector && (ld->dx || ld->dy) // Torque is undefined if the line has no length
    && tmbbox[BOXRIGHT] > ld->bbox[BOXLEFT] &&
    tmbbox[BOXLEFT] < ld->bbox[BOXRIGHT] && tmbbox[BOXTOP] > ld->bbox[BOXBOTTOM] &&
    tmbbox[BOXBOTTOM] < ld->bbox[BOXTOP] && P_BoxOnLineSide(tmbbox, ld) == -1)
    {
        mobj_t* mo = tmthing;
        ...
        // dropoff direction
        if(dist < 0 ? (ld->frontsector->floorheight < mo->z &&
                      ...
                      ld->frontsector->floorheight >= mo->z))
        {
            ...
            dist = FixedDiv(FixedMul(dist,
                            (mo->gear < OVERDRIVE ? y << (OVERDRIVE - mo->gear) :
                                                    y >> (mo->gear - OVERDRIVE))),
            x);
...
void P_SlideMove(mobj_t* mo)
{
    ...
    do
    {
        ...
        if((bestslidefrac -= 0x0800) > 0)
            // killough 03/15/98: Allow objects to drop off ledges
            if(!P_TryMove(mo, mo->x + FixedMul(mo->momx, bestslidefrac),
               mo->y + FixedMul(mo->momy, bestslidefrac), 1))
                goto stairstep;
...
static bool PTR_ShootTraverse(intercept_t* in)
{
    ...
    if(li)
    {
        ...
        // spawn bullet puff
        P_SpawnPuff(dltrace.x + FixedMul(dltrace.dx, frac),
        dltrace.y + FixedMul(dltrace.dy, frac), z, shootangle);
    ...
    else if(r_blood != r_blood_none)
    {
        if(th->type == MT_SKULL && !(th->flags & MF_FUZZ))
        {
            if(r_blood == r_blood_red)
                P_SpawnBlood(x, y, z + M_RandomInt(-8, 8) * FRACUNIT,
                shootangle, la_damage, th);
            else
                P_SpawnPuff(x, y, z - M_RandomInt(0, 16) * FRACUNIT, shootangle);
        }
        else if(th->bloodcolor)
        {
            if(th->type != MT_PLAYER)
                ...
                P_SpawnBlood(x, y, z + M_RandomInt(4, 16) * FRACUNIT,
                shootangle, la_damage * 2, th);
        }
    }
...
fixed_t P_AimLineAttack(mobj_t* t1, angle_t angle, const fixed_t distance, const int mask)
{
    ...
    topslope    = (V_VANILLAHEIGHT / 2) * FRACUNIT / (V_VANILLAWIDTH / 2);
...
static bool PTR_NoWayTraverse(intercept_t* in)
{
    ...
    return (!ld || ld->special || ((flags & ML_TWOSIDED) && (flags & ML_BLOCKING)) ||
    !((flags & ML_BLOCKING) ||
    (P_LineOpening(ld),
...
bool P_DoorClosed(const line_t* line)
{
    ...
    return (openrange <= 0 || openbottom > usething->z + 24 * FRACUNIT ||
    opentop < usething->z + usething->height);
}
...
bool PIT_RadiusAttack(mobj_t* thing)
{
    ...
    (type == MT_CYBORG && bombsource->type == MT_CYBORG) :
    ...
    {
        dist = MAX(0,
        MAX(dist, ABS(thing->z + (thing->height >> 1) - bombspot->z)) >> FRACBITS);
    ...
    if(P_CheckSight(thing, bombspot))
    {
        ...
        const int damage = (bombdamage == bombdistance ?
        bombdamage - dist :
        ...
        if(bombspot->type == MT_ROCKET && type != MT_BARREL &&
        type != MT_LAMP && !(thing->flags & MF_CORPSE))
        {
            if(bombspot->nudge == 1)
            {
                viewplayer->shotssuccessful[wp_missile]++;
        ...
        else if(bombspot->type == MT_FLAME && type != MT_BARREL &&
        type != MT_LAMP && !(thing->flags & MF_CORPSE))
        {
            if(bombspot->nudge == 1)
            {
...
static void PIT_ChangeSector(mobj_t* thing)
{
    ...
    if((flags & MF_CORPSE) || (flags2 & MF2_CRUSHABLE))
    {
        ...
        {
            const int radius =
            ((spritewidth[sprites[thing->sprite].spriteframes[0].lump[0]] >> FRACBITS) >> 1) + 12;
            ...
            for(int i = 0; i < max; i++)
            {
                ...
                P_SpawnBloodSplat(
                x + FixedMul(M_RandomInt(0, radius) << FRACBITS, finecosine[angle]),
                y + FixedMul(M_RandomInt(0, radius) << FRACBITS, finesine[angle]),
                color, true, false, floorz, NULL);
            }
    ...
    if(crushchange && !(game.stats.maptime & 3))
    {
        ...
        (!viewplayer->powers[pw_invulnerability] && !(viewplayer->cheats & CF_GODMODE))))
        {
            ...
            if(!fuzz)
                ...
                REDBLOOD :
                (r_blood == r_blood_green ? GREENBLOOD : thing->bloodcolor));
        ...
        if(thing->health <= 0 && !thing->player && type != MT_BARREL && obituaries)
        {
            ...
            if(*thing->name)
                ...
                ((flags & MF_FRIEND) && type > MT_NULL && type < NUMMOBJTYPES &&
                game.stats.monstercount[type] == 1 ?
                "the" :

...
static void P_XYMovement(mobj_t* mo)
{
    ...
    do
    {
        ...
        if(!P_TryMove(mo, ptryx, ptryy, 1))
        {
            ...
            (!player && blockline && mo->z <= mo->floorz && P_GetFriction(mo, NULL) > ORIG_FRICTION)))
            {
                if(blockline)
                {
                    const fixed_t r = ((blockline->dx >> FRACBITS) * mo->momx +
    ...
    if(corpse && !(flags & MF_NOBLOOD) && mo->bloodcolor > NOBLOOD &&
    r_corpses_smearblood && (mo->momx || mo->momy) && mo->bloodsplats &&
    r_bloodsplats_max && !mo->nudge && (r_blood != r_blood_all || !(flags & MF_FUZZ)))
    {
        ...
        if(max)
        {
            ...
            const int radius =
            (spritewidth
             [sprites[mo->sprite].spriteframes[mo->frame & FF_FRAMEMASK].lump[0]] >>
            FRACBITS) >>
            1;
            ...
            for(int i = 0; i < max; i++)
                ...
                mo->y + (M_BigRandomInt(-radius, radius) << FRACBITS), color,
                true, false, floorz, mo);
        }
    }
    ...
    mo->momy < STOPSPEED &&
    (!player || (player->mo == mo && !player->cmd.forwardmove && !player->cmd.sidemove) ||
...
static void P_ZMovement(mobj_t* mo)
{
    ...
    {
        player->v_viewheight -= floorz - mo->z;
        player->deltaviewheight = (VIEWHEIGHT - player->v_viewheight) >> 3;
    }
...
floater:
    ...
    if(mo->z <= floorz)
    {
        ...
        if(mo->type == MT_BLOOD && !nobloodsplats)
        {
            ...
            if(r_bloodsplats_max)
            {
                ...
                else if(mo->bloodcolor > NOBLOOD)
                {
                    ...
                    const int bloodcolor =
                    colortranslation[mo->bloodcolor - 1][REDBLOODSPLATCOLOR];

                    P_SpawnBloodSplat(x, y, bloodcolor, false, false, 0, NULL);
                    P_SpawnBloodSplat(x + x1, y + y1, bloodcolor, false, false, 0, NULL);
                    P_SpawnBloodSplat(x - x2, y - y2, bloodcolor, false, false, 0, NULL);
                }
            }
        ...
        if(mo->momz < 0)
        {
            ...
            else if(player && player->mo == mo && !(viewplayer->cheats & CF_NOCLIP) && !freeze)
            {
                ...
                {
                    // Squat down.
                    // Decrease v_viewheight for a moment after hitting the ground (hard), and utter appropriate sound.
...
static void P_NightmareRespawn(mobj_t* mobj)
{
    ...
    if(obituaries)
        ...
        ((mo->flags & MF_FRIEND) && mo->type > MT_NULL &&
        mo->type < NUMMOBJTYPES && game.stats.monstercount[mo->type] == 1 ?
        "The" :
        "A"),
...
void P_MobjThinker(mobj_t* mobj)
{
    ...
    mobj->z <= sector->floorheight && !sector->heightsec && r_liquid_bobsprites)
        mobj->z +=
        animatedliquiddiffs[((mobj->floatbob + animatedtic) & (ANIMATEDLIQUIDDIFFS - 1))];
    else if(mobj->z != mobj->floorz || mobj->momz)
    {
        if((flags2 & MF2_PASSMOBJ) && !infiniteheight && !compat_nopassover)
        {
            ...
            else if(player)
            {
                ...
                {
                    player->v_viewheight -= onmo->z + onmo->height - mobj->z;
                    player->deltaviewheight = (VIEWHEIGHT - player->v_viewheight) >> 3;
    ...
    else
    {
        ...
        if((flags & MF_COUNTKILL) && (game.skill == sk_nightmare || respawnmonsters) &&
        ++mobj->movecount >= 12 * TICRATE && !(game.stats.maptime & 31) && M_Random() <= 4)
...
void P_SetShadowColumnFunction(mobj_t* mobj)
{
    if((mobj->flags & MF_FUZZ) && r_textures)
        mobj->shadowcolfunc =
        (r_shadows_translucency ? &R_DrawFuzzyShadowColumn : &R_DrawSolidShadowColumn);
    else
        mobj->shadowcolfunc =
        (r_shadows_translucency ? &R_DrawShadowColumn : &R_DrawSolidShadowColumn);
}
...
mobj_t* P_SpawnMobj(const fixed_t x, const fixed_t y, const fixed_t z, const mobjtype_t type)
{
    ...
    {
        mobj->height = (type != MT_KEEN && info->projectilepassheight ?
        info->projectilepassheight :
        info->height);
...
void P_RemoveMobj(mobj_t* mobj)
{
    ...
    if((flags & MF_SPECIAL) && !(flags & MF_DROPPED))
    {
        ...
        itemrespawntime[iqueuehead]  = game.stats.maptime;
...
void P_RespawnSpecials(void)
{
    ...
    if(game.stats.maptime - itemrespawntime[iqueuetail] < 30 * TICRATE)
        return;
    ...
    x = mthing->x << FRACBITS;
    y = mthing->y << FRACBITS;
    z = ((mobjinfo[i].flags & MF_SPAWNCEILING) ?
    ONCEILINGZ :
    ((mobjinfo[i].flags2 & MF2_FLOATBOB) ? 14 * FRACUNIT : ONFLOORZ));
    ...
    if(obituaries)
        ...
        (isvowel(mo->info->name1[0]) ? "An" : "A"), mo->info->name1);
...
void P_SetPlayerViewHeight(void)
{
    ...
    for(const struct msecnode_s* seclist = mo->touching_sectorlist; seclist;
    ...
    viewplayer->v_viewheight = VIEWHEIGHT;
    viewplayer->viewz = viewplayer->oldviewz = mo->z + viewplayer->v_viewheight;
...
void P_SpawnMoreBlood(mobj_t* mobj)
{
    if(mobj->bloodcolor > NOBLOOD)
    {
        ...
        const int radius =
        ((spritewidth[sprites[mobj->sprite].spriteframes[0].lump[0]] >> FRACBITS) >> 1) + 12;
        ...
        for(int j = 0; j < max; j++)
        {
            ...
            P_SpawnBloodSplat(fx, fy, bloodcolor, true, true, floorz, mobj);
        }
    }
}
...
void P_LookForFriends(void)
{
    for(int i = 0; i < numsectors; i++)
        for(mobj_t* thing = sectors[i].thinglist; thing; thing = thing->snext)
            if((thing->flags & MF_SHOOTABLE) && (thing->flags & MF_FRIEND) &&
            (thing->flags2 & MF2_SPAWNEDBYPLAYER) && thing->health > 0 && numfriends < MAXFRIENDS)
            {
...
static void P_SpawnFriend(const mapthing_t* mthing)
{
    ...
    {
        mobj_t* mobj = P_SpawnMobj(mthing->x << FRACBITS, mthing->y << FRACBITS,
        ONFLOORZ, friendtype[playerstart - 2]);
        ...
        if(game.map == 1)
            C_Warning(0, "A friendly %s entered this map with %s!",
            mobjinfo[mobj->type].name1,
            (M_StringCompare(playername, playername_default) ? "you" : playername));
        else if(*mobj->name)
            C_Warning(0, "%s followed %s into this map!", mobj->name,
            (M_StringCompare(playername, playername_default) ? "you" : playername));
        else
...
mobj_t* P_SpawnMapThing(mapthing_t* mthing, const bool spawnmonsters)
{
    ...
    if(!(options &
       (game.skill == sk_baby ? 1 : (game.skill == sk_nightmare ? 4 : (1 << (game.skill - 1))))))
        return NULL;
    ...
    mobj = P_SpawnMobj(
    x, y, ((mobjinfo[i].flags & MF_SPAWNCEILING) ? ONCEILINGZ : ONFLOORZ), i);
    ...
    if((type == SuperShotgun || (type >= Shotgun && type <= BFG9000)) &&
    (flags & MF_SPECIAL) && (M_BigRandom() & 1) && r_mirroredweapons)
        mobj->flags2 |= MF2_MIRRORED;
    ...
    // [BH] spawn blood splats around corpses
    if(r_corpses_moreblood && r_bloodsplats_max &&
    !(flags & (MF_SHOOTABLE | MF_NOBLOOD | MF_SPECIAL)) &&
    !(flags2 & MF2_DECORATION))
    {
        const short lump =
        sprites[mobj->sprite].spriteframes[mobj->frame & FF_FRAMEMASK].lump[0];
...
void P_SpawnBlood(const fixed_t x, const fixed_t y, const fixed_t z, angle_t angle, const int damage, mobj_t* target)
{
    ...
    else if(target->bloodcolor != NOBLOOD)
    {
        ...
        const int maxz =
        minz + spriteheight[sprites[target->sprite].spriteframes[0].lump[0]];
        ...
        if(!fuzz)
            color = (r_blood == r_blood_red ?
            REDBLOOD :
            (r_blood == r_blood_green ? GREENBLOOD :
                                        (target->bloodcolor == FUZZYBLOOD && r_blood == r_blood_nofuzz ?
                                        REDBLOOD :
                                        target->bloodcolor)));
...
void P_SetBloodSplatColor(bloodsplat_t* splat)
{
    ...
    {
        splat->viscolor =
        (splat->color == FUZZYBLOOD ? REDBLOODSPLATCOLOR : splat->color) +
        M_BigRandomInt(-2, 1);
        splat->viscolfunc = bloodsplatcolfunc;
    }
...
mobj_t* target)
{
    if(r_bloodsplats_total < r_bloodsplats_max && !nobloodsplats)
    {
        ...
        (!checklineside || !P_CheckLineSide(target, x, y)))
        {
            ...
            {
                const int patch =
                firstbloodsplatlump + (M_BigRandom() & (numbloodsplatlumps - 1));
...
mobj_t* P_SpawnMissile(mobj_t* source, const mobj_t* dest, mobjtype_t type)
{
    ...
    !source->subsector->sector->heightsec && r_liquid_clipsprites)
        z -= FOOTCLIPSIZE;
    ...
    MAX(1, P_ApproxDistance(dest->x - source->x, dest->y - source->y) / speed);
...
mobj_t* P_SpawnPlayerMissile(mobj_t* source, mobjtype_t type)
{
    ...
    if((source->flags2 & MF2_FEETARECLIPPED) &&
    !source->subsector->sector->heightsec && r_liquid_lowerview)
        z -= FOOTCLIPSIZE;
    ...
    viewplayer->readyweapon == wp_missile &&
    !norockettrails && !incompatiblepalette)
    {
...
static bool P_FaceMobj(const mobj_t* source, const mobj_t* target, angle_t* delta)
{
    ...
    const angle_t angle2 =
    R_PointToAngle2(source->x, source->y, target->x, target->y);
...
bool P_SeekerMissile(mobj_t* actor, mobj_t** seektarget, angle_t thresh, angle_t turnmax, const bool seekcenter)
{
    ...
    target->z + target->height < actor->z || seekcenter)
        ...
        MAX(1,
        P_ApproxDistance(target->x - actor->x, target->y - actor->y) /
        actor->info->speed);

...
void P_InitSwitchList(void)
{
    int index = 0;
    const int ep =
    (game.mode == registered || game.mode == retail ? 2 : (game.mode == commercial ? 3 : 1));
    ...
    for(int i = 0;; i++)
    {
        ...
        if(index + 1 >= max_numswitches)
            ...
            (size_t)(max_numswitches = (max_numswitches ? max_numswitches * 2 : 8)) *
            sizeof(*switchlist));
        ...
        if(SHORT(alphSwitchList[i].episode) <= ep) // jff 05/11/98 endianness
        {
            ...
            if((texture1 = R_CheckTextureNumForName(alphSwitchList[i].name1)) == -1)
                C_Warning(1,
                "Switch %i in the " BOLD(
                "SWITCHES") " lump has an unknown " BOLD("%s") " texture.",
...
void P_StartButton(line_t* line, bwhere_e bwhere, int texture, int time)
{
    ...
    memset(buttonlist + maxbuttons / 2, 0,
    ((size_t)maxbuttons - maxbuttons / 2) * sizeof(*buttonlist));

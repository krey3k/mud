...
void P_UpdateThinker(thinker_t* thinker)
{
    ...
    th               = &thinkers[(thinker->function == &P_RemoveThinkerDelayed ?
                  th_delete :
...
void P_Ticker(void)
{
    ...
    if(menuactive && !freeze)
    {
        if(!(game.time & 2))
        {
            ...
            for(thinker_t* thinker                 = thinkers[th_misc].cnext;
            thinker != &thinkers[th_misc]; thinker = thinker->cnext)
    ...
    for(currentthinker                                  = thinkers[th_all].next;
    currentthinker != &thinkers[th_all]; currentthinker = currentthinker->next)

...
const bool islightspecial[] = { false, true, true, true, false, false, false,
...
const fixed_t animatedliquiddiffs[ANIMATEDLIQUIDDIFFS] = { 6422, 6422, 6360, 6238,
    6054, 5814, 5516, 5164, 4764, 4318, 3830, 3306, 2748, 2166, 1562, 942, 314,
    -314, -942, -1562, -2166, -2748, -3306, -3830, -4318, -4764, -5164, -5516,
    -5814, -6054, -6238, -6360, -6422, -6422, -6360, -6238, -6054, -5814, -5516,
    -5164, -4764, -4318, -3830, -3306, -2748, -2166, -1562, -942, -314, 314, 942,
    1562, 2166, 2748, 3306, 3830, 4318, 4764, 5164, 5516, 5814, 6054, 6238, 6360 };
...
} texturepacks[] = { { "GRAYSLM1", "GRAYSLM4", GRAYSLIME },
    { "MLAVA1", "MLAVA4", LAVA }, { "OBLODA01", "OBLODA08", BLOOD },
    { "OGOOPY01", "OGOOPY08", GOOP }, { "OICYWA01", "OICYWA08", ICYWATER },
    { "OLAVAC01", "OLAVAC08", LAVA }, { "OLAVAD01", "OLAVAD08", LAVA },
    { "ONUKEA01", "ONUKEA08", NUKAGE }, { "OSLUDG01", "OSLUDG08", SLUDGE },
    { "OTAR__01", "OTAR__08", TAR }, { "OWATER01", "OWATER08", SLUDGE },
    { "PLOOD1", "PLOOD3", LIQUID }, { "PURPW1", "PURPW4", WATER },
    { "TEALW1", "TEALW4", WATER }, { "ZO1_01", "ZO1_99", LAVA }, { "", "", 0 } };
...
void P_InitPicAnims(void)
{
    ...
    terraintypes =
    Z_Calloc(1, ((size_t)numflats + 1) * sizeof(*terraintypes), PU_STATIC, NULL);
    isteleport =
    Z_Calloc(1, ((size_t)numflats + 1) * sizeof(*isteleport), PU_STATIC, NULL);
    ...
    while(SC_GetString())
    {
        ...
        if(noliquid || SC_Compare("LIQUID"))
        {
            ...
            if(first >= 0 && last >= 0 &&
            M_StringEndsWith(lumpinfo[firstflat + first]->wadfile->path, sc_String))
...
fixed_t P_FindShortestTextureAround(const int secnum)
{
    ...
    for(int i = 0; i < linecount; i++)
        if(P_TwoSided(secnum, i))
        {
            ...
            if((texture = P_GetSide(secnum, i, 0)->bottomtexture) > 0 &&
            textureheight[texture] < minsize)
                minsize = textureheight[texture];

            if((texture = P_GetSide(secnum, i, 1)->bottomtexture) > 0 &&
            textureheight[texture] < minsize)
                minsize = textureheight[texture];
        }
...
fixed_t P_FindShortestUpperAround(const int secnum)
{
    ...
    for(int i = 0; i < linecount; i++)
        if(P_TwoSided(secnum, i))
        {
            ...
            if((texture = P_GetSide(secnum, i, 0)->toptexture) > 0 &&
            textureheight[texture] < minsize)
                minsize = textureheight[texture];

            if((texture = P_GetSide(secnum, i, 1)->toptexture) > 0 &&
            textureheight[texture] < minsize)
                minsize = textureheight[texture];
        }
...
sector_t* P_FindModelFloorSector(const fixed_t floordestheight, const int secnum)
{
    ...
    for(int i = 0; i < linecount; i++)
        ...
        {
            sector_t* sec =
            P_GetSector(secnum, i, (P_GetSide(secnum, i, 0)->sector->id == secnum));
...
bool P_CanUnlockGenDoor(const line_t* line)
{
    ...
    case AnyKey:
        ...
        viewplayer->cards[it_yellowcard] <= 0 && viewplayer->cards[it_yellowskull] <= 0)
        {
            ...
            if(M_StringCompare(playername, playername_default))
                ...
                M_snprintf(buffer, sizeof(buffer), s_PD_ANY, playername, "s",
                s_PD_KEYCARDORSKULLKEY);

            if(autousing && numconsolestrings > 0 &&
            M_StringCompare(buffer, console[numconsolestrings - 1].string))
                return false;
    ...
    case RCard:
        ...
        (!skulliscard || viewplayer->cards[it_redskull] <= 0))
        {
            ...
            if(M_StringCompare(playername, playername_default))
                ...
                (skulliscard ? s_PD_REDK : s_PD_REDC), "You", "",
                (viewplayer->cards[it_redskull] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_KEYCARD));
            else
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_REDK : s_PD_REDC), playername, "s",
                (viewplayer->cards[it_redskull] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_KEYCARD));
    ...
    case BCard:
        ...
        (!skulliscard || viewplayer->cards[it_blueskull] <= 0))
        {
            ...
            if(M_StringCompare(playername, playername_default))
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_BLUEK : s_PD_BLUEC), "You", "",
                (viewplayer->cards[it_blueskull] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_KEYCARD));
            else
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_BLUEK : s_PD_BLUEC), playername, "s",
                (viewplayer->cards[it_blueskull] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_KEYCARD));
    ...
    case YCard:
        ...
        (!skulliscard || viewplayer->cards[it_yellowskull] <= 0))
        {
            ...
            if(M_StringCompare(playername, playername_default))
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_YELLOWK : s_PD_YELLOWC), "You", "",
                (viewplayer->cards[it_yellowskull] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_KEYCARD));
            else
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_YELLOWK : s_PD_YELLOWC), playername, "s",
                (viewplayer->cards[it_yellowskull] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_KEYCARD));
    ...
    case RSkull:
        ...
        (!skulliscard || viewplayer->cards[it_redcard] <= 0))
        {
            ...
            if(M_StringCompare(playername, playername_default))
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_REDK : s_PD_REDS), "You", "",
                (viewplayer->cards[it_redcard] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_SKULLKEY));
            else
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_REDK : s_PD_REDS), playername, "s",
                (viewplayer->cards[it_redcard] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_SKULLKEY));
    ...
    case BSkull:
        ...
        (!skulliscard || viewplayer->cards[it_bluecard] <= 0))
        {
            ...
            if(M_StringCompare(playername, playername_default))
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_BLUEK : s_PD_BLUES), "You", "",
                (viewplayer->cards[it_bluecard] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_SKULLKEY));
            else
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_BLUEK : s_PD_BLUES), playername, "s",
                (viewplayer->cards[it_bluecard] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_SKULLKEY));
    ...
    case YSkull:
        ...
        (!skulliscard || viewplayer->cards[it_yellowcard] <= 0))
        {
            ...
            if(M_StringCompare(playername, playername_default))
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_YELLOWK : s_PD_YELLOWS), "You", "",
                (viewplayer->cards[it_yellowcard] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_SKULLKEY));
            else
                M_snprintf(buffer, sizeof(buffer),
                (skulliscard ? s_PD_YELLOWK : s_PD_YELLOWS), playername, "s",
                (viewplayer->cards[it_yellowcard] == CARDNOTFOUNDYET && skulliscard ?
                s_PD_KEYCARDORSKULLKEY :
                s_PD_SKULLKEY));
    ...
    case AllKeys:
        if(!skulliscard &&
        (viewplayer->cards[it_redcard] <= 0 || viewplayer->cards[it_redskull] <= 0 ||
        viewplayer->cards[it_bluecard] <= 0 || viewplayer->cards[it_blueskull] <= 0 ||
        viewplayer->cards[it_yellowcard] <= 0 || viewplayer->cards[it_yellowskull] <= 0))
        {
            if(!viewplayer->neededcardflash || viewplayer->neededcard != it_allkeys)
...
void P_CrossSpecialLine(line_t* line, const int side, mobj_t* thing, const bool bossaction)
{
    ...
    // jff 02/04/98 add check here for generalized linedef types
    {
        ...
        {
            if(!thing->player && !bossaction &&
            ((line->special & FloorChange) || !(line->special & FloorModel)))
        ...
        {
            if(!thing->player && !bossaction &&
            ((line->special & CeilingChange) || !(line->special & CeilingModel)))
    ...
    case W1_SetTheTargetSectorsColormap:
        ...
        for(int s = -1; (s = P_FindSectorFromLineTag(line, s)) >= 0;)
            sectors[s].colormap = (side ? sides[*line->sidenum].backcolormap :
                                          sides[*line->sidenum].frontcolormap);
...
void P_PlayerInSpecialSector(sector_t* sector)
{
    ...
    if(sector->special < 32) // regular sector specials
    {
        ...
        case DamageNegative5Or10PercentHealth:
            if(!(game.stats.maptime & 31) && !(viewplayer->cheats & CF_GODMODE) &&
...
void P_UpdateSpecials(void)
{
    ...
    for(anim_t* anim = anims; anim < lastanim; anim++)
        ...
        {
            const int pic =
            anim->basepic + (animatedtic / anim->speed + i) % anim->numpics;
    ...
    for(int i = 0; i < maxbuttons; i++)
        if(buttonlist[i].btimer && !--buttonlist[i].btimer)
        {
            ...
            if(!sector || (!sector->floordata && !sector->ceilingdata) ||
            line->tag != sector->tag)
...
void P_SpawnSpecials(void)
{
    ...
    const char *p    = M_GetParm("timer");

    if(*p != '\0')
    {
        const int minutes = strtol(p, NULL, 10);
        ...
        if(minutes > 0)
        {
            ...
            C_Output(
            "A " BOLD(
            "-timer") " parameter was found on the command-line. A timer "
                      "has been set for %s minute%s. %s will automatically "
                      "exit each map once the timer runs out.",
            temp, (minutes == 1 ? "" : "s"),
            (M_StringCompare(playername, playername_default) ? "You" : playername));
            P_SetTimer(minutes);
            free(temp);
    ...
    if(M_CheckParm("avg"))
    {
        P_SetTimer(20);
        C_Output(
        "An " BOLD("avg") " parameter was found on the command-line. A timer "
                           "has been set for %i minutes. %s will automatically "
                           "exit each map once the timer runs out.",
...
void T_Scroll(scroll_t* scroller)
{
    ...
    if(scroller->type == sc_side)
    {
        ...
        if(prevaffectee == scroller->affectee && prevtime == game.time)
            return;
    ...
    if(scroller->control != -1)
    {
        ...
        const fixed_t height = sectors[scroller->control].floorheight +
        sectors[scroller->control].ceilingheight;
    ...
    case sc_carry:
    {
        ...
        fixed_t waterheight = (sec->heightsec && sec->heightsec->floorheight > height ?
        sec->heightsec->floorheight :
        FIXED_MIN);
...
static bool PIT_PushThing(mobj_t* thing)
{
    if((sentient(thing) || (thing->flags & MF_SHOOTABLE)) && !(thing->flags & MF_NOCLIP))
    {
        ...
        fixed_t speed =
        (tmpusher->magnitude -
        ((P_ApproxDistance(thing->x - sx, thing->y - sy) >> FRACBITS) >> 1))

...
const int direction)
{
    ...
    {
    case 0:
        if(sector->oldfloorgametime != game.time)
        {
            sector->oldfloorheight   = sector->floorheight;
            sector->oldfloorgametime = game.time;
        }
        ...
        case 1:
            dest =
            (compat_floormove || dest < sector->ceilingheight ? dest : sector->ceilingheight);
    ...
    case 1:
        if(sector->oldceilinggametime != game.time)
        {
            sector->oldceilingheight   = sector->ceilingheight;
            sector->oldceilinggametime = game.time;
        }
        ...
        case -1:
            dest =
            (compat_floormove || dest > sector->floorheight ? dest : sector->floorheight);
...
void T_MoveElevator(elevator_t* elevator)
{
    ...
    if(elevator->direction == -1) // moving down
    {
        ...
        res = T_MovePlane(sec, elevator->speed, elevator->ceilingdestheight,
        false, 1, elevator->direction);

        // jff 04/07/98 don't move ceiling if blocked
        if(res == ok || res == pastdest)
            T_MovePlane(sec, elevator->speed, elevator->floordestheight, false,
            0, elevator->direction);
    }
    else // up
    {
        // jff 04/07/98 reverse order of ceiling/floor
        res = T_MovePlane(sec, elevator->speed, elevator->floordestheight,
        false, 0, elevator->direction);

        // jff 04/07/98 don't move floor if blocked
        if(res == ok || res == pastdest)
            T_MovePlane(sec, elevator->speed, elevator->ceilingdestheight,
            false, 1, elevator->direction);
    }
...
bool EV_DoFloor(const line_t* line, const floor_e floortype)
{
    ...
    manual_floor:
        ...
        case RaiseFloor:
            ...
            MIN(P_FindLowestCeilingSurrounding(sec), sec->ceilingheight) -
        ...
        case RaiseToTexture:
        {
            ...
            floor->floordestheight =
            MIN((sec->floorheight >> FRACBITS) + (minsize >> FRACBITS), 32000) << FRACBITS;
            break;
        }
        ...
        case LowerAndChange:
            ...
            for(int i = 0; i < sec->linecount; i++)
                if(P_TwoSided(secnum, i))
                {
                    ...
                    {
                        if((sec = P_GetSector(secnum, i, 1))->floorheight ==
                        floor->floordestheight)
                        {
                            floor->texture    = sec->floorpic;
                            floor->newspecial = sec->special;
                    ...
                    {
                        if((sec = P_GetSector(secnum, i, 0))->floorheight ==
                        floor->floordestheight)
                        {
                            floor->texture    = sec->floorpic;
                            floor->newspecial = sec->special;
    ...
    manual_stair:
        ...
        do
        {
            ...
            for(int i = 0; i < sec->linecount; i++)
            {
                ...
                if(!(li->flags & ML_TWOSIDED) || secnum != li->frontsector->id ||
                !(tsec = li->backsector) || tsec->floorpic != texture)
                    continue;

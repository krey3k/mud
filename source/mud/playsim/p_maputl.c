...
fixed_t P_InterceptVector(const divline_t* v2, const divline_t* v1)
{
    ...
    return (
    fixed_t)((((int64_t)v1->x - v2->x) * v1->dy - ((int64_t)v1->y - v2->y) * v1->dx) / den);
}
...
{
    // link into subsector
    subsector_t* subsector = thing->subsector =
...
bool P_BlockThingsIterator(const int x, const int y, bool func(mobj_t*), bool blockmapfix)
{
    ...
    if(blockmapfix)
    {
        // (-1, -1)
        if(x > 0 && y > 0)
            for(mobj_t* mobj = blocklinks[(y - 1) * bmapwidth + x - 1]; mobj;
            mobj             = mobj->bnext)
                if(x == (mobj->x + mobj->radius - bmaporgx) >> MAPBLOCKSHIFT &&
                y == (mobj->y + mobj->radius - bmaporgy) >> MAPBLOCKSHIFT && !func(mobj))
                    return false;

        // (0, -1)
        if(y > 0)
            for(mobj_t* mobj = blocklinks[(y - 1) * bmapwidth + x]; mobj;
            mobj             = mobj->bnext)
        ...
        // (1, 0)
        if(x < bmapwidth - 1)
            for(mobj_t* mobj = blocklinks[y * bmapwidth + x + 1]; mobj;
            mobj             = mobj->bnext)
...
static bool PIT_AddThingIntercepts(mobj_t* thing)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        if(!P_PointOnDivlineSide(dltrace.x, dltrace.y, &dl))
        {
            ...
            if(P_PointOnDivlineSide(dl.x, dl.y, &dltrace) !=
            P_PointOnDivlineSide(dl.x + dl.dx, dl.y + dl.dy, &dltrace))
            {
                // It's a hit

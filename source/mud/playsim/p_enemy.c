...
#define DISTFRIEND \
    (128 * FRACUNIT) // distance friends tend to move towards players
...
static void P_RecursiveSound(sector_t* sec, const int soundblocks, mobj_t* soundtarget)
{
    ...
    for(int i = 0; i < sec->linecount; i++)
    {
        ...
        if(!(flags & ML_SOUNDBLOCK))
            P_RecursiveSound(
            sides[line->sidenum[(sides[line->sidenum[0]].sector == sec)]].sector,
...
static bool P_HitFriend(mobj_t* actor)
{
    ...
    return (linetarget && linetarget != target &&
...
static bool P_CheckMissileRange(mobj_t* actor)
{
    ...
    if(actor->flags & MF_JUSTHIT)
    {
        ...
        (target->player ? M_Random() > 128 :
                          (!(target->flags & MF_JUSTHIT) && M_Random() > 128)))));
    }
...
static int P_IsUnderDamage(const mobj_t* actor)
{
    ...
    for(const struct msecnode_s* seclist = actor->touching_sectorlist; seclist;
...
static bool P_SmartMove(mobj_t* actor)
{
    ...
    (target->player ||
    P_ApproxDistance(actor->x - target->x, actor->y - target->y) < 144 * FRACUNIT) &&
    M_Random() < 235)
    ...
    && (underdamage = P_IsUnderDamage(actor)) && (underdamage < 0 || M_Random() < 200)))
...
{
    const dirtype_t opposite[] = { DI_WEST, DI_SOUTHWEST, DI_SOUTH,
        DI_SOUTHEAST, DI_EAST, DI_NORTHEAST, DI_NORTH, DI_NORTHWEST, DI_NODIR };

    const dirtype_t diags[] = { DI_NORTHWEST, DI_NORTHEAST, DI_SOUTHWEST, DI_SOUTHEAST };
...
{
    const int xh =
    P_GetSafeBlockX((tmbbox[BOXRIGHT] = actor->x + actor->radius) - bmaporgx);
    ...
    const int yl =
    P_GetSafeBlockY((tmbbox[BOXBOTTOM] = actor->y - actor->radius) - bmaporgy);
...
static void P_NewChaseDir(mobj_t* actor)
{
    ...
    !(actor->flags & (MF_DROPOFF | MF_FLOAT)) && P_AvoidDropoff(actor))
    {
    ...
    if((actor->flags & target->flags & MF_FRIEND) && !actor->subsector->sector->islift &&
    P_ApproxDistance(deltax, deltay) < DISTFRIEND && !P_IsUnderDamage(actor))
    {
        deltax = -deltax;
        deltay = -deltay;
    }
...
static bool P_LookForPlayer(mobj_t* actor, const bool allaround)
{
    ...
    {
        const angle_t an =
        R_PointToAngle2(actor->x, actor->y, mo->x, mo->y) - actor->angle;
...
static void P_ShakeOnExplode(const mobj_t* actor)
{
    if(actor->type == MT_BARREL && r_shake_barrels)
    {
        ...
        if(mo->z <= mo->floorz &&
        P_ApproxDistance(actor->x - mo->x, actor->y - mo->y) < BARRELRANGE)
        {
...
static void P_SpawnBloodOnMelee(mobj_t* target, const int damage)
{
    ...
    if(target->player)
    {
        if(!viewplayer->powers[pw_invulnerability] && !(viewplayer->cheats & CF_GODMODE))
        {
            ...
            P_SpawnBlood(viewx + 20 * finecosine[an], viewy + 20 * finesine[an],
            viewz, 0, damage, target);
        }
    }
    else if(!(target->flags & MF_NOBLOOD))
        P_SpawnBlood(target->x, target->y,
        target->z + M_RandomInt(4, 16) * FRACUNIT, 0, damage, target);
}
...
void A_Look(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(!(friendly && P_LookForTargets(actor, false)) &&
    !((target = actor->subsector->sector->soundtarget) && (target->flags & MF_SHOOTABLE) &&
    (P_SetTarget(&actor->target, target),
    (!(flags & MF_AMBUSH) || P_CheckSight(actor, target)))) &&
    ...
    if(actor->info->seesound)
    {
        ...
        default:
            S_StartSound(
            ((actor->mbf21flags & (MF_MBF21_BOSS | MF_MBF21_FULLVOLSOUNDS)) ? NULL : actor),
            actor->info->seesound);
            break;
        }
...
void A_Chase(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(actor->flags & MF_JUSTATTACKED)
    {
        ...
        if(game.skill != sk_nightmare && !fastparm)
            P_NewChaseDir(actor);
    ...
    if(info->missilestate != S_NULL)
        if(!(game.skill < sk_nightmare && !fastparm && actor->movecount))
    ...
    if(!actor->threshold)
    {
        ...
        else
        {
            ...
            if(!(target->health > 0 &&
               (((((target->flags ^ actor->flags) & MF_FRIEND) || !(actor->flags & MF_FRIEND)) &&
...
void A_CPosRefire(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(!(target = actor->target) || target->health <= 0 ||
    (actor->flags & target->flags & MF_FRIEND) || !P_CheckSight(actor, target))
        P_SetMobjState(actor, actor->info->seestate);
}
...
static bool PIT_VileCheck(mobj_t* thing)
{
    ...
    else
    {
        ...
        for(const struct msecnode_s* seclist = corpsehit->touching_sectorlist;
        seclist; seclist                     = seclist->m_tnext)
            ...
            for(int by = yl; by <= yh; by++)
                ...
                if(!P_BlockThingsIterator(bx, by, &PIT_VileCheck, true))
                {
                    ...
                    if(type > MT_NULL && type < NUMMOBJTYPES)
                        stat_monsterskilled[type] =
                        SafeAdd(stat_monsterskilled[type], -1);
                    ...
                    if(obituaries)
                    {
                        ...
                        if(*actor->name)
                            ...
                            ((actor->flags & MF_FRIEND) && actor->type > MT_NULL &&
                            actor->type < NUMMOBJTYPES && game.stats.monstercount[actor->type] == 1 ?
                            "the" :
                        ...
                        if(*corpsehit->name)
                            M_StringCopy(corpsehitname, corpsehit->name,
                            sizeof(corpsehitname));
                        else
                            M_snprintf(corpsehitname, sizeof(corpsehitname), "%s dead%s%s",
                            ((corpsehit->flags & MF_FRIEND) && type > MT_NULL &&
                            type < NUMMOBJTYPES && game.stats.monstercount[type] == 1 ?
                            "the" :
                            "a"),
...
void A_BetaSkullAttack(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    ((M_Random() & 7) + 1) * actor->info->damage, true, false);
}
...
static void A_PainShootSkull(mobj_t* actor, angle_t angle)
{
    ...
    game.stats.kills++;
    game.stats.monstercount[MT_SKULL]++;
...
void A_Scream(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    S_StartSound(
    ((actor->mbf21flags & (MF_MBF21_BOSS | MF_MBF21_FULLVOLSOUNDS)) ? NULL : actor), sound);
}
...
{
    line_t junk        = { 0 };
    int numbossactions = P_GetNumBossActions(game.episode, game.map);
    ...
    if(numbossactions)
    {
        ...
        for(i = 0; i < numbossactions; i++)
            if(P_GetBossAction(game.episode, game.map, i)->type == actor->type)
                break;
        ...
        for(i = 0; i < numbossactions; i++)
        {
            bossaction_t* bossaction = P_GetBossAction(game.episode, game.map, i);
    ...
    if(!anybossdeath)
    {
        ...
        else
        {
            ...
            {
            case 1:
                if(game.map != 8 || !(actor->mbf21flags & MF_MBF21_E1M8BOSS))
                    return;

                break;

            case 2:
                if(game.map != 8 || !(actor->mbf21flags & MF_MBF21_E2M8BOSS))
                    return;

                break;

            case 3:
                if(game.map != 8 || !(actor->mbf21flags & MF_MBF21_E3M8BOSS))
                    return;
...
void A_BrainScream(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    {
        mobj_t* th = P_SpawnMobj(x, actor->y - 320 * FRACUNIT,
        128 + M_Random() * 2 * FRACUNIT, MT_ROCKET);
...
{
    mobj_t* th = P_SpawnMobj(actor->x + M_SubRandom() * 2048, actor->y,
    128 + M_Random() * 2 * FRACUNIT, MT_ROCKET);
...
void A_BrainSpit(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if((target = A_NextBrainTarget()))
    {
        ...
        newmobj->reactiontime = P_ApproxDistance(
        target->x - (actor->x + actor->momx), target->y - (actor->y + actor->momy));
...
void A_SpawnFly(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(!nomonsters)
    {
        ...
        if(!P_LookForPlayer(newmobj, true) ||
...
{
    S_StartSound(actor,
    (game.mode == commercial && actor->health < -50 ? sfx_pdiehi : sfx_pldeth));
}
...
void A_SpawnObject(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    dx  = FixedMul(actor->state->args[2], finecosine[fan]) -
    FixedMul(actor->state->args[3], finesine[fan]);
    dy = FixedMul(actor->state->args[2], finesine[fan]) +
    FixedMul(actor->state->args[3], finecosine[fan]);

    // spawn it, yo
    if(!(mo = P_SpawnMobj(actor->x + dx, actor->y + dy,
         actor->z + actor->state->args[4], actor->state->args[0] - 1)))
        return;
    ...
    // set velocity
    mo->momx = FixedMul(actor->state->args[5], finecosine[fan]) -
    FixedMul(actor->state->args[6], finesine[fan]);
    mo->momy = FixedMul(actor->state->args[5], finesine[fan]) +
    FixedMul(actor->state->args[6], finecosine[fan]);
...
void A_MonsterBulletAttack(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    for(int i = 0; i < numbullets; i++)
        P_LineAttack(actor, actor->angle + P_RandomHitscanAngle(actor->state->args[0]),
        MISSILERANGE, aimslope + P_RandomHitscanSlope(actor->state->args[1]),
        (M_Random() % actor->state->args[4] + 1) * actor->state->args[3]);
}
...
void A_MonsterMeleeAttack(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    P_DamageMobj(target, actor, actor,
    (M_Random() % actor->state->args[1] + 1) * actor->state->args[0], true, false);
}
...
void A_RadiusDamage(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    P_RadiusAttack(
    actor, actor->target, actor->state->args[0], actor->state->args[1], false);
    P_ShakeOnExplode(actor);
}
...
void A_HealChase(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(!P_HealCorpse(
       actor, actor->info->radius, actor->state->args[0], actor->state->args[1]))
        A_Chase(actor, NULL, NULL);
}
...
void A_SeekTracer(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    P_SeekerMissile(actor, &actor->tracer, FixedToAngle(actor->state->args[0]),
    FixedToAngle(actor->state->args[1]), true);
}
...
void A_FindTracer(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    actor->tracer = P_RoughTargetSearch(
    actor, FixedToAngle(actor->state->args[0]), actor->state->args[1]);
}
...
void A_JumpIfTargetInSight(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if((fieldofview = FixedToAngle(actor->state->args[1])) > 0 &&
    !P_CheckFOV(actor, target, fieldofview))
        return;
...
void A_JumpIfTargetCloser(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(actor->state->args[1] >
    P_ApproxDistance(actor->x - target->x, actor->y - target->y))
        P_SetMobjState(actor, actor->state->args[0]);
}
...
void A_JumpIfTracerInSight(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    // Check FOV first since it's faster
    if((fieldofview = FixedToAngle(actor->state->args[1])) > 0 &&
    !P_CheckFOV(actor, actor->tracer, fieldofview))
        return;
...
void A_JumpIfTracerCloser(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(actor->state->args[1] >
    P_ApproxDistance(actor->x - actor->tracer->x, actor->y - actor->tracer->y))
        P_SetMobjState(actor, actor->state->args[0]);
}

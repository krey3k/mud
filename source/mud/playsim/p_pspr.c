...
bool P_CheckAmmo(const weapontype_t weapon)
{
    ...
    if(viewplayer->ammo[ammotype] >= weaponinfo[weapon].ammopershot &&
    viewplayer->weaponowned[weapon])
        return true;
    ...
    viewplayer->ammo[weaponinfo[wp_supershotgun].ammotype] >=
    weaponinfo[wp_supershotgun].ammopershot &&
    ...
    viewplayer->ammo[weaponinfo[wp_chaingun].ammotype] >=
    weaponinfo[wp_chaingun].ammopershot)
...
static void P_SubtractAmmo(void)
{
    if(!infiniteammo)
    {
        ...
        {
            const int value =
            MAX(0, viewplayer->ammo[ammotype] - weaponinfo[readyweapon].ammopershot);
...
void A_WeaponReady(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if(pendingweapon != wp_nochange || player->health <= 0)
    {
        if(joy_rumble_weapons)
        {
            if(pendingweapon == wp_chainsaw)
            {
                ...
                IDLE_CHAINSAW_RUMBLE_STRENGTH * joy_rumble_weapons / 100;
    ...
    else
    {
        ...
        if(joy_rumble_weapons && readyweapon == wp_chainsaw)
        {
            idlechainsawrumblestrength =
            IDLE_CHAINSAW_RUMBLE_STRENGTH * joy_rumble_weapons / 100;
            I_ControllerRumble(idlechainsawrumblestrength, idlechainsawrumblestrength);
...
void A_ReFire(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    if((player->cmd.buttons & BT_ATTACK) &&
    player->pendingweapon == wp_nochange && player->health > 0)
    {
        player->refire++;
        P_FireWeapon();
    }
...
void A_FireOldBFG(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    do
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if(!linetarget)
                {
                    ...
                    actor, (an += (1 << 26)), 16 * 64 * FRACUNIT, mask);

                    if(!linetarget)
                    {
                        slope = P_AimLineAttack(
                        actor, (an -= (2 << 26)), 16 * 64 * FRACUNIT, mask);
        ...
        th = P_SpawnMobj(actor->x, actor->y,
        actor->z + 62 * FRACUNIT - player->psprites[ps_weapon].sy, type);
...
static void P_BulletSlope(mobj_t* actor)
{
    ...
    else
    {
        ...
        do
        {
            ...
            {
                bulletslope =
                P_AimLineAttack(actor, (an += (1 << 26)), 16 * 64 * FRACUNIT, mask);

                if(!linetarget)
                {
                    bulletslope =
                    P_AimLineAttack(actor, an - (2 << 26), 16 * 64 * FRACUNIT, mask);
...
void A_FireShotgun2(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    for(int i = 0; i < 20; i++)
        P_LineAttack(actor, actor->angle + (M_SubRandom() << ANGLETOFINESHIFT),
        MISSILERANGE, bulletslope + (M_SubRandom() << 5), 5 * (M_Random() % 3 + 1));
    ...
    if(successfulshot)
    {
        ...
        SafeAdd(stat_shotssuccessful[wp_supershotgun], 1);
    }
...
void A_FireCGun(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    P_SubtractAmmo();
    P_SetPlayerSprite(ps_flash,
    readyweaponinfo.flashstate + (unsigned int)((psp->state - &states[S_CHAIN1]) & 1));
    P_BulletSlope(actor);
    ...
    if(successfulshot)
    {
        ...
        SafeAdd(stat_shotssuccessful[wp_chaingun], 1);
    }
}
...
void P_MovePlayerSprites(void)
{
    ...
    if(weapon->state->action == &A_WeaponReady)
    {
        ...
        if(momx | momy)
            bob = MAX(MIN((FixedMul(momx, momx) + FixedMul(momy, momy)) >> 2, MAXBOB) *
            weaponbob / 100,
            bob);
        ...
        {
            const angle_t angle = ((128 * game.stats.maptime) & FINEMASK);

            weapon->sx = FixedMul(bob, finecosine[angle]);
            weapon->sy =
            WEAPONTOP + FixedMul(bob, finesine[angle & (FINEANGLES / 2 - 1)]);
        }
    }
...
void A_WeaponBulletAttack(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    for(int i = 0; i < numbullets; i++)
        ...
        player->mo->angle + P_RandomHitscanAngle(state->args[0]), MISSILERANGE,
...
//   args[0]: Base damage of attack (e.g. for 2d10, customize the 2); if not
//   set, defaults to 2 args[1]: Attack damage modulus (e.g. for 2d10, customize
//   the 10); if not set, defaults to 10 args[2]: Berserk damage multiplier
//   (fixed point); if not set, defaults to 1.0 (no change). args[3]: Sound to
//   play if attack hits args[4]: Range (fixed point); if not set, defaults to
//   player mobj's melee range
//
...
void A_RefireTo(mobj_t* actor, player_t* player, pspdef_t* psp)
{
    ...
    (player->cmd.buttons & BT_ATTACK) && player->pendingweapon == wp_nochange &&
    player->health > 0)
        P_SetPlayerSpritePtr(psp, state->args[0]);
}

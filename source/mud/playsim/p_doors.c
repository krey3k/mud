...
static void T_GradualLightingToDoor(vldoor_t* door)
{
    ...
    if(level > 0)
    {
        if(door->lighttag)
            ...
            door->line, FixedDiv(sec->ceilingheight - sec->floorheight, level));
        else if(!islightspecial[sec->special] && sec->ceilingpic != skyflatnum)
            EV_LightByAdjacentSectors(
            sec, FixedDiv(sec->ceilingheight - sec->floorheight, level));
    }
}
...
bool EV_DoLockedDoor(line_t* line, vldoor_e type, mobj_t* thing, fixed_t speed)
{
    ...
    case S1_Door_Blue_OpenStay_Fast:
        ...
        {
            // [BH] display player message distinguishing between keycard and
            // skull key [BH] flash needed key on HUD
    ...
    if(sec->ceilingdata)
    {
        ...
        case DR_Door_OpenWaitClose_Fast:
            if(door->direction == -1)
            {
                ...
                S_StartSectorSound(&sec->soundorg,
                (door->type == DoorBlazeRaise ? sfx_bdopn : sfx_doropn));
            }
            else
            {
                if(!player)
                ...
                if(door->thinker.function == &T_VerticalDoor)
                {
                    ...
                    // [BH] play correct door sound
                    S_StartSectorSound(&sec->soundorg,
                    (door->type == DoorBlazeRaise ? sfx_bdcls : sfx_dorcls));
                }

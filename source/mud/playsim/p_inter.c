...
void P_UpdateAmmoStat(const ammotype_t ammotype, const int num)
{
    ...
    case am_shell:
        ...
        SafeAdd(stat_itemspickedup_ammo[am_shell], num);
        break;

    case am_cell:
        viewplayer->itemspickedup_ammo_cells += num;
        stat_itemspickedup_ammo[am_cell] =
        SafeAdd(stat_itemspickedup_ammo[am_cell], num);
...
static int P_GiveAmmo(const ammotype_t ammotype, int num, const bool stat)
{
    ...
    if(num)
    {
        ...
        !viewplayer->ammo[viewplayer->pendingweapon]))
            ammohighlight = I_GetTimeMS() + HUD_AMMO_HIGHLIGHT_WAIT;
    }
    ...
    weaponinfo[readyweapon].ammotype != ammotype)
        for(int i = NUMWEAPONS - 1; i > readyweapon; i--)
            if(viewplayer->weaponowned[i] && !(weaponinfo[i].flags & WPF_NOAUTOSWITCHTO) &&
            weaponinfo[i].ammotype == ammotype && weaponinfo[i].ammopershot > oldammo &&
            (weaponinfo[i].ammopershot <= viewplayer->ammo[ammotype] || infiniteammo))
            {
                if(i == wp_supershotgun && viewplayer->weaponowned[wp_shotgun] &&
...
bool P_GiveAllWeapons(void)
{
    ...
    if(game.mode == commercial && !viewplayer->weaponowned[wp_supershotgun])
    {
...
void P_InitCards(void)
{
    ...
    for(int i = 0; i < numlines; i++)
    {
        ...
        case S1_Door_Blue_OpenStay_Fast:
            ...
            viewplayer->cards[it_blueskull] == CARDNOTINMAP)
            {
                char* temp = commify(i);

                C_Warning(2,
                "Linedef %s has special %i (\"%s\") but there are no " BOLD(
                "bluekeycard") " or " BOLD("blueskullkey") " things in map.",
                temp, line->special, linespecials[line->special]);
                free(temp);
            }
        ...
        case S1_Door_Red_OpenStay_Fast:
            ...
            viewplayer->cards[it_redskull] == CARDNOTINMAP)
            {
                char* temp = commify(i);

                C_Warning(2,
                "Linedef %s has special %i (\"%s\") but there are no " BOLD(
                "redkeycard") " or " BOLD("redskullkey") " things in map.",
                temp, line->special, linespecials[line->special]);
                free(temp);
            }
        ...
        case S1_Door_Yellow_OpenStay_Fast:
            if(viewplayer->cards[it_yellowcard] == CARDNOTINMAP &&
            viewplayer->cards[it_yellowskull] == CARDNOTINMAP)
            {
                char* temp = commify(i);

                C_Warning(2,
                "Linedef %s has special %i (\"%s\") but there are no " BOLD(
                "yellowkeycard") " or " BOLD("yellowskullkey") " things in "
                                                               "map.",
                temp, line->special, linespecials[line->special]);
                free(temp);
            }
...
bool P_GiveAllKeyCards(void)
{
    ...
    viewplayer->cards[it_yellowcard] <= 0 || viewplayer->cards[it_bluecard] <= 0);
...
bool P_GiveAllSkullKeys(void)
{
    ...
    viewplayer->cards[it_yellowskull] <= 0 || viewplayer->cards[it_blueskull] <= 0);
...
bool P_GiveAllCardsInMap(void)
{
    ...
    for(int i = NUMCARDS - 1; i >= 0; i--)
        ...
        {
            if(skulliscard &&
            ((i == it_blueskull && viewplayer->cards[it_bluecard] != CARDNOTINMAP) ||
...
bool P_TouchSpecialThing(mobj_t* special, const mobj_t* toucher, const bool message, const bool stat)
{
    ...
    // [BH] Use floorz instead of z to calculate special's distance from
    // toucher, if lowered while in liquid sector.
    if((delta =
        ((special->flags2 & MF2_FEETARECLIPPED) ? special->floorz : special->z) -
       toucher->z) > toucher->height ||
    delta < -8 * FRACUNIT)
        return false; // out of reach

    duplicate =
    (special->type == prevtouchtype && special->x == prevx && special->y == prevy);
    ...
    case SPR_ROCK:
    {
        ...
        if(message && !duplicate)
        {
            if(ammogiven == clipammo[am_misl] ||
            deh_strlookup[p_GOTROCKET].assigned == 2)
    ...
    case SPR_CELL:
    {
        ...
        if(message && !duplicate)
        {
            if(ammogiven == clipammo[am_cell] || deh_strlookup[p_GOTCELL].assigned == 2)
    ...
    case SPR_SHEL:
    {
        ...
        if(message && !duplicate)
        {
            if(ammogiven == clipammo[am_shell] ||
            deh_strlookup[p_GOTSHELLS].assigned == 2)
    ...
    if(sound != prevsound || game.time != prevtic)
    {
        prevsound = sound;
...
static void
P_WriteObituary(mobj_t* target, mobj_t* inflicter, mobj_t* source, const bool gibbed, const bool telefragged)
{
    if(telefragged)
    {
        if(target->player)
        {
            ...
            else
            {
                ...
                (friendly && source->type > MT_NULL &&
                source->type < NUMMOBJTYPES && game.stats.monstercount[source->type] == 1 ?
                "the" :
                (*source->info->name1 && isvowel(source->info->name1[0]) && !friendly ? "an" : "a")),
                (friendly ? "friendly " : ""),
                (*source->info->name1 && !M_StringStartsWith(source->info->name1, "Deh_Actor_") ?
                source->info->name1 :
                "monster"));
            }
        ...
        else if(source->player)
        {
            ...
            else
            {
                ...
                (friendly && target->type > MT_NULL &&
                target->type < NUMMOBJTYPES && game.stats.monstercount[target->type] == 1 ?
                "the" :
                (*target->info->name1 && isvowel(target->info->name1[0]) && !friendly ? "an" : "a")),
                (friendly ? "friendly " : ""),
                (*target->info->name1 && !M_StringStartsWith(target->info->name1, "Deh_Actor_") ?
                target->info->name1 :
                "monster"));
            }
        ...
        else
        {
            ...
            else
            {
                ...
                M_snprintf(targetname, sizeof(targetname), "%s %s%s",
                (friendly && target->type < NUMMOBJTYPES && game.stats.monstercount[target->type] == 1 ?
                "the" :
                (*target->info->name1 && isvowel(target->info->name1[0]) && !friendly ? "an" : "a")),
                (friendly ? "friendly " : ""),
                (*target->info->name1 && !M_StringStartsWith(target->info->name1, "Deh_Actor_") ?
        ...
        if(inflicter && inflicter->type == MT_BARREL && target->type != MT_BARREL)
        {
            ...
            if(target->player)
            {
                if(inflicter->inflicter == MT_PLAYER)
                {
                    if(M_StringCompare(playername, playername_default))
                        C_PlayerObituary(
                        "You were %s by %s %s that you exploded!",
                        ...
                        playername, (gibbed ? s_GIBBED : s_KILLED),
                        (inflictername && isvowel(inflictername[0]) ? "an" : "a"),
                        (inflictername ? inflictername : "barrel"), pronoun(personal));
                }
                else
                {
                    if(M_StringCompare(playername, playername_default))
                        C_PlayerObituary(
                        "You were %s by %s %s that %s %s exploded!",
                        (gibbed ? s_GIBBED : s_KILLED),
                        (inflictername && isvowel(inflictername[0]) ? "an" : "a"),
                        (inflictername ? inflictername : "barrel"),
                        (inflicter->type == inflicter->inflicter ||
                        M_StringCompare(
                        inflictername, mobjinfo[inflicter->inflicter].name1) ?
                        "another" :
                        (isvowel(mobjinfo[inflicter->inflicter].name1[0]) ?
                        "an" :
                        "a")),
                        (*mobjinfo[inflicter->inflicter].name1 &&
                        !M_StringStartsWith(mobjinfo[inflicter->inflicter].name1, "Deh_Actor_") ?
                        mobjinfo[inflicter->inflicter].name1 :
                        "monster"));
                    else
                        C_PlayerObituary(
                        "%s was %s by %s %s that %s %s exploded!", playername,
                        (gibbed ? s_GIBBED : s_KILLED),
                        (inflictername && isvowel(inflictername[0]) ? "an" : "a"),
                        (inflictername ? inflictername : "barrel"),
                        (inflicter->type == inflicter->inflicter ||
            ...
            else
            {
                ...
                else
                {
                    ...
                    target->type < NUMMOBJTYPES && game.stats.monstercount[target->type] == 1 ?
                    "the" :
                    (*target->info->name1 && isvowel(target->info->name1[0]) && !corpse && !friendly ?
                    "an" :
                    "a")),
                    (corpse && !M_StringStartsWith(target->info->name1, "dead ") ?
                    "dead " :
                    (friendly ? "friendly " : "")),
                    (*target->info->name1 && !M_StringStartsWith(target->info->name1, "Deh_Actor_") ?
                    target->info->name1 :
                ...
                if(inflicter->inflicter == MT_PLAYER)
                {
                    if(M_StringCompare(playername, playername_default))
                        ...
                        temp, (gibbed ? s_GIBBED : s_KILLED),
                        (inflictername && isvowel(inflictername[0]) ? "an" : "a"),
                        (inflictername ? inflictername : "barrel"), playername);
                }
            ...
            if(M_StringCompare(playername, playername_default))
            {
                if(target->player)
                {
                    if(healthcvar)
                        ...
                        (gibbed ? s_GIBBED : s_KILLED), weaponinfo[readyweapon].name);
                }
                else
                {
                    char targetname[128];
                    ...
                    else
                    {
                        ...
                        (friendly ? "friendly " : ""),
                        (*target->info->name1 &&
                        !M_StringStartsWith(target->info->name1, "Deh_Actor_") ?
                        target->info->name1 :
                        "monster"));
                    }
                    ...
                    if(readyweapon == wp_fist && viewplayer->powers[pw_strength])
                        ...
                        (target->type == MT_BARREL ? "exploded" :
                                                     (gibbed ? s_GIBBED : s_KILLED)),
                        targetname, weaponinfo[readyweapon].name, berserk);
                    else
                        C_PlayerMessage("You %s %s with your %s.",
                        (target->type == MT_BARREL ? "exploded" :
                                                     (gibbed ? s_GIBBED : s_KILLED)),
                        targetname, weaponinfo[readyweapon].name);
                }
            }
            else
            {
                if(target->player)
                {
                    if(healthcvar)
                        C_PlayerObituary(
                        "%s %s %s!", playername, s_KILLED, pronoun(reflexive));
                    else
                        C_PlayerObituary("%s %s %s with %s own %s!", playername,
                        (gibbed ? s_GIBBED : s_KILLED), pronoun(reflexive),
                        pronoun(possessive), weaponinfo[readyweapon].name);
                }
                else
                {
                    char targetname[128];
                    ...
                    if(readyweapon == wp_fist && viewplayer->powers[pw_strength])
                        C_PlayerMessage("%s %s %s with %s %s while %s.", playername,
                        (target->type == MT_BARREL ? "exploded" :
                                                     (gibbed ? s_GIBBED : s_KILLED)),
                        targetname, pronoun(possessive),
                        weaponinfo[readyweapon].name, berserk);
                    else
                        C_PlayerMessage("%s %s %s with %s %s.", playername,
                        (target->type == MT_BARREL ? "exploded" :
                                                     (gibbed ? s_GIBBED : s_KILLED)),
                        targetname, pronoun(possessive), weaponinfo[readyweapon].name);
                }
            }
        }
        else
        {
            ...
            if(target->player)
            {
                if(M_StringCompare(playername, playername_default))
                    ...
                    C_PlayerObituary("%s was %s by %s!", playername,
                    (gibbed ? s_GIBBED : s_KILLED), sourcename);
            }
            else
            {
                ...
                else
                {
                    ...
                    M_StringCompare(source->info->name1, target->info->name1) ?
                    "another" :
                    (friendly && target->type > MT_NULL &&
                    target->type < NUMMOBJTYPES && game.stats.monstercount[target->type] == 1 ?
                    "the" :
    ...
    else if(target->player && target->player->mo == target)
    {
        ...
        else
        {
            ...
            {
                const char* liquids[][2] = { { "liquid", "liquid" }, { "nukage", "nukage" },
                    { "water", "water" }, { "lava", "lava" }, { "blood", "blood" },
                    { "slime", "slime" }, { "gray slime", "grey slime" },
                    { "goop", "goop" }, { "icy water", "icy water" },
                    { "tar", "tar" }, { "sludge", "sludge" } };
...
static void P_SpawnGibBlood(mobj_t* target)
{
    if(r_blood_gibs && !(target->flags & MF_NOBLOOD))
        for(int i = 1; i <= 30; i++)
        {
            ...
            target->x, target->y, target->z + target->height * 2 / 3, MT_BLOOD);
...
void P_KillMobj(mobj_t* target, mobj_t* inflicter, mobj_t* source, const bool telefragged)
{
    ...
    if(type != MT_BARREL)
    {
        ...
        !(target->flags2 & MF2_NOMIRROREDCORPSE))
    ...
    if(info->droppeditem != MT_NULL)
    {
        ...
        if(tossdrop)
        {
            ...
            target->floorz + target->height * 3 / 2 - 3 * FRACUNIT, info->droppeditem);
...
void P_DamageMobj(mobj_t* target, mobj_t* inflicter, mobj_t* source, int damage, const bool adjust, const bool telefragged)
{
    ...
    if(!(flags & (MF_SHOOTABLE | MF_BOUNCES)) &&
    (!corpse || !r_corpses_slide || target->giblevel == 2))
        return;
    ...
    if(tplayer && game.skill == sk_baby && adjust)
        damage >>= (damage > 1);
    ...
    if(massacre)
    {
        if(r_bloodsplats_max && !(flags & MF_NOBLOOD) && type != MT_SKULL &&
        target->bloodcolor > NOBLOOD)
        {
            const short lump =
            sprites[target->sprite].spriteframes[target->frame & FF_FRAMEMASK].lump[0];

            if(lumpinfo[firstspritelump + lump]->wadfile->type == IWAD)
            {
                angle_t ang =
                R_PointToAngle2(target->x + (M_BigRandomInt(-100, 100) << FRACBITS),
                target->y + (M_BigRandomInt(-100, 100) << FRACBITS), target->x,
                target->y) >>
                ANGLETOFINESHIFT;
    ...
    {
        angle_t ang =
        R_PointToAngle2(inflicter->x, inflicter->y, target->x, target->y);
        fixed_t thrust =
        damage * (FRACUNIT >> 3) * 100 / MAX((corpse ? 200 : 1), info->mass);
    ...
    if(tplayer)
    {
        ...
        {
            const short strength =
            (30000 + (100 - MIN(tplayer->health, 100)) / 100 * 30000) *
    ...
    (!tplayer || !(viewplayer->cheats & CF_GODMODE)))
    {
        justhit = true;
    ...
    !P_InfightingImmune(target, source))
    {
        ...
        if((state == spawnstate || state == &states[spawnstate->nextstate]) &&
    ...
    // fight back!
    if(justhit &&
    (target->target == source || !target->target || !(flags & target->target->flags & MF_FRIEND)))

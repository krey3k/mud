...
static byte saveg_read8(void)
{
    ...
    if(FS_Read(&result, 1, 1, save_stream) < 1)
        return 0;
...
{
    FS_Write(&value, 1, 1, save_stream);
}
...
static void saveg_read_mobj_t(mobj_t* str)
{
    ...
    str->state =
    ((state = saveg_read32()) > 0 && state < numstates ? &states[state] : NULL);
...
static void saveg_read_player_t(void)
{
    ...
    viewplayer->v_viewheight      = saveg_read32();
    ...
    game.stats.totaltime = saveg_read32();
...
static void saveg_write_player_t(void)
{
    ...
    saveg_write32(viewplayer->v_viewheight);
    ...
    saveg_write32(game.stats.totaltime);
...
void P_WriteSaveGameHeader(const char* description)
{
    ...
    saveg_write8(game.skill);
    saveg_write8(game.episode);
    ...
    if(M_StringCompare(mapnum, "E1M4B"))
        ...
        saveg_write8(game.map);

    saveg_write8(game.mission);
    saveg_write8((game.stats.maptime >> 16) & 0xFF);
    saveg_write8((game.stats.maptime >> 8) & 0xFF);
    saveg_write8(game.stats.maptime & 0xFF);
}
...
bool P_ReadSaveGameHeader(char* description)
{
    ...
    !M_StringCompare(savegameversion, DOOMRETRO_SAVEGAMEVERSION_5_7_2))
    {
        ...
        C_ShowConsole(false);
        C_Warning(0,
        "This savegame is incompatible with " ITALICS(DOOMRETRO_NAMEANDVERSIONSTRING "."));

        return false; // bad version
    }

    if(game.skill != sk_none)
        game.prevskill = game.skill;

    game.skill   = (skill_t)saveg_read8();
    game.episode = saveg_read8();
    game.map     = saveg_read8();

    if(game.mode != commercial)
    {
        if(game.map == 10 && W_CheckNumForName("E1M10") < 0)
        {
            game.map = 4;
            M_StringCopy(speciallumpname, "E1M4B", sizeof(speciallumpname));
        }
        else if(game.map == 11 && W_CheckNumForName("E1M11") < 0)
        {
            game.map = 8;
    ...
    saveg_read8(); // game.mission

    // get the times
    a       = saveg_read8();
    b       = saveg_read8();
    c       = saveg_read8();
    game.stats.maptime = (a << 16) + (b << 8) + c;
...
void P_UnarchiveThinkers(void)
{
    ...
    game.stats.items = viewplayer->itemcount;
    game.stats.kills = viewplayer->killcount;
    ...
    while(true)
    {
        ...
        case tc_mobj:
        {
            ...
            (mobj->type == MT_MUSICSOURCE ? &MusInfoThinker : &P_MobjThinker);
        ...
        case tc_bloodsplat:
        {
            ...
            if(splat)
            {
                ...
                {
                    if(splat->patch < firstbloodsplatlump ||
                    splat->patch >= firstbloodsplatlump + numbloodsplatlumps)
                        splat->patch = firstbloodsplatlump +
                        (M_BigRandom() & (numbloodsplatlumps - 1));
...
{
    // save off the current thinkers
    for(thinker_t* th = thinkers[th_misc].cnext; th != &thinkers[th_misc];
    th                = th->cnext)
    {
        if(th->function == &T_CeilingStay)
        {
            for(ceilinglist_t* ceilinglist = activeceilings; ceilinglist;
            ceilinglist                    = ceilinglist->next)
        ...
        else if(th->function == &T_PlatStay)
        {
            ...
            for(platlist_t* platlist = activeplats; platlist;
...
{
    if((automapactive = saveg_read_bool()))
    ...
    {
        breadcrumb = I_Realloc(breadcrumb,
        (size_t)(maxbreadcrumbs = numbreadcrumbs * 2) * sizeof(*breadcrumb));

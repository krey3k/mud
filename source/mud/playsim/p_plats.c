...
void T_PlatRaise(plat_t* plat)
{
    ...
    case down:
        ...
        if(res == pastdest)
        {
            ...
            // jff 01/26/98 remove the plat if it bounced so it can be tried
            // again only affects plats that raise and bounce
            switch(plat->type)
            {
            case RaiseAndChange:
            case RaiseToNearestAndChange:
...
void P_ActivateInStasis(int tag)
{
    for(platlist_t* platlist = activeplats; platlist; platlist = platlist->next) // search the active plats
    {
        ...
        {
            plat->status =
            (plat->type == ToggleUpDn ? (plat->oldstatus == up ? down : up) :
                                        plat->oldstatus);
            plat->thinker.function = &T_PlatRaise;
        }
    }

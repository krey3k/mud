...
void P_CalcHeight(void)
{
    ...
    if(viewplayer->playerstate == PST_LIVE)
    {
        ...
        if(momx | momy)
            bob = MAX(
            MIN((FixedMul(momx, momx) + FixedMul(momy, momy)) >> 2, MAXBOB) * movebob / 200, bob);
        ...
        // move v_viewheight
        viewplayer->v_viewheight += viewplayer->deltaviewheight;

        if(viewplayer->v_viewheight > VIEWHEIGHT)
        {
            viewplayer->v_viewheight      = VIEWHEIGHT;
            viewplayer->deltaviewheight = 0;
        }

        if(viewplayer->v_viewheight < VIEWHEIGHT / 2)
        {
            viewplayer->v_viewheight = VIEWHEIGHT / 2;
        ...
        viewplayer->viewz +=
        FixedMul(bob, finesine[(FINEANGLES / 20 * game.stats.maptime) & FINEMASK]);
    }
    ...
    if(mo->flags2 & MF2_FEETARECLIPPED)
    {
        ...
        else if(r_liquid_lowerview)
        {
            ...
            if(!sector->heightsec ||
            mo->z + viewplayer->v_viewheight - FOOTCLIPSIZE >= sector->heightsec->floorheight)
                viewplayer->viewz -= FOOTCLIPSIZE;
        }
    }

    viewplayer->viewz =
    BETWEEN(mo->floorz + 4 * FRACUNIT, viewplayer->viewz, mo->ceilingz - 4 * FRACUNIT);
}
...
{
    const sector_t* sector1 =
    R_PointInSubsector(viewx + width * viewcos, viewy + width * viewsin)->sector;
    const sector_t* sector2 =
    R_PointInSubsector(viewx + width * 2 * viewcos, viewy + width * 2 * viewsin)->sector;
...
void P_MovePlayer(void)
{
    ...
    {
        if(cmd->lookdir)
            viewplayer->lookdir = BETWEEN(-LOOKDIRMAX / render.scale * MLOOKUNIT,
            viewplayer->lookdir + cmd->lookdir, LOOKDIRMAX / render.scale * MLOOKUNIT);
...
static void P_DeathThink(void)
{
    ...
    if(mo->z <= mo->floorz || (mo->flags2 & MF2_ONMOBJ))
    {
        if(canfreelook)
        {
            ...
            if(deadlookdir == -1)
            {
                const double viewheightrange =
                ((double)viewplayer->v_viewheight - DEADVIEWHEIGHT) / FRACUNIT;
        ...
        if(viewplayer->v_viewheight > DEADVIEWHEIGHT)
            viewplayer->v_viewheight -= FRACUNIT;

        if(viewplayer->v_viewheight < DEADVIEWHEIGHT)
            viewplayer->v_viewheight = DEADVIEWHEIGHT;
    }
    ...
    {
        const angle_t angle =
        R_PointToAngle2(mo->x, mo->y, attacker->x, attacker->y);
    ...
    game.keydown[KEY_ENTER] ||
    ((viewplayer->cmd.buttons & BT_ATTACK) && !viewplayer->damagecount &&
    deathcount > TICRATE * 2))
    {
        deathcount              = 0;
        damagerumbletics        = 1;
        viewplayer->playerstate = PST_REBORN;
        facingkiller            = false;
        game.keydown[KEY_SPACE]  = false;
        game.keydown[KEY_ENTER]  = false;
...
void P_ChangeWeapon(weapontype_t newweapon, bool best)
{
    ...
    else
    {
        ...
        viewplayer->ammo[ammotype] < weaponinfo[newweapon].ammopershot && !infiniteammo)
            newweapon = wp_nochange;

        // Select the preferred shotgun.
        else if(newweapon == wp_shotgun && best)
        {
            if((!viewplayer->weaponowned[wp_shotgun] || readyweapon == wp_shotgun ||
            ...
            viewplayer->ammo[am_shell] == 1))
                viewplayer->preferredshotgun = wp_shotgun;
        }
    }

    if(newweapon != wp_nochange && newweapon != readyweapon &&
    viewplayer->weaponowned[newweapon])
    {
...
void P_PlayerThink(void)
{
    ...
    if(regenhealth && mo->health < initial_health && !(game.stats.maptime % TICRATE) &&
    !viewplayer->damagecount)
    {
    ...
    (mo->z <= mo->floorz || (mo->flags2 & MF2_ONMOBJ)) && !viewplayer->jumptics)
    {
        mo->momz             = JUMPHEIGHT;

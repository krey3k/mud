...
static bool SetupList(searchlist_t* list,
searchlist_t* src_list,
char* startname,
char* endname,
char* startname2,
char* endname2)
...
{
    sprite_frames =
    Z_Malloc(sprite_frames_alloced * sizeof(*sprite_frames), PU_STATIC, NULL);
    num_sprite_frames = 0;
}
...
static sprite_frame_t* FindSpriteFrame(const char* name, char frame)
{
    ...
    {
        sprite_frame_t* newframes = Z_Malloc(
        (size_t)sprite_frames_alloced * 2 * sizeof(*sprite_frames), PU_STATIC, NULL);
...
static void AddSpriteLump(lumpinfo_t* lump)
{
    static struct
    {
        const char* spr1;
        const char* spr2;
    } weaponsprites[] = { { "PUNG", "" }, { "PISG", "PISF" }, { "SHTG", "SHTF" },
        { "CHGG", "CHGF" }, { "MISG", "MISF" }, { "PLSG", "PLSF" },
        { "BFGG", "BFGF" }, { "SAWG", "" }, { "SHT2", "SHT2" }, { "", "" } };
    ...
    if(lump->wadfile->type == PWAD)
    {
        if(!isresourcewad)
        {
            ...
            while(*weaponsprites[i].spr1)
            {
                ...
                (*weaponsprites[i].spr2 &&
                M_StringStartsWith(lump->name, weaponsprites[i].spr2)))
        ...
        case SECTION_HIDEF:
            if(!strncasecmp(lump->name, "HI_END", 8) && histart != -1)
            {
                ...
                if(patches)
                {
                    ...
                    C_Warning(1,
                    "The %s%s between the " BOLD("HI_START") " and " BOLD(
                    "HI_END") " markers will be ignored.",
...
bool W_MergeFile(const char* filename, bool autoloaded)
{

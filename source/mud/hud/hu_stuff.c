...
static void (
*fillrectfunc)(int, int, int, int, int, int, int, bool, bool, const byte*, const byte*);
static void (
*fillrectfunc2)(int, int, int, int, int, int, int, bool, bool, const byte*, const byte*);
...
static struct
{
    ...
    patch_t* patch;
} keypics[] = { { "BKEYA0", "BKEYB0", NULL }, { "YKEYA0", "YKEYB0", NULL },
    { "RKEYA0", "RKEYB0", NULL }, { "BSKUA0", "BSKUB0", NULL },
    { "YSKUA0", "YSKUB0", NULL }, { "RSKUA0", "RSKUB0", NULL } };
...
void HU_Init(void)
{
    ...
    for(int i = 0; i < NUMWEAPONS; i++)
    {
        ...
        if(!sprite || (game.mode == shareware && (i == wp_plasma || i == wp_bfg)) ||
        (game.mode != commercial && i == wp_supershotgun))
        ...
        else
        {
            ...
            for(int j = numstates - 1; j >= 0; j--)
            {
                ...
                {
                    spriteframe_t* frame =
                    &sprites[sprite].spriteframes[state->frame & FF_FRAMEMASK];

                    if(frame)
                        weaponinfo[i].ammopatch =
                        W_CacheLumpNum(firstspritelump + frame->lump[0]);
    ...
    keypics[it_yellowcard].patch = HU_LoadHUDKeyPatch(it_yellowcard);
...
void HU_Start(void)
{
    ...
    char* author = P_GetMapAuthor(game.episode, game.map);
    ...
    HUlib_InitSText(&w_message, w_message.l.x, w_message.l.y, hu_font,
...
static void DrawHUDNumber(int* x,
int y,
int val,
const byte* tinttab,
...
{
    if(r_hud_translucency)
        althudfunc((video.screen_width - crosshairwidth[crosshair - 1]) / 2,
        (video.screen_height - V_SBARHEIGHT * (r_screensize < r_screensize_max) -
        crosshairheight[crosshair - 1]) /
        2,
        ...
        crosshairheight[crosshair - 1]) /
        2,
        crosshairpatch[crosshair - 1], WHITE,
        (viewplayer->attackdown ? nearestcolors[crosshaircolor] :
...
{
    const int health = BETWEEN(HUD_NUMBER_MIN,
    ...
    (viewplayer->cheats & CF_BUDDHA) || gamepaused ?
     tinttab80 :
     tinttab25);
    ...
    const weapontype_t weapon =
    (pendingweapon == wp_nochange ? viewplayer->readyweapon : pendingweapon);
    const ammotype_t ammotype = weaponinfo[weapon].ammotype;

    if(patch)
        hudfunc(HUD_HEALTH_X - SHORT(patch->width) / 2 - 1,
        HUD_HEALTH_Y - SHORT(patch->height) - 2, patch, tinttab80);
    ...
    if((armor += armordiff))
    {
        ...
        if((patch = (viewplayer->armortype == blue_armor_class ? bluearmorpatch : greenarmorpatch)))
            hudfunc(HUD_ARMOR_X - SHORT(patch->width) / 2,
            HUD_ARMOR_Y - SHORT(patch->height) - 3, patch, tinttab80);
    ...
    if(viewplayer->neededcardflash)
    {
        ...
        if(neededcard == it_allkeys)
        {
            ...
            if(flashkeys && (showkey || gamepaused))
                for(int i = 0; i < NUMCARDS; i++)
                    ...
                    {
                        keypic_x -= SHORT(patch->width);
                        hudfunc(keypic_x, HUD_KEYS_Y - (SHORT(patch->height) - 16),
                        patch, tinttab80);
                        keypic_x -= 5;
                    }
        }
        else if((patch = keypics[neededcard].patch))
        {
            ...
            if(flashkeys && (showkey || gamepaused))
                hudfunc(keypic_x - SHORT(patch->width),
                HUD_KEYS_Y - (SHORT(patch->height) - 16), patch, tinttab80);
        }
    }
    else
    {
    ...
    if(ammotype != am_noammo)
    {
        ...
        if((patch = weaponinfo[weapon].ammopatch))
            hudfunc(HUD_AMMO_X - SHORT(patch->width) / 2 - 1,
            HUD_AMMO_Y - SHORT(patch->height) - 3, patch, tinttab80);
...
static altkeypic_t altkeypics[NUMCARDS] = { { BLUE3, NULL, NULL },
    { YELLOW1, NULL, NULL }, { RED2, NULL, NULL }, { BLUE3, NULL, NULL },
    { YELLOW1, NULL, NULL }, { RED2, NULL, NULL } };
...
static pistol_t pistol1[] = { { 57, 62, -126, -106, -106 },
    { 79, 82, -104, -86, -109 }, { 66, 81, -119, -87, -111 },
    { 61, 81, -125, -87, -87 }, { 78, 103, -106, -65, -65 } };

static pistol_t pistol2[] = { { 107, 94, -76, -106, -106 },
    { 116, 114, -67, -86, -86 }, { 111, 113, -74, -87, -87 },
    { 109, 113, -77, -87, -87 }, { 113, 135, -71, -65, -65 } };
...
static bool HU_DefaultPistolSprites(void)
{
    ...
    for(int i = 0; i < 5; i++)
    {
        ...
        {
            patch_t* patch =
            W_CacheLumpNum(firstspritelump + frame[i & FF_FRAMEMASK].lump[0]);
...
static void HU_AltInit(void)
{
    ...
    for(int i = 0; i < NUMWEAPONS; i++)
    {
        ...
        if(W_CheckNumForName(buffer) >= 0)
        {
            ...
            weaponinfo[i].weapony =
            ALTHUD_Y + 11 - SHORT(weaponinfo[i].weaponpatch->height) / 2;
        }
        else
        {
            ...
            (game.mode != commercial && i == wp_supershotgun))
                ...
                for(int j = numstates - 1; j >= 0; j--)
                {
                    ...
                    if(state->sprite == sprite)
                    {
                        ...
                        .spriteframes[state->frame & FF_FRAMEMASK]
                        .lump[0]);
                        weaponinfo[i].weapony =
                        ALTHUD_Y + 11 - SHORT(weaponinfo[i].weaponpatch->height) / 2;
                        break;
...
static void DrawAltHUDNumber(int x, int y, int val, int color, const byte* tinttab, int shadow)
{
    if(val < 0)
    {
        ...
        {
            val = -val;
            althudfunc(x - altminuspatchwidth -
            (val == 1 || val == 7 || (val >= 10 && val <= 19) ||
            (val >= 70 && val <= 79) || (val >= 100 && val <= 199) ?
            1 :
            2),
            y, altminuspatch, WHITE, color, tinttab, shadow);
        }
        else
            val = 0;
    }
    ...
    if(val >= 100)
    {
        ...
        althudfunc(x, y, (patch = altnumpatch[(val %= 100) / 10]), WHITE, color,
        tinttab, shadow);
        althudfunc(x + SHORT(patch->width) + 2, y, altnumpatch[val % 10], WHITE,
        color, tinttab, shadow);
    }
    else if(val >= 10)
    {
...
static void HU_DrawAltHUD(void)
{
    ...
    const int color =
    (inverted ? nearestblack : (r_hud_translucency ? nearestwhite : nearestlightgray));
    int health   = BETWEEN(HUD_NUMBER_MIN,
      (negativehealth && minuspatch && !viewplayer->health ? viewplayer->negativehealth :
                                                             viewplayer->health) +
      healthdiff,
      HUD_NUMBER_MAX);
    ...
    if(r_hud_translucency)
        DrawAltHUDNumber(ALTHUD_LEFT_X - AltHUDNumberWidth(ABS(health)),
        ALTHUD_Y + 12, health, color,
        (healthhighlight > currenttime ? tinttab80 : tinttab60), shadowcolor);
    else
        DrawAltHUDNumber(ALTHUD_LEFT_X - AltHUDNumberWidth(ABS(health)),
        ALTHUD_Y + 12, health,
    ...
    if(health == 100)
    {
        if(r_hud_translucency)
        {
            ...
            althudfunc(ALTHUD_LEFT_X + 5, ALTHUD_Y + 11, altleftpatch1, WHITE,
            color, tinttab60, shadowcolor);
            althudfunc(ALTHUD_LEFT_X + 25, ALTHUD_Y + 13, altendpatch, WHITE,
            color2, NULL, -1);
            althudfunc(ALTHUD_LEFT_X + 123, ALTHUD_Y + 22, altmarkpatch, WHITE,
            color2, NULL, shadowcolor);
            althudfunc(ALTHUD_LEFT_X + 125, ALTHUD_Y + 13, altendpatch, WHITE,
            color2, NULL, -1);
        }
        else
        {
            fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 13,
            MAX(1, health) + 1, 8, green2, green2, true, true, NULL, NULL);
            althudfunc(ALTHUD_LEFT_X + 5, ALTHUD_Y + 11, altleftpatch1, WHITE,
            color, NULL, shadowcolor);
            althudfunc(ALTHUD_LEFT_X + 25, ALTHUD_Y + 13, altendpatch, WHITE,
            green1, NULL, -1);
            althudfunc(ALTHUD_LEFT_X + 123, ALTHUD_Y + 22, altmarkpatch, WHITE,
            green1, NULL, shadowcolor);
            althudfunc(ALTHUD_LEFT_X + 125, ALTHUD_Y + 13, altendpatch, WHITE,
            green1, NULL, -1);
        }
    }
    ...
    {
        const int barcolor = (health < HUD_HEALTH_MIN &&
        !(viewplayer->cheats & CF_BUDDHA) && healthdiff != -initial_health ?
        red2 :
        color);
        ...
        if(r_hud_translucency)
        {
            ...
            barcolor, barcolor, true, true, tinttab60, tinttab25);
            althudfunc(ALTHUD_LEFT_X + 5, ALTHUD_Y + 11, altleftpatch1, WHITE,
            color, tinttab60, shadowcolor);
            althudfunc(ALTHUD_LEFT_X + 25, ALTHUD_Y + 13, altendpatch, WHITE,
            color2, NULL, -1);
            althudfunc(ALTHUD_LEFT_X + 25 + health - 3, ALTHUD_Y + 22,
            altmarkpatch, WHITE, color2, NULL, shadowcolor);
            althudfunc(ALTHUD_LEFT_X + 25 + health - 1, ALTHUD_Y + 13,
            altendpatch, WHITE, color2, NULL, -1);
        }
        else
        {
    ...
    else
    {
        ...
        if(r_hud_translucency)
        {
            ...
            fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 13, 101, 8, green2,
            green2, true, true, tinttab60, tinttab25);
            ...
            althudfunc(ALTHUD_LEFT_X + 125, ALTHUD_Y + 13, altendpatch, WHITE,
            color2, NULL, -1);
            althudfunc(ALTHUD_LEFT_X + 25 + health - 102, ALTHUD_Y + 9,
            altmark2patch, WHITE, green1, NULL, shadowcolor);
            althudfunc(ALTHUD_LEFT_X + 25 + health - 100, ALTHUD_Y + 13,
            altendpatch, WHITE, green1, NULL, -1);
        }
        else
        {
            fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 13, 101, 8, green3,
            green3, true, true, NULL, NULL);

            if(inverted)
                fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 13,
                MAX(1, health - 99), 8, nearestdarkgray, nearestdarkgray, true,
                (health == 200), NULL, NULL);
            else
                fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 13, MAX(1, health - 99),
    ...
    if(armor)
    {
        if(r_hud_translucency)
            ...
            DrawAltHUDNumber2(ALTHUD_LEFT_X - AltHUDNumber2Width(armor), ALTHUD_Y, armor,
            (armorhighlight > currenttime ? (inverted ? nearestdarkgray : nearestwhite) : color),
            NULL, shadowcolor);

        althudfunc(ALTHUD_LEFT_X + 8, ALTHUD_Y, altleftpatch2, WHITE, color,
        tinttab60, shadowcolor);
        ...
        if((armor *= 200 / max_armor) > 100)
        {
            if(r_hud_translucency)
            {
                if(viewplayer->armortype == green_armor_class)
                {
                    ...
                    fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2,
                    armor - 100 + (armor == 200), 4, green2, green2, true,
                    (armor == 200), tinttab60, tinttab40);
                }
                else
                {
                    fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2, 101, 4,
                    ...
                    fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2,
                    armor - 100 + (armor == 200), 4, blue2, blue2, true,
                    (armor == 200), tinttab90, tinttab10);
                }
            }
            else
            {
                if(viewplayer->armortype == green_armor_class)
                {
                    ...
                    if(inverted)
                        fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2,
                        armor - 100 + (armor == 200), 4, nearestdarkgray,
                        nearestdarkgray, true, (armor == 200), NULL, NULL);
                    else
                        fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2,
                        armor - 100 + (armor == 200), 4, green2, green2, true,
                        (armor == 200), NULL, NULL);
                ...
                {
                    fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2, 101, 4,
                    blue3, blue3, true, true, NULL, NULL);
        ...
        else
        {
            if(r_hud_translucency)
            {
                if(viewplayer->armortype == green_armor_class)
                    fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2,
                    armor + (armor == 100), 4, green2, green2, true, true,
                    tinttab60, tinttab25);
                else
                    fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2,
                    armor + (armor == 100), 4, blue3, gray, true, true, tinttab90, tinttab5);
            }
            else
            {
                const int color2 =
                (viewplayer->armortype == blue_armor_class ? blue3 : green3);

                fillrectfunc(0, ALTHUD_LEFT_X + 25, ALTHUD_Y + 2,
                armor + (armor == 100), 4, color2, color2, true, true, NULL, NULL);
            }
        }
    }
    else
    {
        if(r_hud_translucency)
            althudfunc(ALTHUD_LEFT_X + 8, ALTHUD_Y, altleftpatch2, -1, 0,
            tinttab60, shadowcolor);
        else
    ...
    if(health)
    {
        ...
        if(ammotype != am_noammo)
        {
            ...
            if(r_hud_translucency)
                DrawAltHUDNumber(ALTHUD_RIGHT_X + 101 - AltHUDNumberWidth(ammo),
                ALTHUD_Y - 2, ammo, color,
                (ammohighlight > currenttime ? tinttab80 : tinttab60), shadowcolor);
            else
                DrawAltHUDNumber(ALTHUD_RIGHT_X + 101 - AltHUDNumberWidth(ammo),
                ALTHUD_Y - 2, ammo,
                (ammohighlight > currenttime ? (inverted ? nearestdarkgray : nearestwhite) : color),
                NULL, shadowcolor);
            ...
            if(ammo)
            {
                ...
                if(viewplayer->backpack && ammo > (max /= 2))
                {
                    ...
                    if(r_hud_translucency)
                    {
                        ...
                        fillrectfunc(0, ALTHUD_RIGHT_X, ALTHUD_Y + 13, 101, 8,
                        color, color, true, true, tinttab60, tinttab25);
                        fillrectfunc(0, ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                        ammo + 1, 8, color, color, false, true, tinttab60, tinttab25);
                        althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altrightpatch,
                        WHITE, color, tinttab60, shadowcolor);
                        althudfunc(ALTHUD_RIGHT_X + 100, ALTHUD_Y + 13,
                        altendpatch, WHITE, color2, NULL, -1);
                        althudfunc(ALTHUD_RIGHT_X - 2, ALTHUD_Y + 22,
                        altmarkpatch, WHITE, color2, NULL, shadowcolor);
                        althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altendpatch,
                        WHITE, color2, NULL, -1);
                        althudfunc(ALTHUD_RIGHT_X + 100 - ammo - 2, ALTHUD_Y + 9,
                        altmark2patch, WHITE, color, NULL, shadowcolor);
                        althudfunc(ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                        altendpatch, WHITE, color, NULL, -1);
                    }
                    else
                    {
                        fillrectfunc(0, ALTHUD_RIGHT_X, ALTHUD_Y + 13, 101, 8,
                        color, color, true, true, NULL, NULL);

                        if(inverted)
                        {
                            fillrectfunc(0, ALTHUD_RIGHT_X + 100 - ammo,
                            ALTHUD_Y + 13, ammo + 1, 8, nearestdarkgray,
                            nearestdarkgray, false, true, NULL, NULL);
                            althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13,
                            altrightpatch, WHITE, color, NULL, shadowcolor);
                            althudfunc(ALTHUD_RIGHT_X + 100, ALTHUD_Y + 13,
                            altendpatch, WHITE, nearestblack, NULL, -1);
                            althudfunc(ALTHUD_RIGHT_X - 2, ALTHUD_Y + 22,
                            altmarkpatch, WHITE, nearestblack, NULL, shadowcolor);
                            althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13,
                            altendpatch, WHITE, nearestblack, NULL, -1);
                            althudfunc(ALTHUD_RIGHT_X + 100 - ammo - 2, ALTHUD_Y + 9,
                            altmark2patch, WHITE, nearestblack, NULL, shadowcolor);
                            althudfunc(ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                            altendpatch, WHITE, nearestblack, NULL, -1);
                        }
                        else
                        {
                            fillrectfunc(0, ALTHUD_RIGHT_X + 100 - ammo,
                            ALTHUD_Y + 13, ammo + 1, 8, nearestwhite,
                            nearestwhite, false, true, NULL, NULL);
                            althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13,
                            altrightpatch, WHITE, color, NULL, shadowcolor);
                            althudfunc(ALTHUD_RIGHT_X + 100, ALTHUD_Y + 13,
                            altendpatch, WHITE, nearestwhite, NULL, -1);
                            althudfunc(ALTHUD_RIGHT_X - 2, ALTHUD_Y + 22,
                            altmarkpatch, WHITE, nearestwhite, NULL, shadowcolor);
                            althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13,
                            altendpatch, WHITE, nearestwhite, NULL, -1);
                            althudfunc(ALTHUD_RIGHT_X + 100 - ammo - 2, ALTHUD_Y + 9,
                            altmark2patch, WHITE, nearestwhite, NULL, shadowcolor);
                            althudfunc(ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                            altendpatch, WHITE, nearestwhite, NULL, -1);
                        }
                    }
                ...
                else if((ammo = 100 * ammo / max) < HUD_AMMO_MIN && ammodiff[ammotype] >= 0)
                {
                    ...
                    {
                        fillrectfunc(0, ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                        ammo + 1, 8, yellow2, yellow2, true, true, tinttab60, tinttab25);
                        althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altrightpatch,
                        WHITE, color, tinttab60, shadowcolor);
                    }
                    else
                    {
                        fillrectfunc(0, ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                        ammo + 1, 8, yellow2, yellow2, true, true, NULL, NULL);
                        althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altrightpatch,
                        WHITE, color, NULL, shadowcolor);
                    }

                    althudfunc(ALTHUD_RIGHT_X + 100, ALTHUD_Y + 13, altendpatch,
                    WHITE, yellow1, NULL, -1);
                    althudfunc(ALTHUD_RIGHT_X + 100 - ammo - 2, ALTHUD_Y + 22,
                    altmarkpatch, WHITE, yellow1, NULL, shadowcolor);
                    althudfunc(ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                    altendpatch, WHITE, yellow1, NULL, -1);
                }
                else
                {
                    if(r_hud_translucency)
                    {
                        ...
                        fillrectfunc(0, ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                        ammo + 1, 8, color, color, true, true, tinttab60, tinttab25);
                        althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altrightpatch,
                        WHITE, color, tinttab60, shadowcolor);
                        althudfunc(ALTHUD_RIGHT_X + 100, ALTHUD_Y + 13,
                        altendpatch, WHITE, color2, NULL, -1);
                    ...
                    else
                    {
                        ...
                        fillrectfunc(0, ALTHUD_RIGHT_X + 100 - ammo, ALTHUD_Y + 13,
                        ammo + 1, 8, color, color, true, true, NULL, NULL);
                        althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altrightpatch,
                        WHITE, color, NULL, shadowcolor);
                        althudfunc(ALTHUD_RIGHT_X + 100, ALTHUD_Y + 13,
                        altendpatch, WHITE, color2, NULL, -1);
                ...
                althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altrightpatch, WHITE,
                color, tinttab60, shadowcolor);
        }
        else
            althudfunc(ALTHUD_RIGHT_X, ALTHUD_Y + 13, altrightpatch, WHITE,
        ...
        if((patch = weaponinfo[weapon].weaponpatch))
            althudweaponfunc(ALTHUD_RIGHT_X + 108, weaponinfo[weapon].weapony,
        ...
        for(int i = 1; i <= NUMCARDS; i++)
            for(int j = 0; j < NUMCARDS; j++)
                if(viewplayer->cards[j] == i)
                {
                    ...
                    if(inverted)
                        althudfunc(keypic_x, ALTHUD_Y - altkeypic.height + 7,
                        patch, WHITE, nearestblack, tinttab60, shadowcolor);
                    else
                    ...
                    for(int i = 0; i < NUMCARDS; i++)
                        if(viewplayer->cards[i] == CARDNOTFOUNDYET)
                        {
                            ...
                            if(inverted)
                                ...
                                althudfunc(keypic_x,
                                ALTHUD_Y - altkeypic.height + 7, patch, WHITE,
                                altkeypic.color, altkeypic.tinttab, shadowcolor);
        ...
        if(powertics &&
        (powerupbar =
         (powerupbar == INT_MAX ? 101 : (int)(powerupbar * 101.0 / powertics + 0.5))))
        {
            const int color2 = (r_hud_translucency ? color : darkgray);

            fillrectfunc2(0, ALTHUD_RIGHT_X, ALTHUD_Y + 27, 101 - powerupbar, 3,
            color2, color2, false, false, tinttab20, NULL);
            fillrectfunc2(0, ALTHUD_RIGHT_X + 101 - powerupbar, ALTHUD_Y + 27,
            powerupbar, 3, color, color, false, false, tinttab60, NULL);
        }
    }
}
...
void HU_DrawDisk(void)
{
    ...
    {
        V_DrawPatch(video.screen_width / 2 - HU_MSGX * (vid_widescreen ? 2 : 1) - stdiskwidth - video.widescreen_delta,
        HU_MSGY - 1, 0, stdisk);
...
void HU_Drawer(void)
{
    ...
    if(*w_message.l.l)
    {
        ...
        {
            w_message.l.x = video.max_widescreen_delta / 2 - HU_MSGX - 3;
            w_message.l.y = HU_MSGY + 4;
        }
        else if(vid_widescreen && r_screensize == r_screensize_max - 1)
        {
            w_message.l.x = video.widescreen_delta + HU_MSGX * !automapactive;
    ...
    if(automapactive)
    {
        ...
        if(r_althud && r_althudfont && r_screensize == r_screensize_max)
        {
            ...
            {
                w_author.y = video.screen_height - 30;
                HUlib_DrawAltAutomapTextLine(&w_author, false);

                w_title.y = video.screen_height - 44;
            }
            else
                w_title.y = video.screen_height - 30;
        ...
        else
        {
            if(author && *author && am_author)
            {
                ...
                {
                    w_author.x =
                    (r_screensize == r_screensize_max - 1 ? video.widescreen_delta * 2 : OVERLAYTEXTX);
                    w_author.y = V_MAPHEIGHT - SHORT(hu_font[0]->height) * 2 -
                    (r_screensize == r_screensize_max - 1 ? 6 : 20);

                    w_title.x =
                    (r_screensize == r_screensize_max - 1 ? video.widescreen_delta * 2 : OVERLAYTEXTX);
                    w_title.y = V_MAPHEIGHT - SHORT(hu_font[0]->height) * 2 -
                    (r_screensize == r_screensize_max - 1 ? 22 : 38);
                }
                else
                {
                    w_author.x = 0;
                    w_author.y = V_MAPHEIGHT - SHORT(hu_font[0]->height) * 2 - 6;

                    w_title.x = 0;
                    w_title.y = V_MAPHEIGHT - SHORT(hu_font[0]->height) * 2 - 22;
                }
            ...
            else
            {
                ...
                {
                    w_title.x = 0;
                    w_title.y = V_MAPHEIGHT - SHORT(hu_font[0]->height) * 2 - 6;
                }
            }
...
void HU_SetPlayerMessage(char* message, bool group, bool external)
{
    ...
    else
    {
        ...
        if(game.time - viewplayer->prevmessagetics < HU_MSGTIMEOUT &&
...
void HU_PlayerMessage(char* message, bool group, bool external)
{
    ...
    if(game.state == GS_LEVEL && !message_dontfuckwithme)

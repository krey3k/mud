...
byte tempscreen[V_MAXSCREENAREA];
...
static void
HU_DrawTranslucentChar(int x, int y, int ch, byte* screen, int screenwidth, byte* cr)
{
    const int width = (int)strlen(smallcharset[ch]) / 10;
...
static void HUlib_DrawAltHUDTextLine(hu_textline_t* l)
{
    ...
    int x                     = HU_ALTHUDMSGX;
    int y                     = HU_ALTHUDMSGY;
    int color                 = (message_secret ?
                    nearestgold :
                    (message_warning ? nearestred : (r_hud_translucency ? nearestwhite : nearestlightgray)));
    ...
    {
        color   = (r_textures ?
          (viewplayer->fixedcolormap == INVERSECOLORMAP ? colormaps[0][32 * 256 + color] : color) :
          (viewplayer->fixedcolormap == INVERSECOLORMAP ?
          colormaps[0][32 * 256 + color] :
          (message_secret ? nearestgold : (message_warning ? nearestred : nearestblack))));
        tinttab = tinttab50;
    ...
    {
        althudtextfunc(x, y, v_screens[0], altbuddha, false, color, shadowcolor,
        video.screen_width, tinttab);
        return;
    }

    if(idbehold)
        althudtextfunc(x, y + 12, v_screens[0], altunderscores, false, color,
        shadowcolor, video.screen_width, tinttab);
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        else
        {
            ...
            althudtextfunc(x, y, v_screens[0], patch, italics, color, shadowcolor,
            video.screen_width, tinttab);
            x += SHORT(patch->width);
...
void HUlib_DrawAltAutomapTextLine(hu_textline_t* l, bool external)
{
    ...
    unsigned char prevletter2 = '\0';
    int x                     = HU_ALTHUDMSGX;
    byte* fb                  = (external ? v_mapscreen : v_screens[0]);
    const int len             = l->len;
    int color =
    (secretmap ? nearestgold : (r_hud_translucency ? nearestwhite : nearestlightgray));
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        else
        {
            ...
            althudtextfunc(x, l->y, fb, patch,
            (italics && letter != '_' && letter != '-' && letter != '+' &&
            letter != ',' && letter != '/'),
            color, nearestdarkgray, (external ? V_MAPWIDTH : video.screen_width), tinttab70);
            x += SHORT(patch->width);
        }
...
static void HUlib_DrawTextLine(hu_textline_t* l, bool external)
{
    ...
    if(external)
    {
        fb1         = v_mapscreen;
        fb2         = v_mapscreen;
        screenwidth = V_MAPWIDTH;
    }
    else
    {
        fb1 = v_screens[0];
        fb2 = v_screens[(r_screensize < r_screensize_max - 1 && !automapactive)];
        screenwidth = video.screen_width;
    }
    ...
    if(len > 40)
    {
        ...
        const int maxwidth2 =
        maxwidth1 - (r_screensize == r_screensize_max ? 0 : video.widescreen_delta * 2);

        if(width > maxwidth2 && width <= maxwidth1 && vid_widescreen)
            x = (screenwidth / 2 - width) / 2;
        else if(width > maxwidth2)
            for(wrap = len; wrap > 0; wrap--)
                if((width -= M_CharacterWidth(l->l[wrap], '\0')) <= maxwidth2 &&
                isbreak(l->l[wrap]))
                    break;
    }
    ...
    else
    {
        ...
        if(idbehold && !STCFNxxx && s_STSTR_BEHOLD2 && !vanilla)
            for(int y1 = 0; y1 < 4; y1++)
                for(int x1 = 0; x1 < V_VANILLAWIDTH; x1++)
                {
                    const unsigned char src = underscores[y1 * V_VANILLAWIDTH + x1];
    ...
    if(fade)
    {
        byte* tinttabs1[] = { NULL, tinttab10, tinttab20, tinttab20, tinttab30,
            tinttab30, tinttab40, tinttab40 };
        byte* tinttabs2[] = { NULL, tinttab10, tinttab20, tinttab30, tinttab40,
            tinttab50, tinttab60, tinttab70 };
...
void HUlib_DrawAutomapTextLine(hu_textline_t* l, bool external)
{
    ...
    const int black = (nearestblack << 8);
    char s[513];
    const int maxwidth = (external ?
    V_MAPWIDTH / 2 :
    (r_screensize == r_screensize_max ? video.screen_width / 2 : V_VANILLAWIDTH));
    int len            = l->len;
    int screenwidth;
    int screenarea;
    ...
    else
    {
        ...
        screenwidth = video.screen_width;
        screenarea =
        (r_screensize == r_screensize_max ? video.screen_area : video.screen_area - V_SBARHEIGHT * video.screen_width);
    }
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        else if(c != '\n' && c >= l->sc && c <= '_')
        {
            ...
            if(STCFNxxx)
            {
                ...
                V_DrawPatchToTempScreen(x / 2, y / 2, l->f[j],
                (secretmap ? cr_gold : cr_none), screenwidth);
            }
            else
            {
                // [BH] have matching curly single and double quotes
                ...
                if(r_hud_translucency)
                    HU_DrawTranslucentChar(x / 2, y / 2, j, fb1, screenwidth,
                    (secretmap && !vanilla ? cr_gold : cr_none));
                else
                    HU_DrawChar(x / 2, y / 2, j, fb1, screenwidth,
                    (secretmap && !vanilla ? cr_gold : cr_none));
            }
...
void HUlib_EraseTextLine(hu_textline_t* l)
{
    ...
    if(!automapactive && v_viewwindowx && l->needsupdate)
    {
        const int lh = (SHORT(l->f[0]->height) + 4) * 2;

        for(int y = l->y, yoffset = y * video.screen_width; y < l->y + lh; y++, yoffset += video.screen_width)
            if(y < v_viewwindowy || y >= v_viewwindowy + v_viewheight)
                R_VideoErase(yoffset, video.screen_width); // erase entire line
            else
            {
                R_VideoErase(yoffset, v_viewwindowx); // erase left border
                R_VideoErase(yoffset + v_viewwindowx + v_viewwidth, v_viewwindowx); // erase right border
            }
    }

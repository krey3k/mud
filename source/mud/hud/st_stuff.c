...
#define ST_X 0
#define ST_Y (video.screen_height - V_SBARHEIGHT)
...
#define ST_FACEBACKX (144 * 2 + video.widescreen_delta * 2)
...
// Ammunition counter.
#define ST_AMMOBGX (248 + video.widescreen_delta)
...
cheatseq_t cheat_powerup[7] = { CHEAT("idbeholdv", 0, true),
    CHEAT("idbeholds", 0, true), CHEAT("idbeholdi", 0, true),
    CHEAT("idbeholdr", 0, true), CHEAT("idbeholda", 0, true),
    CHEAT("idbeholdl", 0, true), CHEAT("idbehold", 0, true) };

static bool movekey(char key)
{
    return (key == keyboardright || key == keyboardright2 || key == keyboardleft ||
    key == keyboardleft2 || key == keyboardforward || key == keyboardforward2 ||
    key == keyboardback || key == keyboardback2 || key == keyboardstrafeleft ||
    key == keyboardstrafeleft2 || key == keyboardstraferight || key == keyboardstraferight2);
}
...
{
    if(sbarwidth < video.screen_width)
        R_FillBezel();
    V_DrawWidePatch((video.screen_width / 2 - sbarwidth) / 2,
    V_VANILLAHEIGHT - V_VANILLASBARHEIGHT, 0, sbar);
    V_DrawPatch(ST_ARMSBGX,
    V_VANILLAHEIGHT - V_VANILLASBARHEIGHT, 0, armsbg);
}
...
bool ST_Responder(const event_t* ev)
{
    ...
    if(ev->type == ev_keydown || *consolecheat)
    {
        if(!menuactive && !paused) // [BH] no cheats when in menu or paused
        {
            ...
            if(!*consolecheat && cht_CheckCheat(&cheat_mus, ev->data2) && !nomusic && s_musicvolume)
                idmus = true;

            // 'dqd' cheat for toggleable god mode
            if(cht_CheckCheat(&cheat_god, ev->data2) && game.skill != sk_nightmare)
            {
            ...
            game.skill != sk_nightmare && viewplayer->health > 0)
            {
                bool ammogiven;
            ...
            else if(cht_CheckCheat(&cheat_mus_xy, ev->data2) && !nomusic && s_musicvolume)
            {
                char buffer[3];
            ...
            else if(((cht_CheckCheat(&cheat_noclip, ev->data2) && game.mode != commercial) ||
                    (cht_CheckCheat(&cheat_commercial_noclip, ev->data2) && game.mode == commercial)) &&
            game.skill != sk_nightmare
            // [BH] can only enter cheat while player is alive
            && viewplayer->health > 0)
            {
                ...
                {
                    ST_PlayerCheated(
                    (game.mode == commercial ? cheat_commercial_noclip.sequence :
                                              cheat_noclip.sequence),
                    "", NULL, true);
            ...
            for(int i = 1; i < NUMPOWERS; i++)
            {
                ...
                && viewplayer->health > 0)
                {
                    ...
                    if((i != pw_strength && viewplayer->powers[i] >= 0 &&
                       viewplayer->powers[i] <= STARTFLASHING) ||
                    (i == pw_strength && !viewplayer->powers[i]))
                    {
                        ...
                        else
                        {
                            ...
                            if(viewplayer->cheats & CF_CHOPPERS)
                            {
                                ...
                                viewplayer->chainsawbeforechoppers;
                        ...
                        cheat_powerup[i - 1]
                        .sequence[strlen(cheat_powerup[i - 1].sequence) - 1]);
            ...
            && viewplayer->health > 0)
            {
                ...
                if(!(viewplayer->cheats & CF_CHOPPERS))
                {
                    ...
                    viewplayer->invulnbeforechoppers =
                    viewplayer->powers[pw_invulnerability];
                    viewplayer->chainsawbeforechoppers =
                    viewplayer->weaponowned[wp_chainsaw];
            ...
            else if(cht_CheckCheat(&cheat_mypos, ev->data2))
            {
                ...
                ST_PlayerCheated(
                cheat_mypos.sequence, "", NULL, (viewplayer->cheats & CF_MYPOS));
            }
            ...
            if(cht_CheckCheat(&cheat_clev_xy, ev->data2))
            {
                ...
                if(W_CheckNumForName(lump) < 0 || (game.mission == pack_nerve && map > 9))
                    return false;
                else
                {
                    static char message[128];
                    ...
                    if(M_StringCompare(lump, mapnum))
                        ...
                        M_snprintf(message, sizeof(message),
                        s_STSTR_CLEV, (M_StringCompare(playername, playername_default) ? "you" : playername),
                        lump);
                    ...
                    samelevel   = (game.episode == epsd && game.map == map);
                    game.episode = epsd;
...
static void ST_UpdateFaceWidget(void)
{
    ...
    if(priority < 10)
    {
        ...
        if(viewplayer->health <= 0)
        {
            ...
            if(xdthfaces)
            {
                const int state = (int)(viewplayer->mo->state - states) -
                mobjinfo[MT_PLAYER].xdeathstate;

                faceindex =
                (state >= 0 ? ST_XDTHFACE + MIN(state, xdthfaces - 1) : ST_XDTHFACE);
            }
            else
    ...
    if(priority < 8)
    {
        ...
        viewplayer->attacker != viewplayer->mo)
        {
            ...
            {
                const angle_t badguyangle =
                R_PointToAngle(viewplayer->attacker->x, viewplayer->attacker->y);
                angle_t diffang;
                ...
                if(badguyangle > viewangle)
                {
                    ...
                    faceindex = (diffang < ANG45 ? ST_RAMPAGEOFFSET :
                                                   ST_TURNOFFSET + (diffang <= ANG180));
                }
                else
                {
...
void ST_Ticker(void)
{
    ...
    if(idclevtics && !--idclevtics)
    {
        ...
        if(vanilla)
            G_DeferredInitNew(game.skill, game.episode, game.map);
        else
            G_DeferredLoadLevel(game.skill, game.episode, game.map);
    }
}
...
static void ST_DoPaletteStuff(void)
{
    ...
    viewplayer->health > 0)
    {
        ...
        else
        {
            ...
            if(ironfeet <= STARTFLASHING && (ironfeet & FLASHONTIC) && r_radsuiteffect)
                ...
                palette = MIN((viewplayer->damagecount >> 3) +
                r_berserkeffect * (PLAYPALs > 2 ? 1 : 2),
                NUMREDPALS);
        }
    }
    else
    {
        ...
        if(damagecount && !(viewplayer->cheats & CF_GODMODE) && r_damageeffect)
            palette = (r_blood == r_blood_green ? RADIATIONPAL :
                                                          STARTREDPALS +
            MIN((damagecount + 8) / (PLAYPALs > 2 ? 8 : 5), NUMREDPALS) - 1);
        else if(viewplayer->health > 0)
        {
            ...
            else
            {
                ...
                {
                    // [BH] flash radsuit effect when r_radsuiteffect CVAR is
                    // off and radsuit just picked up or is running out
                    if((ironfeet <= STARTFLASHING && (ironfeet & FLASHONTIC)) ||
...
static void ST_DrawWidgets(bool refresh)
{
    ...
    st_shotguns =
    (viewplayer->weaponowned[wp_shotgun] || viewplayer->weaponowned[wp_supershotgun]);
    ...
    if(facebackcolor != facebackcolor_default)
        ...
        nearestcolors[facebackcolor], 0, false, false, NULL, NULL);
    else if(solonet)
...
static void ST_LoadUnloadGraphics(void callback(const char*, patch_t**))
{
    ...
    // back screen
    callback((game.mode == commercial ? "GRNROCK" : "FLOOR7_2"), (patch_t**)&grnrock);
...
static void ST_LoadCallback(const char* lumpname, patch_t** variable)
{
    if(M_StringCompare(lumpname, "STARMS") || M_StringCompare(lumpname, "STFGOD0"))
        *variable = ((FREEDOOM && !modifiedgame) ?
        W_CacheLastLumpName(lumpname) :
        W_CacheLumpName(lumpname));
    else
...
static void ST_CreateWidgets(void)
{
    ...
    // weapons owned
    STlib_InitMultIcon(&w_arms[0], ST_ARMSX, ST_ARMSY, arms[0],
    &viewplayer->weaponowned[wp_pistol]);
    ...
    STlib_InitMultIcon(&w_arms[3], ST_ARMSX, ST_ARMSY + ST_ARMSYSPACE, arms[3],
    &viewplayer->weaponowned[wp_missile]);

    if(game.mode != shareware)
    {
        STlib_InitMultIcon(&w_arms[4], ST_ARMSX + ST_ARMSXSPACE,
        ST_ARMSY + ST_ARMSYSPACE, arms[4], &viewplayer->weaponowned[wp_plasma]);
        STlib_InitMultIcon(&w_arms[5], ST_ARMSX + 2 * ST_ARMSXSPACE,
        ST_ARMSY + ST_ARMSYSPACE, arms[5], &viewplayer->weaponowned[wp_bfg]);
    }
    ...
    ST_KEY0X + (STBARs >= 3), ST_KEY0Y, keys, &keyboxes[0]);
    STlib_InitMultIcon(&w_keyboxes[1],
    ST_KEY1X + (STBARs >= 3), ST_KEY1Y, keys, &keyboxes[1]);
    STlib_InitMultIcon(&w_keyboxes[2],
    ST_KEY2X + (STBARs >= 3), ST_KEY2Y, keys, &keyboxes[2]);

    // ammo count (all four kinds)
    STlib_InitNum(&w_ammo[am_clip], ST_AMMO0X, ST_AMMO0Y, shortnum,
    &viewplayer->ammo[am_clip], ST_AMMO0WIDTH);
    STlib_InitNum(&w_ammo[am_shell], ST_AMMO1X, ST_AMMO1Y, shortnum,
    &viewplayer->ammo[am_shell], ST_AMMO1WIDTH);
    STlib_InitNum(&w_ammo[am_misl], ST_AMMO2X, ST_AMMO2Y, shortnum,
    &viewplayer->ammo[am_misl], ST_AMMO2WIDTH);
    STlib_InitNum(&w_ammo[am_cell], ST_AMMO3X, ST_AMMO3Y, shortnum,
    &viewplayer->ammo[am_cell], ST_AMMO3WIDTH);

    // max ammo count (all four kinds)
    STlib_InitNum(&w_maxammo[am_clip], ST_MAXAMMO0X, ST_MAXAMMO0Y, shortnum,
    &viewplayer->maxammo[am_clip], ST_MAXAMMO0WIDTH);
    STlib_InitNum(&w_maxammo[am_shell], ST_MAXAMMO1X, ST_MAXAMMO1Y, shortnum,
    &viewplayer->maxammo[am_shell], ST_MAXAMMO1WIDTH);
    STlib_InitNum(&w_maxammo[am_misl], ST_MAXAMMO2X, ST_MAXAMMO2Y, shortnum,
    &viewplayer->maxammo[am_misl], ST_MAXAMMO2WIDTH);
    STlib_InitNum(&w_maxammo[am_cell], ST_MAXAMMO3X, ST_MAXAMMO3Y, shortnum,
    &viewplayer->maxammo[am_cell], ST_MAXAMMO3WIDTH);
}
...
void ST_Init(void)
{
    ...
    lumpinfo[W_GetNumForName(game.mode == commercial ? "GRNROCK" : "FLOOR7_2")]
    ->wadfile->type == IWAD);
    ...
    usesmallnums =
    ((!STYSNUM0 && STBARs == 2) || game.mode == shareware);

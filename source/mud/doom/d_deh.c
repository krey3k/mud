...
static char* dehfgets(char* str, int count, DEHFILE* fp)
{
    ...
    if(fp->file)
        return FS_GetString(str, count, fp->file);
    else if(fp->lump)
        return FS_MemGetString(str, count, fp->lump);
...
{
    if(fp->file)
        return FS_EOF(fp->file);
    else if(fp->lump)
        return FS_MemEOF(fp->lump);
...
{
    if(fp->file)
        return FS_GetChar(fp->file);
    else if(fp->lump)
        return FS_MemGetChar(fp->lump);

    return -1;
}

static fs_int64 dehftell(DEHFILE* fp)
{
    if(fp->file)
        return FS_Tell(fp->file);
    else if(fp->lump)
        return FS_MemTell(fp->lump);
...
{
    if(fp->file)
        return FS_Seek(fp->file, offset, FS_SEEK_SET);
    else if(fp->lump)
        return FS_MemSeek(fp->lump, offset, FS_SEEK_SET);

    return 0;
}

// #include "doom/d_deh.h" -- we don't do that here but we declare the
...
char* s_M_CONTROLLERSENSITIVITY = "";
...
// M_StringDuplicate to set these new values that we read from the file,
// orphaning the original value set above.

deh_strs deh_strlookup[] = { { &s_VERSION, "VERSION" },

    { &s_D_DEVSTR, "D_DEVSTR" }, { &s_D_CDROM, "D_CDROM" },

    { &s_PRESSKEY, "PRESSKEY" }, { &s_PRESSYN, "PRESSYN" }, { &s_PRESSA, "PRESSA" },
    { &s_QUITMSG, "QUITMSG" }, { &s_QUITMSG1, "QUITMSG1" }, { &s_QUITMSG2, "QUITMSG2" },
    { &s_QUITMSG3, "QUITMSG3" }, { &s_QUITMSG4, "QUITMSG4" },
    { &s_QUITMSG5, "QUITMSG5" }, { &s_QUITMSG6, "QUITMSG6" },
    { &s_QUITMSG7, "QUITMSG7" }, { &s_QUITMSG8, "QUITMSG8" },
    { &s_QUITMSG9, "QUITMSG9" }, { &s_QUITMSG10, "QUITMSG10" },
    { &s_QUITMSG11, "QUITMSG11" }, { &s_QUITMSG12, "QUITMSG12" },
    { &s_QUITMSG13, "QUITMSG13" }, { &s_QUITMSG14, "QUITMSG14" },
    { &s_DEVQUITMSG1, "DEVQUITMSG1" }, { &s_DEVQUITMSG2, "DEVQUITMSG2" },
    { &s_DEVQUITMSG3, "DEVQUITMSG3" }, { &s_DEVQUITMSG4, "DEVQUITMSG4" },
    { &s_DEVQUITMSG5, "DEVQUITMSG5" }, { &s_DEVQUITMSG6, "DEVQUITMSG6" },
    { &s_DEVQUITMSG7, "DEVQUITMSG7" }, { &s_LOADNET, "LOADNET" },
    { &s_QLOADNET, "QLOADNET" }, { &s_QSAVESPOT, "QSAVESPOT" },
    { &s_SAVEDEAD, "SAVEDEAD" }, { &s_QSPROMPT, "QSPROMPT" },
    { &s_QLPROMPT, "QLPROMPT" }, { &s_DELPROMPT, "DELPROMPT" },
    { &s_NEWGAME, "NEWGAME" }, { &s_NIGHTMARE, "NIGHTMARE" },
    { &s_SWSTRING, "SWSTRING" }, { &s_MSGOFF, "MSGOFF" }, { &s_MSGON, "MSGON" },
    { &s_NETEND, "NETEND" }, { &s_ENDGAME, "ENDGAME" }, { &s_DOSY, "DOSY" },
    { &s_DOSA, "DOSA" }, { &s_DETAILHI, "DETAILHI" }, { &s_DETAILLO, "DETAILLO" },
    { &s_GAMMALVL0, "GAMMALVL0" }, { &s_GAMMALVL1, "GAMMALVL1" },
    { &s_GAMMALVL2, "GAMMALVL2" }, { &s_GAMMALVL3, "GAMMALVL3" },
    { &s_GAMMALVL4, "GAMMALVL4" }, { &s_GAMMALVL, "GAMMALVL" },
    { &s_GAMMAOFF, "GAMMAOFF" }, { &s_EMPTYSTRING, "EMPTYSTRING" },

    { &s_GOTARMOR, "GOTARMOR" }, { &s_GOTMEGA, "GOTMEGA" },
    { &s_GOTHTHBONUS, "GOTHTHBONUS" }, { &s_GOTARMBONUS, "GOTARMBONUS" },
    { &s_GOTSTIM, "GOTSTIM" }, { &s_GOTMEDINEED, "GOTMEDINEED" },
    { &s_GOTMEDIKIT, "GOTMEDIKIT" }, { &s_GOTSUPER, "GOTSUPER" },

    { &s_GOTBLUECARD, "GOTBLUECARD" }, { &s_GOTYELWCARD, "GOTYELWCARD" },
    { &s_GOTREDCARD, "GOTREDCARD" }, { &s_GOTBLUESKUL, "GOTBLUESKUL" },
    { &s_GOTYELWSKUL, "GOTYELWSKUL" }, { &s_GOTREDSKUL, "GOTREDSKUL" },
    { &s_GOTREDSKULL, "GOTREDSKULL" },

    { &s_GOTINVUL, "GOTINVUL" }, { &s_GOTBERSERK, "GOTBERSERK" },
    { &s_GOTINVIS, "GOTINVIS" }, { &s_GOTSUIT, "GOTSUIT" },
    { &s_GOTMAP, "GOTMAP" }, { &s_GOTVISOR, "GOTVISOR" },

    { &s_GOTCLIP, "GOTCLIP" }, { &s_GOTCLIPBOX, "GOTCLIPBOX" },
    { &s_GOTROCKET, "GOTROCKET" }, { &s_GOTROCKETX2, "GOTROCKETX2" },
    { &s_GOTROCKBOX, "GOTROCKBOX" }, { &s_GOTCELL, "GOTCELL" },
    { &s_GOTCELLX2, "GOTCELLX2" }, { &s_GOTCELLBOX, "GOTCELLBOX" },
    { &s_GOTSHELLS, "GOTSHELLS" }, { &s_GOTSHELLSX2, "GOTSHELLSX2" },
    { &s_GOTSHELLBOX, "GOTSHELLBOX" }, { &s_GOTBACKPACK, "GOTBACKPACK" },

    { &s_GOTBFG9000, "GOTBFG9000" }, { &s_GOTCHAINGUN, "GOTCHAINGUN" },
    { &s_GOTCHAINSAW, "GOTCHAINSAW" }, { &s_GOTLAUNCHER, "GOTLAUNCHER" },
    { &s_GOTMSPHERE, "GOTMSPHERE" }, { &s_GOTPLASMA, "GOTPLASMA" },
    { &s_GOTSHOTGUN, "GOTSHOTGUN" }, { &s_GOTSHOTGUN2, "GOTSHOTGUN2" },

    { &s_PD_BLUEO, "PD_BLUEO" }, { &s_PD_REDO, "PD_REDO" },
    { &s_PD_YELLOWO, "PD_YELLOWO" }, { &s_PD_BLUEK, "PD_BLUEK" },
    { &s_PD_REDK, "PD_REDK" }, { &s_PD_YELLOWK, "PD_YELLOWK" },
    { &s_PD_BLUEC, "PD_BLUEC" }, { &s_PD_REDC, "PD_REDC" },
    { &s_PD_YELLOWC, "PD_YELLOWC" }, { &s_PD_BLUES, "PD_BLUES" },
    { &s_PD_REDS, "PD_REDS" }, { &s_PD_YELLOWS, "PD_YELLOWS" },
    { &s_PD_ANY, "PD_ANY" }, { &s_PD_ALL3, "PD_ALL3" }, { &s_PD_ALL6, "PD_ALL6" },
    { &s_PD_KEYCARD, "PD_KEYCARD" }, { &s_PD_SKULLKEY, "PD_SKULLKEY" },
    { &s_PD_KEYCARDORSKULLKEY, "PD_KEYCARDORSKULLKEY" }, { &s_PD_BLUECO, "PD_BLUECO" },
    { &s_PD_REDCO, "PD_REDCO" }, { &s_PD_YELLOWCO, "PD_YELLOWCO" },

    { &s_KILLED, "KILLED" }, { &s_GIBBED, "GIBBED" },

    { &s_SECRETMESSAGE, "SECRETMESSAGE" },

    { &s_GGSAVED, "GGSAVED" }, { &s_GGAUTOSAVED, "GGAUTOSAVED" },
    { &s_GGLOADED, "GGLOADED" }, { &s_GGAUTOLOADED, "GGAUTOLOADED" },
    { &s_GGDELETED, "GGDELETED" }, { &s_GSCREENSHOT, "GSCREENSHOT" },

    { &s_ALWAYSRUNOFF, "ALWAYSRUNOFF" }, { &s_ALWAYSRUNON, "ALWAYSRUNON" },

    { &s_HUSTR_MSGU, "HUSTR_MSGU" },

    { &s_HUSTR_E1M1, "HUSTR_E1M1" }, { &s_HUSTR_E1M2, "HUSTR_E1M2" },
    { &s_HUSTR_E1M3, "HUSTR_E1M3" }, { &s_HUSTR_E1M4, "HUSTR_E1M4" },
    { &s_HUSTR_E1M4B, "HUSTR_E1M4B" }, { &s_HUSTR_E1M8B, "HUSTR_E1M8B" },
    { &s_HUSTR_E1M5, "HUSTR_E1M5" }, { &s_HUSTR_E1M6, "HUSTR_E1M6" },
    { &s_HUSTR_E1M7, "HUSTR_E1M7" }, { &s_HUSTR_E1M8, "HUSTR_E1M8" },
    { &s_HUSTR_E1M9, "HUSTR_E1M9" }, { &s_HUSTR_E2M1, "HUSTR_E2M1" },
    { &s_HUSTR_E2M2, "HUSTR_E2M2" }, { &s_HUSTR_E2M3, "HUSTR_E2M3" },
    { &s_HUSTR_E2M4, "HUSTR_E2M4" }, { &s_HUSTR_E2M5, "HUSTR_E2M5" },
    { &s_HUSTR_E2M6, "HUSTR_E2M6" }, { &s_HUSTR_E2M7, "HUSTR_E2M7" },
    { &s_HUSTR_E2M8, "HUSTR_E2M8" }, { &s_HUSTR_E2M9, "HUSTR_E2M9" },
    { &s_HUSTR_E3M1, "HUSTR_E3M1" }, { &s_HUSTR_E3M2, "HUSTR_E3M2" },
    { &s_HUSTR_E3M3, "HUSTR_E3M3" }, { &s_HUSTR_E3M4, "HUSTR_E3M4" },
    { &s_HUSTR_E3M5, "HUSTR_E3M5" }, { &s_HUSTR_E3M6, "HUSTR_E3M6" },
    { &s_HUSTR_E3M7, "HUSTR_E3M7" }, { &s_HUSTR_E3M7_ALT, "HUSTR_E3M7_ALT" },
    { &s_HUSTR_E3M8, "HUSTR_E3M8" }, { &s_HUSTR_E3M9, "HUSTR_E3M9" },
    { &s_HUSTR_E4M1, "HUSTR_E4M1" }, { &s_HUSTR_E4M2, "HUSTR_E4M2" },
    { &s_HUSTR_E4M3, "HUSTR_E4M3" }, { &s_HUSTR_E4M4, "HUSTR_E4M4" },
    { &s_HUSTR_E4M5, "HUSTR_E4M5" }, { &s_HUSTR_E4M6, "HUSTR_E4M6" },
    { &s_HUSTR_E4M7, "HUSTR_E4M7" }, { &s_HUSTR_E4M8, "HUSTR_E4M8" },
    { &s_HUSTR_E4M9, "HUSTR_E4M9" }, { &s_HUSTR_E5M1, "HUSTR_E5M1" },
    { &s_HUSTR_E5M2, "HUSTR_E5M2" }, { &s_HUSTR_E5M3, "HUSTR_E5M3" },
    { &s_HUSTR_E5M4, "HUSTR_E5M4" }, { &s_HUSTR_E5M5, "HUSTR_E5M5" },
    { &s_HUSTR_E5M6, "HUSTR_E5M6" }, { &s_HUSTR_E5M7, "HUSTR_E5M7" },
    { &s_HUSTR_E5M8, "HUSTR_E5M8" }, { &s_HUSTR_E5M9, "HUSTR_E5M9" },
    { &s_HUSTR_E6M1, "HUSTR_E6M1" }, { &s_HUSTR_E6M2, "HUSTR_E6M2" },
    { &s_HUSTR_E6M3, "HUSTR_E6M3" }, { &s_HUSTR_E6M4, "HUSTR_E6M4" },
    { &s_HUSTR_E6M5, "HUSTR_E6M5" }, { &s_HUSTR_E6M6, "HUSTR_E6M6" },
    { &s_HUSTR_E6M7, "HUSTR_E6M7" }, { &s_HUSTR_E6M8, "HUSTR_E6M8" },
    { &s_HUSTR_E6M9, "HUSTR_E6M9" }, { &s_HUSTR_1, "HUSTR_1" },
    { &s_HUSTR_2, "HUSTR_2" }, { &s_HUSTR_3, "HUSTR_3" }, { &s_HUSTR_4, "HUSTR_4" },
    { &s_HUSTR_5, "HUSTR_5" }, { &s_HUSTR_6, "HUSTR_6" }, { &s_HUSTR_7, "HUSTR_7" },
    { &s_HUSTR_8, "HUSTR_8" }, { &s_HUSTR_9, "HUSTR_9" }, { &s_HUSTR_10, "HUSTR_10" },
    { &s_HUSTR_11, "HUSTR_11" }, { &s_HUSTR_11_ALT, "HUSTR_11_ALT" },
    { &s_HUSTR_12, "HUSTR_12" }, { &s_HUSTR_13, "HUSTR_13" },
    { &s_HUSTR_14, "HUSTR_14" }, { &s_HUSTR_15, "HUSTR_15" },
    { &s_HUSTR_16, "HUSTR_16" }, { &s_HUSTR_17, "HUSTR_17" },
    { &s_HUSTR_18, "HUSTR_18" }, { &s_HUSTR_19, "HUSTR_19" },
    { &s_HUSTR_20, "HUSTR_20" }, { &s_HUSTR_21, "HUSTR_21" },
    { &s_HUSTR_22, "HUSTR_22" }, { &s_HUSTR_23, "HUSTR_23" },
    { &s_HUSTR_24, "HUSTR_24" }, { &s_HUSTR_25, "HUSTR_25" },
    { &s_HUSTR_26, "HUSTR_26" }, { &s_HUSTR_27, "HUSTR_27" },
    { &s_HUSTR_28, "HUSTR_28" }, { &s_HUSTR_29, "HUSTR_29" },
    { &s_HUSTR_30, "HUSTR_30" }, { &s_HUSTR_31, "HUSTR_31" },
    { &s_HUSTR_31_BFG, "HUSTR_31_BFG" }, { &s_HUSTR_32, "HUSTR_32" },
    { &s_HUSTR_32_BFG, "HUSTR_32_BFG" }, { &s_HUSTR_33, "HUSTR_33" },
    { &s_PHUSTR_1, "PHUSTR_1" }, { &s_PHUSTR_2, "PHUSTR_2" },
    { &s_PHUSTR_3, "PHUSTR_3" }, { &s_PHUSTR_4, "PHUSTR_4" },
    { &s_PHUSTR_5, "PHUSTR_5" }, { &s_PHUSTR_6, "PHUSTR_6" },
    { &s_PHUSTR_7, "PHUSTR_7" }, { &s_PHUSTR_8, "PHUSTR_8" },
    { &s_PHUSTR_9, "PHUSTR_9" }, { &s_PHUSTR_10, "PHUSTR_10" },
    { &s_PHUSTR_11, "PHUSTR_11" }, { &s_PHUSTR_12, "PHUSTR_12" },
    { &s_PHUSTR_13, "PHUSTR_13" }, { &s_PHUSTR_14, "PHUSTR_14" },
    { &s_PHUSTR_15, "PHUSTR_15" }, { &s_PHUSTR_16, "PHUSTR_16" },
    { &s_PHUSTR_17, "PHUSTR_17" }, { &s_PHUSTR_18, "PHUSTR_18" },
    { &s_PHUSTR_19, "PHUSTR_19" }, { &s_PHUSTR_20, "PHUSTR_20" },
    { &s_PHUSTR_21, "PHUSTR_21" }, { &s_PHUSTR_22, "PHUSTR_22" },
    { &s_PHUSTR_23, "PHUSTR_23" }, { &s_PHUSTR_24, "PHUSTR_24" },
    { &s_PHUSTR_25, "PHUSTR_25" }, { &s_PHUSTR_26, "PHUSTR_26" },
    { &s_PHUSTR_27, "PHUSTR_27" }, { &s_PHUSTR_28, "PHUSTR_28" },
    { &s_PHUSTR_29, "PHUSTR_29" }, { &s_PHUSTR_30, "PHUSTR_30" },
    { &s_PHUSTR_31, "PHUSTR_31" }, { &s_PHUSTR_32, "PHUSTR_32" },
    { &s_THUSTR_1, "THUSTR_1" }, { &s_THUSTR_2, "THUSTR_2" },
    { &s_THUSTR_3, "THUSTR_3" }, { &s_THUSTR_4, "THUSTR_4" },
    { &s_THUSTR_5, "THUSTR_5" }, { &s_THUSTR_6, "THUSTR_6" },
    { &s_THUSTR_7, "THUSTR_7" }, { &s_THUSTR_8, "THUSTR_8" },
    { &s_THUSTR_9, "THUSTR_9" }, { &s_THUSTR_10, "THUSTR_10" },
    { &s_THUSTR_11, "THUSTR_11" }, { &s_THUSTR_12, "THUSTR_12" },
    { &s_THUSTR_13, "THUSTR_13" }, { &s_THUSTR_14, "THUSTR_14" },
    { &s_THUSTR_15, "THUSTR_15" }, { &s_THUSTR_16, "THUSTR_16" },
    { &s_THUSTR_17, "THUSTR_17" }, { &s_THUSTR_18, "THUSTR_18" },
    { &s_THUSTR_19, "THUSTR_19" }, { &s_THUSTR_20, "THUSTR_20" },
    { &s_THUSTR_21, "THUSTR_21" }, { &s_THUSTR_22, "THUSTR_22" },
    { &s_THUSTR_23, "THUSTR_23" }, { &s_THUSTR_24, "THUSTR_24" },
    { &s_THUSTR_25, "THUSTR_25" }, { &s_THUSTR_26, "THUSTR_26" },
    { &s_THUSTR_27, "THUSTR_27" }, { &s_THUSTR_28, "THUSTR_28" },
    { &s_THUSTR_29, "THUSTR_29" }, { &s_THUSTR_30, "THUSTR_30" },
    { &s_THUSTR_31, "THUSTR_31" }, { &s_THUSTR_32, "THUSTR_32" },
    { &s_NHUSTR_1, "NHUSTR_1" }, { &s_NHUSTR_2, "NHUSTR_2" },
    { &s_NHUSTR_3, "NHUSTR_3" }, { &s_NHUSTR_4, "NHUSTR_4" },
    { &s_NHUSTR_5, "NHUSTR_5" }, { &s_NHUSTR_6, "NHUSTR_6" },
    { &s_NHUSTR_7, "NHUSTR_7" }, { &s_NHUSTR_8, "NHUSTR_8" }, { &s_NHUSTR_9, "NHUSTR_9" },

    { &s_HUSTR_CHATMACRO1, "HUSTR_CHATMACRO1" }, { &s_HUSTR_CHATMACRO2, "HUSTR_CHATMACRO2" },
    { &s_HUSTR_CHATMACRO3, "HUSTR_CHATMACRO3" }, { &s_HUSTR_CHATMACRO4, "HUSTR_CHATMACRO4" },
    { &s_HUSTR_CHATMACRO5, "HUSTR_CHATMACRO5" }, { &s_HUSTR_CHATMACRO6, "HUSTR_CHATMACRO6" },
    { &s_HUSTR_CHATMACRO7, "HUSTR_CHATMACRO7" }, { &s_HUSTR_CHATMACRO8, "HUSTR_CHATMACRO8" },
    { &s_HUSTR_CHATMACRO9, "HUSTR_CHATMACRO9" }, { &s_HUSTR_CHATMACRO0, "HUSTR_CHATMACRO0" },
    ...
    { &s_HUSTR_PLRBROWN, "HUSTR_PLRBROWN" }, { &s_HUSTR_PLRRED, "HUSTR_PLRRED" },
    ...
    { &s_AMSTR_MARKEDSPOT, "AMSTR_MARKEDSPOT" }, { &s_AMSTR_MARKCLEARED, "AMSTR_MARKCLEARED" },
    { &s_AMSTR_MARKSCLEARED, "AMSTR_MARKSCLEARED" },
    { &s_AMSTR_PATHON, "AMSTR_PATHON" }, { &s_AMSTR_PATHOFF, "AMSTR_PATHOFF" },
    { &s_AMSTR_ROTATEON, "AMSTR_ROTATEON" }, { &s_AMSTR_ROTATEOFF, "AMSTR_ROTATEOFF" },

    { &s_STSTR_MUS, "STSTR_MUS" }, { &s_STSTR_NOMUS, "STSTR_NOMUS" },
    { &s_STSTR_DQDON, "STSTR_DQDON" }, { &s_STSTR_DQDOFF, "STSTR_DQDOFF" },
    { &s_STSTR_KFAADDED, "STSTR_KFAADDED" }, { &s_STSTR_FAADDED, "STSTR_FAADDED" },
    { &s_STSTR_NCON, "STSTR_NCON" }, { &s_STSTR_NCOFF, "STSTR_NCOFF" },
    { &s_STSTR_BEHOLD, "STSTR_BEHOLD" }, { &s_STSTR_BEHOLDX, "STSTR_BEHOLDX" },
    { &s_STSTR_BEHOLDON, "STSTR_BEHOLDON" }, { &s_STSTR_BEHOLDOFF, "STSTR_BEHOLDOFF" },
    { &s_STSTR_BUDDHA, "STSTR_BUDDHA" }, { &s_STSTR_CHOPPERS, "STSTR_CHOPPERS" },
    { &s_STSTR_CLEV, "STSTR_CLEV" }, { &s_STSTR_CLEVSAME, "STSTR_CLEVSAME" },
    { &s_STSTR_NTON, "STSTR_NTON" }, { &s_STSTR_NTOFF, "STSTR_NTOFF" },
    { &s_STSTR_GODON, "STSTR_GODON" }, { &s_STSTR_GODOFF, "STSTR_GODOFF" },
    { &s_STSTR_NMON, "STSTR_NMON" }, { &s_STSTR_NMOFF, "STSTR_NMOFF" },
    { &s_STSTR_PSON, "STSTR_PSON" }, { &s_STSTR_PSOFF, "STSTR_PSOFF" },
    { &s_STSTR_FMON, "STSTR_FMON" }, { &s_STSTR_FMOFF, "STSTR_FMOFF" },
    { &s_STSTR_RION, "STSTR_RION" }, { &s_STSTR_RIOFF, "STSTR_RIOFF" },
    { &s_STSTR_RMON, "STSTR_RMON" }, { &s_STSTR_RMOFF, "STSTR_RMOFF" },
    { &s_STSTR_FON, "STSTR_FON" }, { &s_STSTR_FOFF, "STSTR_FOFF" },
    { &s_STSTR_IAON, "STSTR_IAON" }, { &s_STSTR_IAOFF, "STSTR_IAOFF" },
    { &s_STSTR_RHON, "STSTR_RHON" }, { &s_STSTR_RHOFF, "STSTR_RHOFF" },
    { &s_STSTR_VON, "STSTR_VON" }, { &s_STSTR_VOFF, "STSTR_VOFF" },
    { &s_STSTR_SUCKS, "STSTR_SUCKS" }, { &s_STSTR_KILLS, "STSTR_KILLS" },
    { &s_STSTR_ITEMS, "STSTR_ITEMS" }, { &s_STSTR_SECRETS, "STSTR_SECRETS" },

    { &s_E1TEXT, "E1TEXT" }, { &s_E2TEXT, "E2TEXT" }, { &s_E3TEXT, "E3TEXT" },
    { &s_E4TEXT, "E4TEXT" }, { &s_E5TEXT, "E5TEXT" }, { &s_E6TEXT, "E6TEXT" },
    { &s_C1TEXT, "C1TEXT" }, { &s_C2TEXT, "C2TEXT" }, { &s_C3TEXT, "C3TEXT" },
    { &s_C4TEXT, "C4TEXT" }, { &s_C5TEXT, "C5TEXT" }, { &s_C6TEXT, "C6TEXT" },
    { &s_P1TEXT, "P1TEXT" }, { &s_P2TEXT, "P2TEXT" }, { &s_P3TEXT, "P3TEXT" },
    { &s_P4TEXT, "P4TEXT" }, { &s_P5TEXT, "P5TEXT" }, { &s_P6TEXT, "P6TEXT" },
    { &s_T1TEXT, "T1TEXT" }, { &s_T2TEXT, "T2TEXT" }, { &s_T3TEXT, "T3TEXT" },
    { &s_T4TEXT, "T4TEXT" }, { &s_T5TEXT, "T5TEXT" }, { &s_T6TEXT, "T6TEXT" },
    { &s_N1TEXT, "N1TEXT" },

    { &s_CC_ZOMBIE, "CC_ZOMBIE" }, { &s_CC_SHOTGUN, "CC_SHOTGUN" },
    { &s_CC_HEAVY, "CC_HEAVY" }, { &s_CC_IMP, "CC_IMP" },
    { &s_CC_DEMON, "CC_DEMON" }, { &s_CC_SPECTRE, "CC_SPECTRE" },
    { &s_CC_LOST, "CC_LOST" }, { &s_CC_CACO, "CC_CACO" }, { &s_CC_HELL, "CC_HELL" },
    { &s_CC_BARON, "CC_BARON" }, { &s_CC_ARACH, "CC_ARACH" },
    { &s_CC_PAIN, "CC_PAIN" }, { &s_CC_REVEN, "CC_REVEN" }, { &s_CC_MANCU, "CC_MANCU" },
    { &s_CC_ARCH, "CC_ARCH" }, { &s_CC_SPIDER, "CC_SPIDER" },
    { &s_CC_CYBER, "CC_CYBER" }, { &s_CC_GHOUL, "CC_GHOUL" },
    { &s_CC_BANSHEE, "CC_BANSHEE" }, { &s_CC_SHOCK, "CC_SHOCK" },
    { &s_CC_MIND, "CC_MIND" }, { &s_CC_VASSAGO, "CC_VASSAGO" },
    { &s_CC_TYRANT, "CC_TYRANT" }, { &s_CC_HERO, "CC_HERO" },

    { &s_M_NEWGAME, "M_NEWGAME" }, { &s_M_OPTIONS, "M_OPTIONS" },
    { &s_M_LOADGAME, "M_LOADGAME" }, { &s_M_SAVEGAME, "M_SAVEGAME" },
    { &s_M_QUITGAME, "M_QUITGAME" }, { &s_M_WHICHEPISODE, "M_WHICHEPISODE" },
    { &s_M_EPISODE1, "M_EPISODE1" }, { &s_M_EPISODE2, "M_EPISODE2" },
    { &s_M_EPISODE3, "M_EPISODE3" }, { &s_M_EPISODE4, "M_EPISODE4" },
    { &s_M_EPISODE5, "M_EPISODE5" }, { &s_M_EPISODE6, "M_EPISODE6" },
    { &s_M_EPISODE7, "M_EPISODE7" }, { &s_M_EPISODE8, "M_EPISODE8" },
    { &s_M_EPISODE9, "M_EPISODE9" }, { &s_M_EPISODE10, "M_EPISODE10" },
    { &s_M_WHICHEXPANSION, "M_WHICHEXPANSION" }, { &s_M_EXPANSION1, "M_EXPANSION1" },
    { &s_M_EXPANSION2, "M_EXPANSION2" }, { &s_M_CHOOSESKILLLEVEL, "M_CHOOSESKILLLEVEL" },
    { &s_M_SKILLLEVEL1, "M_SKILLLEVEL1" }, { &s_M_SKILLLEVEL2, "M_SKILLLEVEL2" },
    { &s_M_SKILLLEVEL3, "M_SKILLLEVEL3" }, { &s_M_SKILLLEVEL4, "M_SKILLLEVEL4" },
    { &s_M_SKILLLEVEL5, "M_SKILLLEVEL5" }, { &s_M_ENDGAME, "M_ENDGAME" },
    { &s_M_MESSAGES, "M_MESSAGES" }, { &s_M_ON, "M_ON" }, { &s_M_OFF, "M_OFF" },
    { &s_M_GRAPHICDETAIL, "M_GRAPHICDETAIL" }, { &s_M_HIGH, "M_HIGH" },
    { &s_M_LOW, "M_LOW" }, { &s_M_SCREENSIZE, "M_SCREENSIZE" },
    { &s_M_MOUSESENSITIVITY, "M_MOUSESENSITIVITY" },
    { &s_M_CONTROLLERSENSITIVITY, "M_CONTROLLERSENSITIVITY" },
    { &s_M_SOUNDVOLUME, "M_SOUNDVOLUME" }, { &s_M_CONSOLE, "M_CONSOLE" },
    { &s_M_SFXVOLUME, "M_SFXVOLUME" }, { &s_M_MUSICVOLUME, "M_MUSICVOLUME" },
    { &s_M_PAUSED, "M_PAUSED" },

    { &s_CAPTION_DOOM, "CAPTION_DOOM" }, { &s_CAPTION_DOOM2, "CAPTION_DOOM2" },
    { &s_CAPTION_EPISODE1, "CAPTION_EPISODE1" }, { &s_CAPTION_EPISODE2, "CAPTION_EPISODE2" },
    { &s_CAPTION_EPISODE3, "CAPTION_EPISODE3" }, { &s_CAPTION_EPISODE4, "CAPTION_EPISODE4" },
    { &s_CAPTION_EPISODE5, "CAPTION_EPISODE5" }, { &s_CAPTION_EPISODE6, "CAPTION_EPISODE6" },
    { &s_CAPTION_EPISODE7, "CAPTION_EPISODE7" }, { &s_CAPTION_EPISODE8, "CAPTION_EPISODE8" },
    { &s_CAPTION_EPISODE9, "CAPTION_EPISODE9" }, { &s_CAPTION_EPISODE10, "CAPTION_EPISODE10" },
    { &s_CAPTION_EXPANSION1, "CAPTION_EXPANSION1" },
    { &s_CAPTION_EXPANSION2, "CAPTION_EXPANSION2" },
    { &s_CAPTION_EXPANSION3, "CAPTION_EXPANSION3" },
    { &s_CAPTION_PLUTONIA, "CAPTION_PLUTONIA" }, { &s_CAPTION_TNT, "CAPTION_TNT" },
    { &s_CAPTION_CHEX, "CAPTION_CHEX" }, { &s_CAPTION_CHEX2, "CAPTION_CHEX2" },
    { &s_CAPTION_HACX, "CAPTION_HACX" }, { &s_CAPTION_FREEDOOM1, "CAPTION_FREEDOOM1" },
    { &s_CAPTION_FREEDOOM2, "CAPTION_FREEDOOM2" },
    { &s_CAPTION_FREEDM, "CAPTION_FREEDM" }, { &s_CAPTION_BTSXE1, "CAPTION_BTSXE1" },
    { &s_CAPTION_BTSXE2, "CAPTION_BTSXE2" }, { &s_CAPTION_BTSXE3, "CAPTION_BTSXE3" },
    { &s_CAPTION_REKKR, "CAPTION_REKKR" }, { &s_CAPTION_REKKRSL, "CAPTION_REKKRSL" },
    { &s_CAPTION_ANOMALYREPORT, "CAPTION_ANOMALYREPORT" },
    { &s_CAPTION_ARRIVAL, "CAPTION_ARRIVAL" }, { &s_CAPTION_DBIMPACT, "CAPTION_DBIMPACT" },
    { &s_CAPTION_DEATHLESS, "CAPTION_DEATHLESS" },
    { &s_CAPTION_DOOMZERO, "CAPTION_DOOMZERO" }, { &s_CAPTION_EARTHLESS, "CAPTION_EARTHLESS" },
    { &s_CAPTION_GANYMEDE, "CAPTION_GANYMEDE" }, { &s_CAPTION_GOINGDOWN, "CAPTION_GOINGDOWN" },
    { &s_CAPTION_GOINGDOWNTURBO, "CAPTION_GOINGDOWNTURBO" },
    { &s_CAPTION_HARMONY, "CAPTION_HARMONY" }, { &s_CAPTION_ID1, "CAPTION_ID1" },
    { &s_CAPTION_IDDM1, "CAPTION_IDDM1" }, { &s_CAPTION_KDIKDIZD, "CAPTION_KDIKDIZD" },
    { &s_CAPTION_MASTERLEVELS, "CAPTION_MASTERLEVELS" },
    { &s_CAPTION_NEIS, "CAPTION_NEIS" }, { &s_CAPTION_REVOLUTION, "CAPTION_REVOLUTION" },
    { &s_CAPTION_SCIENTIST, "CAPTION_SCIENTIST" }, { &s_CAPTION_SYRINGE, "CAPTION_SYRINGE" },
    { &s_CAPTION_TTNS, "CAPTION_TTNS" }, { &s_CAPTION_TTP, "CAPTION_TTP" },

    { &s_AUTHOR_ROMERO, "AUTHOR_ROMERO" },

    { &bgflatE1, "BGFLATE1" }, { &bgflatE2, "BGFLATE2" },
    { &bgflatE3, "BGFLATE3" }, { &bgflatE4, "BGFLATE4" }, { &bgflatE5, "BGFLATE5" },
    { &bgflatE6, "BGFLATE6" }, { &bgflat06, "BGFLAT06" }, { &bgflat11, "BGFLAT11" },
    { &bgflat15, "BGFLAT15" }, { &bgflat20, "BGFLAT20" }, { &bgflat30, "BGFLAT30" },
    { &bgflat31, "BGFLAT31" }, { &bgcastcall, "BGCASTCALL" },

    // Ty 04/08/98 - added 5 general purpose startup announcement
    // strings for hacker use.
    { &startup1, "STARTUP1" }, { &startup2, "STARTUP2" },
    { &startup3, "STARTUP3" }, { &startup4, "STARTUP4" }, { &startup5, "STARTUP5" },

    { &savegamename, "SAVEGAMENAME" },

    { &s_BANNER1, "BANNER1" }, { &s_BANNER2, "BANNER2" },
    { &s_BANNER3, "BANNER3" }, { &s_BANNER4, "BANNER4" },
    { &s_BANNER5, "BANNER5" }, { &s_BANNER6, "BANNER6" }, { &s_BANNER7, "BANNER7" },

    { &s_COPYRIGHT1, "COPYRIGHT1" }, { &s_COPYRIGHT2, "COPYRIGHT2" },
    { &s_COPYRIGHT3, "COPYRIGHT3" },

    { &s_OB_SUICIDE, "OB_SUICIDE" }, { &s_OB_FALLING, "OB_FALLING" },
    { &s_OB_CRUSH, "OB_CRUSH" }, { &s_OB_EXIT, "OB_EXIT" },
    { &s_OB_WATER, "OB_WATER" }, { &s_OB_SLIME, "OB_SLIME" },
    { &s_OB_LAVA, "OB_LAVA" }, { &s_OB_BARREL, "OB_BARREL" },
    { &s_OB_SPLASH, "OB_SPLASH" }, { &s_OB_R_SPLASH, "OB_R_SPLASH" },
    { &s_OB_ROCKET, "OB_ROCKET" }, { &s_OB_KILLEDSELF, "OB_KILLEDSELF" },
    { &s_OB_STEALTHBABY, "OB_STEALTHBABY" }, { &s_OB_STEALTHVILE, "OB_STEALTHVILE" },
    { &s_OB_STEALTHBARON, "OB_STEALTHBARON" }, { &s_OB_STEALTHCACO, "OB_STEALTHCACO" },
    { &s_OB_STEALTHCHAINGUY, "OB_STEALTHCHAINGUY" },
    { &s_OB_STEALTHDEMON, "OB_STEALTHDEMON" }, { &s_OB_STEALTHKNIGHT, "OB_STEALTHKNIGHT" },
    { &s_OB_STEALTHIMP, "OB_STEALTHIMP" }, { &s_OB_STEALTHFATSO, "OB_STEALTHFATSO" },
    { &s_OB_STEALTHUNDEAD, "OB_STEALTHUNDEAD" }, { &s_OB_STEALTHSHOTGUY, "OB_STEALTHSHOTGUY" },
    { &s_OB_STEALTHZOMBIE, "OB_STEALTHZOMBIE" }, { &s_OB_UNDEADHIT, "OB_UNDEADHIT" },
    { &s_OB_IMPHIT, "OB_IMPHIT" }, { &s_OB_CACOHIT, "OB_CACOHIT" },
    { &s_OB_DEMONHIT, "OB_DEMONHIT" }, { &s_OB_SPECTREHIT, "OB_SPECTREHIT" },
    { &s_OB_BARONHIT, "OB_BARONHIT" }, { &s_OB_KNIGHTHIT, "OB_KNIGHTHIT" },
    { &s_OB_ZOMBIE, "OB_ZOMBIE" }, { &s_OB_SHOTGUY, "OB_SHOTGUY" },
    { &s_OB_VILE, "OB_VILE" }, { &s_OB_UNDEAD, "OB_UNDEAD" },
    { &s_OB_FATSO, "OB_FATSO" }, { &s_OB_CHAINGUY, "OB_CHAINGUY" },
    { &s_OB_SKULL, "OB_SKULL" }, { &s_OB_IMP, "OB_IMP" }, { &s_OB_CACO, "OB_CACO" },
    { &s_OB_BARON, "OB_BARON" }, { &s_OB_KNIGHT, "OB_KNIGHT" },
    { &s_OB_SPIDER, "OB_SPIDER" }, { &s_OB_BABY, "OB_BABY" },
    { &s_OB_CYBORG, "OB_CYBORG" }, { &s_OB_WOLFSS, "OB_WOLFSS" },
    { &s_OB_MPFIST, "OB_MPFIST" }, { &s_OB_MPCHAINSAW, "OB_MPCHAINSAW" },
    { &s_OB_MPPISTOL, "OB_MPPISTOL" }, { &s_OB_MPSHOTGUN, "OB_MPSHOTGUN" },
    { &s_OB_MPSSHOTGUN, "OB_MPSSHOTGUN" }, { &s_OB_MPCHAINGUN, "OB_MPCHAINGUN" },
    { &s_OB_MPROCKET, "OB_MPROCKET" }, { &s_OB_MPR_SPLASH, "OB_MPR_SPLASH" },
    { &s_OB_MPPLASMARIFLE, "OB_MPPLASMARIFLE" }, { &s_OB_MPBFG_BOOM, "OB_MPBFG_BOOM" },
    { &s_OB_MPBFG_SPLASH, "OB_MPBFG_SPLASH" }, { &s_OB_MPTELEFRAG, "OB_MPTELEFRAG" },
    { &s_OB_RAILGUN, "OB_RAILGUN" }, { &s_OB_MPBFG_MBF, "OB_MPBFG_MBF" },
    { &s_OB_MONTELEFRAG, "OB_MONTELEFRAG" }, { &s_OB_DEFAULT, "OB_DEFAULT" },
    { &s_OB_FRIENDLY1, "OB_FRIENDLY1" }, { &s_OB_FRIENDLY2, "OB_FRIENDLY2" },
    { &s_OB_FRIENDLY3, "OB_FRIENDLY3" }, { &s_OB_FRIENDLY4, "OB_FRIENDLY4" },
    { &s_OB_VOODOO, "OB_VOODOO" }, { &s_OB_MPDEFAULT, "OB_MPDEFAULT" } };
...
char** mapnames[] = { &s_HUSTR_E1M1, &s_HUSTR_E1M2, &s_HUSTR_E1M3, &s_HUSTR_E1M4,
    ...
    &s_HUSTR_E2M6, &s_HUSTR_E2M7, &s_HUSTR_E2M8, &s_HUSTR_E2M9, &s_HUSTR_E3M1,
    &s_HUSTR_E3M2, &s_HUSTR_E3M3, &s_HUSTR_E3M4, &s_HUSTR_E3M5, &s_HUSTR_E3M6,
    &s_HUSTR_E3M7, &s_HUSTR_E3M8, &s_HUSTR_E3M9, &s_HUSTR_E4M1, &s_HUSTR_E4M2,
    &s_HUSTR_E4M3, &s_HUSTR_E4M4, &s_HUSTR_E4M5, &s_HUSTR_E4M6, &s_HUSTR_E4M7,
    &s_HUSTR_E4M8, &s_HUSTR_E4M9, &s_HUSTR_E5M1, &s_HUSTR_E5M2, &s_HUSTR_E5M3,
    &s_HUSTR_E5M4, &s_HUSTR_E5M5, &s_HUSTR_E5M6, &s_HUSTR_E5M7, &s_HUSTR_E5M8,
    &s_HUSTR_E5M9, &s_HUSTR_E6M1, &s_HUSTR_E6M2, &s_HUSTR_E6M3, &s_HUSTR_E6M4,
    &s_HUSTR_E6M5, &s_HUSTR_E6M6, &s_HUSTR_E6M7, &s_HUSTR_E6M8, &s_HUSTR_E6M9 };

// DOOM 2 map names.
char** mapnames2[] = { &s_HUSTR_1, &s_HUSTR_2, &s_HUSTR_3, &s_HUSTR_4,
    &s_HUSTR_5, &s_HUSTR_6, &s_HUSTR_7, &s_HUSTR_8, &s_HUSTR_9, &s_HUSTR_10,
    &s_HUSTR_11, &s_HUSTR_12, &s_HUSTR_13, &s_HUSTR_14, &s_HUSTR_15,
    &s_HUSTR_16, &s_HUSTR_17, &s_HUSTR_18, &s_HUSTR_19, &s_HUSTR_20, &s_HUSTR_21,
    &s_HUSTR_22, &s_HUSTR_23, &s_HUSTR_24, &s_HUSTR_25, &s_HUSTR_26, &s_HUSTR_27,
    &s_HUSTR_28, &s_HUSTR_29, &s_HUSTR_30, &s_HUSTR_31, &s_HUSTR_32 };

// DOOM 2 map names.
char** mapnames2_bfg[] = { &s_HUSTR_1, &s_HUSTR_2, &s_HUSTR_3, &s_HUSTR_4,
    &s_HUSTR_5, &s_HUSTR_6, &s_HUSTR_7, &s_HUSTR_8, &s_HUSTR_9, &s_HUSTR_10,
    &s_HUSTR_11, &s_HUSTR_12, &s_HUSTR_13, &s_HUSTR_14, &s_HUSTR_15, &s_HUSTR_16,
    &s_HUSTR_17, &s_HUSTR_18, &s_HUSTR_19, &s_HUSTR_20, &s_HUSTR_21, &s_HUSTR_22,
    &s_HUSTR_23, &s_HUSTR_24, &s_HUSTR_25, &s_HUSTR_26, &s_HUSTR_27, &s_HUSTR_28,
    &s_HUSTR_29, &s_HUSTR_30, &s_HUSTR_31, &s_HUSTR_32, &s_HUSTR_33 };

// Plutonia WAD map names.
char** mapnamesp[] = { &s_PHUSTR_1, &s_PHUSTR_2, &s_PHUSTR_3, &s_PHUSTR_4,
    &s_PHUSTR_5, &s_PHUSTR_6, &s_PHUSTR_7, &s_PHUSTR_8, &s_PHUSTR_9, &s_PHUSTR_10,
    &s_PHUSTR_11, &s_PHUSTR_12, &s_PHUSTR_13, &s_PHUSTR_14, &s_PHUSTR_15,
    &s_PHUSTR_16, &s_PHUSTR_17, &s_PHUSTR_18, &s_PHUSTR_19, &s_PHUSTR_20, &s_PHUSTR_21,
    &s_PHUSTR_22, &s_PHUSTR_23, &s_PHUSTR_24, &s_PHUSTR_25, &s_PHUSTR_26, &s_PHUSTR_27,
    &s_PHUSTR_28, &s_PHUSTR_29, &s_PHUSTR_30, &s_PHUSTR_31, &s_PHUSTR_32 };

// TNT WAD map names.
char** mapnamest[] = { &s_THUSTR_1, &s_THUSTR_2, &s_THUSTR_3, &s_THUSTR_4,
    &s_THUSTR_5, &s_THUSTR_6, &s_THUSTR_7, &s_THUSTR_8, &s_THUSTR_9, &s_THUSTR_10,
    &s_THUSTR_11, &s_THUSTR_12, &s_THUSTR_13, &s_THUSTR_14, &s_THUSTR_15,
    &s_THUSTR_16, &s_THUSTR_17, &s_THUSTR_18, &s_THUSTR_19, &s_THUSTR_20, &s_THUSTR_21,
    &s_THUSTR_22, &s_THUSTR_23, &s_THUSTR_24, &s_THUSTR_25, &s_THUSTR_26, &s_THUSTR_27,
    &s_THUSTR_28, &s_THUSTR_29, &s_THUSTR_30, &s_THUSTR_31, &s_THUSTR_32 };

// Nerve WAD map names.
char** mapnamesn[] = { &s_NHUSTR_1, &s_NHUSTR_2, &s_NHUSTR_3, &s_NHUSTR_4,
    &s_NHUSTR_5, &s_NHUSTR_6, &s_NHUSTR_7, &s_NHUSTR_8, &s_NHUSTR_9 };
...
static void deh_procThing(DEHFILE* fpin, const char* line);
static void deh_procFrame(DEHFILE* fpin, const char* line);
static void deh_procPointer(DEHFILE* fpin, const char* line);
static void deh_procSounds(DEHFILE* fpin, const char* line);
static void deh_procAmmo(DEHFILE* fpin, const char* line);
static void deh_procWeapon(DEHFILE* fpin, const char* line);
static void deh_procSprite(DEHFILE* fpin, const char* line);
static void deh_procCheat(DEHFILE* fpin, const char* line);
static void deh_procMisc(DEHFILE* fpin, const char* line);
static void deh_procText(DEHFILE* fpin, const char* line);
static void deh_procPars(DEHFILE* fpin, const char* line);
static void deh_procStrings(DEHFILE* fpin, const char* line);
static void deh_procError(DEHFILE* fpin, const char* line);
static void deh_procBexCodePointers(DEHFILE* fpin, const char* line);

// haleyjd: handlers to fully deprecate the DeHackEd text section
static void deh_procBexSounds(DEHFILE* fpin, const char* line);
static void deh_procBexMusic(DEHFILE* fpin, const char* line);
static void deh_procBexSprites(DEHFILE* fpin, const char* line);
...
static const struct deh_flag_s deh_mobjflags2[] = { { "TRANSLUCENT", MF2_TRANSLUCENT },
    ...
    { "TRANSLUCENT_33", MF2_TRANSLUCENT_33 }, { "TRANSLUCENT_50", MF2_TRANSLUCENT_50 },
    { "EXPLODING", MF2_EXPLODING }, { "NOMIRROREDCORPSE", MF2_NOMIRROREDCORPSE },
    { "TRANSLUCENT_BLUE_25", MF2_TRANSLUCENT_BLUE_25 }, { "FLOATBOB", MF2_FLOATBOB },
    { "MOREBLOOD", MF2_MOREBLOOD }, { "PASSMOBJ", MF2_PASSMOBJ },
    { "FOOTCLIP", MF2_FOOTCLIP }, { "NOLIQUIDBOB", MF2_NOLIQUIDBOB },
    { "CASTSHADOW", MF2_CASTSHADOW }, { "DONTMAP", MF2_DONTMAP },
    { "CRUSHABLE", MF2_CRUSHABLE }, { "DECORATION", MF2_DECORATION } };
...
static const struct deh_flag_s deh_stateflags_mbf21[] = { { "SKILL5FAST", STATEF_SKILL5FAST }, // tics halve on nightmare skill
    { NULL, 0 } };
...
static const deh_bexptr deh_bexptrs[] = { { &A_Light0, "A_Light0" },
    { &A_WeaponReady, "A_WeaponReady" }, { &A_Lower, "A_Lower" },
    { &A_Raise, "A_Raise" }, { &A_Punch, "A_Punch" }, { &A_ReFire, "A_ReFire" },
    { &A_FirePistol, "A_FirePistol" }, { &A_Light1, "A_Light1" },
    { &A_FireShotgun, "A_FireShotgun" }, { &A_Light2, "A_Light2" },
    { &A_FireShotgun2, "A_FireShotgun2" }, { &A_CheckReload, "A_CheckReload" },
    { &A_OpenShotgun2, "A_OpenShotgun2" }, { &A_LoadShotgun2, "A_LoadShotgun2" },
    { &A_CloseShotgun2, "A_CloseShotgun2" }, { &A_FireCGun, "A_FireCGun" },
    { &A_GunFlash, "A_GunFlash" }, { &A_FireMissile, "A_FireMissile" },
    { &A_Saw, "A_Saw" }, { &A_FirePlasma, "A_FirePlasma" },
    { &A_BFGSound, "A_BFGSound" }, { &A_FireBFG, "A_FireBFG" },
    { &A_BFGSpray, "A_BFGSpray" }, { &A_Explode, "A_Explode" }, { &A_Pain, "A_Pain" },
    { &A_PlayerScream, "A_PlayerScream" }, { &A_Fall, "A_Fall" },
    { &A_XScream, "A_XScream" }, { &A_Look, "A_Look" }, { &A_Chase, "A_Chase" },
    { &A_FaceTarget, "A_FaceTarget" }, { &A_PosAttack, "A_PosAttack" },
    { &A_Scream, "A_Scream" }, { &A_SPosAttack, "A_SPosAttack" },
    { &A_VileChase, "A_VileChase" }, { &A_VileStart, "A_VileStart" },
    { &A_VileTarget, "A_VileTarget" }, { &A_VileAttack, "A_VileAttack" },
    { &A_StartFire, "A_StartFire" }, { &A_Fire, "A_Fire" },
    { &A_FireCrackle, "A_FireCrackle" }, { &A_Tracer, "A_Tracer" },
    { &A_SkelWhoosh, "A_SkelWhoosh" }, { &A_SkelFist, "A_SkelFist" },
    { &A_SkelMissile, "A_SkelMissile" }, { &A_FatRaise, "A_FatRaise" },
    { &A_FatAttack1, "A_FatAttack1" }, { &A_FatAttack2, "A_FatAttack2" },
    { &A_FatAttack3, "A_FatAttack3" }, { &A_BossDeath, "A_BossDeath" },
    { &A_CPosAttack, "A_CPosAttack" }, { &A_CPosRefire, "A_CPosRefire" },
    { &A_TroopAttack, "A_TroopAttack" }, { &A_SargAttack, "A_SargAttack" },
    { &A_HeadAttack, "A_HeadAttack" }, { &A_BruisAttack, "A_BruisAttack" },
    { &A_SkullAttack, "A_SkullAttack" }, { &A_Metal, "A_Metal" },
    { &A_SpidRefire, "A_SpidRefire" }, { &A_BabyMetal, "A_BabyMetal" },
    { &A_BspiAttack, "A_BspiAttack" }, { &A_Hoof, "A_Hoof" },
    { &A_CyberAttack, "A_CyberAttack" }, { &A_PainAttack, "A_PainAttack" },
    { &A_PainDie, "A_PainDie" }, { &A_KeenDie, "A_KeenDie" },
    { &A_BrainPain, "A_BrainPain" }, { &A_BrainScream, "A_BrainScream" },
    { &A_BrainDie, "A_BrainDie" }, { &A_BrainAwake, "A_BrainAwake" },
    { &A_BrainSpit, "A_BrainSpit" }, { &A_SpawnSound, "A_SpawnSound" },
    { &A_SpawnFly, "A_SpawnFly" }, { &A_BrainExplode, "A_BrainExplode" },
    {
    &A_Detonate,
    "A_Detonate",
    MBF,
    ...
    { &A_HealChase, "A_HealChase", MBF21, 2 }, { &A_SeekTracer, "A_SeekTracer", MBF21, 2 },
    ...
    { &A_AddFlags, "A_AddFlags", MBF21, 2 }, { &A_RemoveFlags, "A_RemoveFlags", MBF21, 2 },
    ...
    { &A_CheckAmmo, "A_CheckAmmo", MBF21, 2 }, { &A_RefireTo, "A_RefireTo", MBF21, 2 },
...
void D_ProcessDehFile(const char* filename, int lumpnum, bool autoloaded)
{
    DEHFILE infile  = { 0 };
    ...
    if(filename)
    {
        if(!(infile.file = FS_OpenFile(filename, FS_READ, FS_TRUE)))
    ...
    else
    {
        ...
        infile.lump        = FS_OpenMem(buf, W_LumpLength(lumpnum));
        infile.file        = NULL;
    ...
    if(infile.lump)
        FS_CloseMem(infile.lump);
    else if(infile.file)
        FS_CloseFile(infile.file);
    ...
    if(infile.lump)
    {
        ...
        if(!infile.resourcewad && !devparm)
            C_Output("%s line%s %s been parsed in the " BOLD(
                     "%s") " lump in the WAD " BOLD("%s") ".",
            temp1, (linecount == 1 ? "" : "s"), (linecount == 1 ? "has" : "have"),
            temp2, filename);
    ...
    else
    {
        ...
        C_Output(
        "%s line%s %s been %s in the " ITALICS("DeHackEd") "%s file " BOLD("%s") ".",
        temp, (linecount == 1 ? "" : "s"), (linecount == 1 ? "has" : "have"),
        (autoloaded ? "automatically parsed" : "parsed"),
        (M_StringEndsWith(filename, "BEX") ?
        " with " ITALICS("BOOM") " extensions" :
        ""),
        filename);
...
static void deh_procBexCodePointers(DEHFILE* fpin, const char* line)
{
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        {
            C_Warning(1, "Invalid BEX codepointer line - must start with \"FRAME\": \"%s\".",
            inbuffer);
            return; // early return
        }
        ...
        if(indexnum < 0)
        {
            C_Warning(1, "Bad pointer number %i.", indexnum);
...
static void deh_procThing(DEHFILE* fpin, const char* line)
{
    ...
    if(*name)
    {
        ...
        if(name[0] == '_')
            memmove(name, name + 1, strlen(name));  // overlapping buffers, use memmove
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        if((string = M_StringCompare(key, "Name")) ||
        (string = M_StringCompare(key, "Name1")))
        {
            M_StringCopy(mobjinfo[indexnum].name1,
            lowercase(trimwhitespace(strval)), sizeof(mobjinfo[0].name1));
            M_snprintf(mobjinfo[indexnum].plural1, sizeof(mobjinfo[0].plural1),
            "%ss", mobjinfo[indexnum].name1);
            namechange = true;
        }
        else if((string = M_StringCompare(key, "Plural")) ||
        (string = M_StringCompare(key, "Plural1")))
            M_StringCopy(mobjinfo[indexnum].plural1,
            lowercase(trimwhitespace(strval)), sizeof(mobjinfo[0].plural1));
        else if((string = M_StringCompare(key, "Name2")))
        {
            M_StringCopy(mobjinfo[indexnum].name2,
            lowercase(trimwhitespace(strval)), sizeof(mobjinfo[0].name2));
            M_snprintf(mobjinfo[indexnum].plural2, sizeof(mobjinfo[0].plural2),
            "%ss", mobjinfo[indexnum].name2);
        }
        else if((string = M_StringCompare(key, "Plural2")))
            M_StringCopy(mobjinfo[indexnum].plural2,
            lowercase(trimwhitespace(strval)), sizeof(mobjinfo[0].plural2));
        else if((string = M_StringCompare(key, "Name3")))
        {
            M_StringCopy(mobjinfo[indexnum].name3,
            lowercase(trimwhitespace(strval)), sizeof(mobjinfo[0].name3));
            M_snprintf(mobjinfo[indexnum].plural3, sizeof(mobjinfo[0].plural3),
            "%ss", mobjinfo[indexnum].name3);
        }
        else if((string = M_StringCompare(key, "Plural3")))
            M_StringCopy(mobjinfo[indexnum].plural3,
            lowercase(trimwhitespace(strval)), sizeof(mobjinfo[0].plural3));
        else if(*name && !namechange)
        {
            M_StringCopy(mobjinfo[indexnum].name3, mobjinfo[indexnum].name1,
            sizeof(mobjinfo[0].name3));
            M_StringCopy(mobjinfo[indexnum].name1,
            lowercase(trimwhitespace(name)), sizeof(mobjinfo[0].name1));
            M_snprintf(mobjinfo[indexnum].plural1, sizeof(mobjinfo[0].plural1),
            "%ss", mobjinfo[indexnum].name1);
            namechange = true;
        }
        ...
        for(ix = 0; ix < DEH_MOBJINFOMAX; ix++)
        {
                ...
                else
                {
                    ...
                    for(value = 0; (strval = strtok(strval, ",+| \t\f\r")); strval = NULL)
                    {
                        ...
                        for(iy = 0; iy < DEH_MOBJFLAGMAX; iy++)
                        {
                            ...
                            if(devparm)
                                C_Output("ORed value 0x%08x %s.",
                                deh_mobjflags[iy].value, strval);

                            if(M_StringCompare(key, "TRANSLUCENT"))
                                boomcompatible = true;
                            else if(M_StringCompare(key, "TOUCHY") ||
                            M_StringCompare(key, "BOUNCES") || M_StringCompare(key, "FRIEND"))
                                mbfcompatible = true;
            ...
            else if(M_StringCompare(key, "Blood"))
            {
                ...
                mobjinfo[indexnum].projectilegroup =
                (value < 0 ? PG_GROUPLESS : value + PG_END);
    ...
    if((mobjinfo[indexnum].flags2 & MF2_FLOATBOB) && !(mobjinfo[indexnum].flags & MF_SPECIAL))
        mobjinfo[indexnum].flags2 &=
        ~(MF2_FLOATBOB | MF2_TRANSLUCENT_33 | MF2_TRANSLUCENT_BLUE_25);
...
static void deh_procFrame(DEHFILE* fpin, const char* line)
{
    ...
    if(indexnum < 0)
    {
        C_Warning(1, "Bad frame number %i.", indexnum);
        return;
    }
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        if(M_StringCompare(key, deh_state[0])) // Sprite number
        {
            ...
            states[indexnum].dehacked = dehacked = true;
        }
...
static void deh_procPointer(DEHFILE* fpin, const char* line)
{
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        if(value < 0)
        {
            C_Warning(1, "Bad pointer number %i.", value);
            return;
        }
        ...
        if(M_StringCompare(key, deh_state[4])) // Codep frame (not set in Frame deh block)
        {
            ...
            for(int i = 0; i < arrlen(deh_bexptrs); i++)
                ...
                {
                    if(devparm)
                        C_Output("BEX [CODEPTR] -> FRAME %i = %s", indexnum,
            ...
            for(int i = 0; i < NUMWEAPONS; i++)
                if(indexnum == weaponinfo[i].ammotype)
                {
                    if(M_StringCompare(key, "Name"))
                        M_StringCopy(weaponinfo[i].ammoname,
                        lowercase(trimwhitespace(strval)), sizeof(weaponinfo[0].ammoname));
                    else if(M_StringCompare(key, "Plural"))
                        M_StringCopy(weaponinfo[i].ammoplural,
                        lowercase(trimwhitespace(strval)),
...
static void deh_procWeapon(DEHFILE* fpin, const char* line)
{
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        else if(M_StringCompare(key, deh_weapon[7])) // MBF21 bits
        {
            ...
            weaponinfo[indexnum].name =
            M_StringDuplicate(lowercase(trimwhitespace(strval)));
...
// Returns: void
//
static void deh_procSprite(DEHFILE* fpin, const char* line) // Not supported
{
    char key[DEH_MAXKEYLEN];
    char inbuffer[DEH_BUFFERMAX];
    int indexnum;
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        if(sscanf(inbuffer, "par %i %i %i", &ep, &level, &partime) != 3)
        {
            ...
            else
            {
                ...
                else
                {
                    if(devparm)
                        C_Output(
                        "Changed par time for MAP%02i from %i to %i seconds.",
        ...
        else
        {
            if(ep < 1 || ep > 9 || level < 1 || level > 9)
                C_Warning(1, "Invalid E" ITALICS("x") "M" ITALICS("y") " values E%iM%i.",
                ep, level);
            else
            {
                if(devparm)
                    C_Output(
                    "Changed par time for E%iM%i from %i to %i seconds.", ep,
                    level, pars[ep][level], partime);
...
static void D_UpdateConsoleCheat(const char* cheat1, const char* cheat2)
{
    for(int i = 0; *consolecmds[i].name; i++)
        if(consolecmds[i].type == CT_CHEAT && M_StringCompare(consolecmds[i].name, cheat1))
        {
            consolecmds[i].name = M_StringDuplicate(cheat2);
            break;
        }
}
...
static void deh_procCheat(DEHFILE* fpin, const char* line)
{
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        if(M_StringCompare(key, deh_cheat[0]))
        {
            ...
            D_UpdateConsoleCheat(cheat_mus.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[1]))
        {
            ...
            D_UpdateConsoleCheat(cheat_choppers.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[2]))
        {
            ...
            D_UpdateConsoleCheat(cheat_god.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[3]))
        {
            ...
            D_UpdateConsoleCheat(cheat_ammo.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[4]))
        {
            ...
            D_UpdateConsoleCheat(cheat_ammonokey.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[5]))
        {
            ...
            D_UpdateConsoleCheat(cheat_noclip.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[6]))
        {
            ...
            D_UpdateConsoleCheat(cheat_commercial_noclip.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[7]))
        {
            ...
            D_UpdateConsoleCheat(cheat_powerup[0].sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[8]))
        {
            ...
            D_UpdateConsoleCheat(cheat_powerup[1].sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[9]))
        {
            ...
            D_UpdateConsoleCheat(cheat_powerup[2].sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[10]))
        {
            ...
            D_UpdateConsoleCheat(cheat_powerup[3].sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[11]))
        {
            ...
            D_UpdateConsoleCheat(cheat_powerup[4].sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[12]))
        {
            ...
            D_UpdateConsoleCheat(cheat_powerup[5].sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[13]))
        {
            ...
            D_UpdateConsoleCheat(cheat_powerup[6].sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[14]))
        {
            ...
            D_UpdateConsoleCheat(cheat_clev.sequence, p);
        ...
        else if(M_StringCompare(key, deh_cheat[15]))
        {
            ...
            D_UpdateConsoleCheat(cheat_mypos.sequence, p);
...
static void deh_procText(DEHFILE* fpin, const char* line)
{
    ...
    if(!found && fromlen < 7 && tolen < 7) // lengths of music and sfx are 6 or shorter
    {
        ...
        if(fromlen != tolen && devparm)
            C_Warning(1, "Mismatched lengths from %i to %i. Using %i.", fromlen,
            tolen, usedlen);
        ...
        {
            if(devparm)
                C_Output("Changing name of sfx from %s to %*s", s_sfx[i].name1,
    ...
    {
        if(devparm)
            C_Output("Checking text area through strings for \"%.12s%s\" from "
                     "= %i to = %i",
...
static void deh_procStrings(DEHFILE* fpin, const char* line)
{
    ...
    128; // maximum string length, bumped 128 at a time as needed
            ...
            *t = '\0'; // cap off the target string
            ...
            if(devparm)
            {
                ...
                else if(lookfor)
                {
                    ...
                    lookfor, (strlen(lookfor) > 12 ? "..." : ""), newstring,
...
//
void deh_procBexSprites(DEHFILE* fpin, const char* line)
{
    char key[DEH_MAXKEYLEN];
    char inbuffer[DEH_BUFFERMAX];
    int value;
...
void deh_procBexSounds(DEHFILE* fpin, const char* line)
{
    char key[DEH_MAXKEYLEN];
    char inbuffer[DEH_BUFFERMAX];
    int value;
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        if(len < 1 || len > 6)
        {
            C_Warning(1, "Bad length for sound name \"%s\"", candidate);
            continue;
        }
        ...
        if((match = dsdh_GetOriginalSFXIndex(key)) >= 0)
        {
            ...
            M_StringCopy(s_sfx[match].name1, candidate, sizeof(s_sfx[0].name1));
        }
    }
}
...
static void deh_procBexMusic(DEHFILE* fpin, const char* line)
{
    ...
    while(!dehfeof(fpin) && *inbuffer && *inbuffer != ' ')
    {
        ...
        while(deh_musicnames[rover])
        {
            ...
            {
                if(devparm)
                    C_Output("Substituting \"%s\" for music \"%s\"", candidate,
...
void D_PostProcessDeh(void)
{
    ...
    for(int i = 0, j; i < numstates; i++)
    {
        ...
        for(j = MAXSTATEARGS - 1; j >= bexptr_match->argcount; j--)
            if(states[i].args[j])
                I_Error("Action %s on state %i expects no more than %i "
                        "non-zero args (%i found).\nCheck your DEHACKED lump.",
    ...
    norockettrails |= (mobjinfo[MT_TRAIL].dehacked || states[S_TRAIL].dehacked ||
    states[S_TRAIL2].dehacked || states[S_TRAIL3].dehacked || states[S_TRAIL4].dehacked);

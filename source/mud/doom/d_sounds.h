...
typedef struct
{
    ...
    // stream struct once registered
    void* stream;

    // music handle once added to mixer
    uint32_t handle;
} musicinfo_t;
...
typedef enum
{
    MUS_INVALID, // Invalid/can't identify
    MUS_MUS,     // DMX/"Doom" format
    MUS_MIDI,    // Standard MIDI
    MUS_MDTS,    // MUDTracker song
    MUS_OGG      // Ogg Vorbis song
} musictype_e;

extern musictype_e musictype;

...
static registryvalue_t uninstall_values[] = {
    ...
    { HKEY_LOCAL_MACHINE, SOFTWARE_KEY "Microsoft\\Windows\\CurrentVersion\\Uninstall\\Ultimate Doom for Windows 95",
    "UninstallString" },

    // DOOM II, CD version (Depths of DOOM trilogy)
    { HKEY_LOCAL_MACHINE, SOFTWARE_KEY "Microsoft\\Windows\\CurrentVersion\\Uninstall\\Doom II for Windows 95",
    "UninstallString" },

    // Final DOOM
    { HKEY_LOCAL_MACHINE, SOFTWARE_KEY "Microsoft\\Windows\\CurrentVersion\\Uninstall\\Final Doom for Windows 95",
    "UninstallString" },

    // Shareware version
    { HKEY_LOCAL_MACHINE, SOFTWARE_KEY "Microsoft\\Windows\\CurrentVersion\\Uninstall\\Doom Shareware for Windows 95",
    "UninstallString" }
};
...
static registryvalue_t root_path_keys[] = {
    ...
    { HKEY_LOCAL_MACHINE, SOFTWARE_KEY "Activision\\DOOM Collector's Edition\\v1.0", "INSTALLPATH" },
...
static const char* root_path_subdirs[] = { ".", "Doom2", "Final Doom",
    "Ultimate Doom", "TNT", "Plutonia", "base\\wads" };

// Location where the Bethesda.net Launcher is installed
static registryvalue_t bethesda_install_location = { HKEY_LOCAL_MACHINE,
    SOFTWARE_KEY "Bethesda Softworks\\Bethesda.net", "installLocation" };
...
static const char* bethesda_install_subdirs[] = {
    ...
    "games\\DOOM 3 BFG Edition\\base\\wads", "games\\DOOM Eternal\\base\\classicwads"
};
...
static const char* steam_install_subdirs[] = {
    ...
    "steamapps\\common\\Doom 2\\base", "steamapps\\common\\Doom 2\\finaldoombase",
    "steamapps\\common\\Doom 2\\masterbase\\doom2", "steamapps\\common\\Ultimate Doom\\rerelease",
    ...
    "steamapps\\common\\Final Doom\\base", "steamapps\\common\\Master Levels of Doom\\doom2",
...
static const iwads_t iwads[] = { { "doom2", doom2 }, { "nerve", pack_nerve },
    { "plutonia", pack_plut }, { "tnt", pack_tnt }, { "doom", doom },
    { "doom1", doom }, { "hacx", doom2 }, { "", 0 } };
...
static bool DirIsFile(char* path, char* filename)
{
    ...
    bool ret              = (!!strchr(path, DIR_SEPARATOR) &&
    M_StringCompare(leafname(path), filenamewithext));
...
{
    char* result = NULL;
    const char *iwadparm = M_GetParm("iwad");

    if(*iwadparm != '\0')
    {
        char *iwadfile = NULL;
        // Search through IWAD directories for an IWAD with the given name.
        if(!M_StringEndsWith(iwadparm, ".wad"))
            iwadfile = M_StringJoin(iwadparm, ".wad", NULL);
        else
            iwadfile = M_StringDuplicate(iwadparm);
...
static char* SaveGameIWADName(void)
{
    ...
    if(game.mode == shareware)
        return "DOOM1";
    if(FREEDOOM)
        return (game.mode == commercial ? "freedoom2" : "freedoom1");

    for(size_t i = 0; iwads[i].name[0]; i++)
        if(game.mission == iwads[i].mission)
            return iwads[i].name;
...
{
    const char *p = M_GetParms("save", "savedir", NULL);

    if(*p != '\0')
    {
        if(p[strlen(p) - 1] != DIR_SEPARATOR)
            savegamefolder = M_StringJoin(p, DIR_SEPARATOR_S, NULL);
        else
            savegamefolder = M_StringDuplicate(p);
    }
    else
    {
        ...
        if(*pwadfile)
        {
            ...
            savegamefolder = M_StringJoin(
            tempsavegamefolder, SaveGameIWADName(), DIR_SEPARATOR_S, NULL);
...
{
    const char *p = M_GetParms("autoload", "autoloaddir", NULL);

    if(*p != '\0')
    {
        if(p[strlen(p) - 1] != DIR_SEPARATOR)
            autoloadfolder = M_StringJoin(p, DIR_SEPARATOR_S, NULL);
        else
            autoloadfolder = M_StringDuplicate(p);
    }
    else
    {
        const char* appdatafolder = M_GetAppDataFolder();
    ...
    M_StringJoin(autoloadfolder, SaveGameIWADName(), DIR_SEPARATOR_S, NULL);
...
{
    const char *p = M_GetParms("shot", "shotdir", NULL);

    if(*p != '\0')
        M_StringCopy(screenshotfolder, p, sizeof(screenshotfolder));
    else
    {
        const char* appdatafolder = M_GetAppDataFolder();
...
{
    // game.mission is set up by the D_FindIWAD() function. But if
    // we specify '-iwad', we have to identify using
    // D_IdentifyIWADByName(). However, if the IWAD does not match
    // any known IWAD name, we may have a dilemma. Try to
    // identify by its contents.
    if(game.mission == none)
    {
    ...
    // Make sure game.mode is set up correctly
...
void D_SetGameDescription(void)
{
    if(game.mission == doom)
    {
        ...
        if(modifiedgame && *pwadfile)
            M_StringCopy(game.description, pwadfile, sizeof(game.description));
        else if(FREEDOOM)
            M_StringCopy(game.description, s_CAPTION_FREEDOOM1, sizeof(game.description));
        else
            M_StringCopy(game.description, s_CAPTION_DOOM, sizeof(game.description));
    }
    else
    {
        ...
        if(modifiedgame && *pwadfile)
            ...
            M_StringCopy(game.description,
            (FREEDM ? s_CAPTION_FREEDM : s_CAPTION_FREEDOOM2), sizeof(game.description));
        else if(game.mission == doom2)
            M_snprintf(game.description, sizeof(game.description), "%s: %s",
            s_CAPTION_DOOM2, s_CAPTION_EXPANSION1);
        else if(game.mission == pack_plut)
            M_StringCopy(game.description, s_CAPTION_PLUTONIA, sizeof(game.description));
        else if(game.mission == pack_tnt)
            M_StringCopy(game.description, s_CAPTION_TNT, sizeof(game.description));
    }
}

...
#include "wad/w_wad.h"
#include "intermission/wi_interlvl.h"
...
#define SP_TIMEX 16
#define SP_TIMEY (V_VANILLAHEIGHT - 25)
...
//
static anim_t epsd0animinfo[] = { { ANIM_ALWAYS, TICRATE / 3, 3, { 224, 104 }, 0 },
...
static anim_t epsd2animinfo[] = { { ANIM_ALWAYS, TICRATE / 3, 3, { 104, 168 }, 0 },
...
static int numanims[] = { arrlen(epsd0animinfo), arrlen(epsd1animinfo), arrlen(epsd2animinfo) };

static anim_t* anims[] = { epsd0animinfo, epsd1animinfo, epsd2animinfo };
...
static bool DrawAnimation(void)
{
    ...
    array_foreach(animstate, animation->states) V_DrawMenuPatch(animstate->xpos,
    animstate->ypos, W_CacheLumpName(animstate->frames[animstate->frameindex].imagelump),
...
static bool SetupAnimation(void)
{
    ...
    if(animation->interlevelexiting)
        animation->exitingstates =
        SetupAnimationStates(animation->interlevelexiting->layers, false);

    if(animation->interlevelentering)
        animation->enteringstates =
        SetupAnimationStates(animation->interlevelentering->layers, true);
...
{
    memcpy(v_screens[0], v_screens[1], video.screen_area);
}
...
static void WI_DrawLF(void)
{
    ...
    if(titlepatch > 0)
    {
        ...
        if(height < V_VANILLAHEIGHT)
            V_DrawMenuPatch((V_VANILLAWIDTH - SHORT(patch->width)) / 2 + 1, y + 1,
            patch, false, video.screen_width);
        else
    ...
    else
    {
        ...
        if(W_GetNumLumps(name) > 1)
        {
            ...
            if(height < V_VANILLAHEIGHT)
                ...
                y + 1, patch, false, video.screen_width);
            else
                V_DrawPagePatch(0, patch);
    ...
    if(y >= V_VANILLAHEIGHT)
        y = WI_TITLEY + 24;

    // draw "Finished!"
    if(SHORT(wif->height) < V_VANILLAHEIGHT)
        V_DrawMenuPatch((V_VANILLAWIDTH - SHORT(wif->width)) / 2 + 1, y + 1, wif,
        false, video.screen_width);
    else
...
static void WI_DrawEL(void)
{
    ...
    // draw "Entering"
    if(SHORT(wienter->height) < V_VANILLAHEIGHT)
        V_DrawMenuPatch((V_VANILLAWIDTH - SHORT(wienter->width)) / 2 + 1, y + 1,
        wienter, false, video.screen_width);
    else
...
static void WI_DrawOnLnode(int n, patch_t* c[])
{
    ...
    do
    {
        ...
        if(left >= 0 && right < V_VANILLAWIDTH && top >= 0 && bottom < V_VANILLAHEIGHT)
            fits = true;
        else
            i++;
    } while(!fits && i != 2 && c[i]);
    ...
    if(fits && i < 2)
    {
        if(c[i] == splat[0] || c[i] == splat[1])
            V_DrawTranslucentNoGreenPatch(
            lnodes[wbs->epsd][n].x, lnodes[wbs->epsd][n].y, c[i]);
        else if(c[i] == yah[0] || c[i] == yah[1])
            V_DrawNoGreenPatchWithShadow(
            lnodes[wbs->epsd][n].x + 1, lnodes[wbs->epsd][n].y + 1, c[i]);
        else
...
static int WI_DrawNum(int x, int y, int n, int digits)
{
    ...
    while(digits--)
    {
        x -= fontwidth;
        x += 2 * (n % 10 == 1);
        V_DrawMenuPatch(x + 1, y + 1, num[n % 10], false, video.screen_width);
        x -= 2 * (n % 10 == 1);
        n /= 10;
    }
...
static void WI_DrawPercent(int x, int y, int p)
{
    ...
    V_DrawMenuPatch(x + 1, y + 1, wipcnt, false, video.screen_width);
    WI_DrawNum(x, y, p, -1);
}
...
//
static void WI_DrawTime(int x, int y, int tics, bool end, bool sucks)
{
    if(tics < 0)
        return;

    if(sucks && sucktime && tics >= sucktime * 61 * 59 * TICRATE)
        V_DrawMenuPatch(SP_TIMEX + SHORT(witime->width) + 8, y + 1, wisucks, false, video.screen_width);
    else
    {
        int div = 1;
        ...
        do
        {
            ...
            if((div *= 60) == 60 || tics / div)
                V_DrawMenuPatch(x + 1, y + 1, wicolon, false, video.screen_width);
        } while(tics / div);
...
static void WI_InitNoState(void)
{
    ...
    cnt             = (game.mode == commercial || animation ? TICRATE : 10);
...
static void WI_InitShowNextLoc(void)
{
    ...
    if(game.mode != commercial && game.map == 8)
    {
        G_WorldDone();
        return;
    }
...
{
    if(P_GetMapEndGame(game.episode, game.map) ||
    P_GetMapEndCast(game.episode, game.map))
        return;
    ...
    if(game.mission == pack_nerve && wbs->last == 7)
        return;

    // draws which level you are entering...
    if(game.mode != commercial || wbs->next != 30)
        WI_DrawEL();
}
...
static void WI_InitStats(void)
{
    ...
    int tics               = game.stats.totaltime / TICRATE;
    const int hours        = tics / 3600;
    const int minutes      = ((tics %= 3600)) / 60;
    const int seconds      = tics % 60;
    const int milliseconds = (game.stats.totaltime * 1000 / TICRATE) % 1000;
    ...
    C_TabbedOutput(tabs, "Kills\t%s of %s (%i%%)", temp1, temp2,
    (wbs->skills * 100) / wbs->maxkills);
    free(temp1);
    free(temp2);

    temp1 = commify(wbs->sitems);
    temp2 = commify(wbs->maxitems);
    C_TabbedOutput(tabs, "Items\t%s of %s (%i%%)", temp1, temp2,
    (wbs->sitems * 100) / wbs->maxitems);
    free(temp1);
    free(temp2);
    ...
    if(game.stats.secrets)
    {
        ...
        C_TabbedOutput(tabs, "Secrets\t%s of %s (%i%%)", temp1, temp2,
        (wbs->ssecret * 100) / wbs->maxsecret);
        free(temp1);
        free(temp2);
    }
    ...
    if(shours || parhours || hours)
    {
        if(sucktime && wbs->stime / TICRATE > sucktime * 61 * 59)
            ...
            C_TabbedOutput(tabs,
            "Time\t" MONOSPACED("%02i") ":" MONOSPACED("%02i") ":" MONOSPACED("%02i"),
        ...
        if(game.stats.totaltime > game.stats.maptime)
            C_TabbedOutput(tabs,
            "Total time\t" MONOSPACED("%02i") ":" MONOSPACED("%02i") ":" MONOSPACED("%02i"),
...
static void WI_UpdateStats(void)
{
    ...
    else if(sp_state == 10)
    {
        if(acceleratestage)
        {
            ...
            if(NextLocAnimation() || game.mode != commercial)
                WI_InitShowNextLoc();
            else
                WI_InitNoState();
        }
...
static void WI_DrawStats(void)
{
    ...
    V_DrawMenuPatch(SP_STATSX, SP_STATSY + 1, wiostk, false, video.screen_width);
    WI_DrawPercent(V_VANILLAWIDTH - SP_STATSX - 14, SP_STATSY, cnt_kills);

    V_DrawMenuPatch(SP_STATSX, SP_STATSY + lh + 1, wiosti, false, video.screen_width);
    WI_DrawPercent(V_VANILLAWIDTH - SP_STATSX - 14, SP_STATSY + lh, cnt_items);
    ...
    if(game.stats.secrets)
    {
        if(!WISCRT2)
            ...
            V_DrawMenuPatch(SP_STATSX, SP_STATSY + 2 * lh + 1, wiscrt2, false, video.screen_width);

        WI_DrawPercent(V_VANILLAWIDTH - SP_STATSX - 14, SP_STATSY + 2 * lh, cnt_secret);
    }

    V_DrawMenuPatch(SP_TIMEX + 1, SP_TIMEY + 1, witime, false, video.screen_width);
    WI_DrawTime(V_VANILLAWIDTH / 2 - SP_TIMEX * 2 + 18, SP_TIMEY, cnt_time,
    (cnt_time == wbs->stime), (wbs->stime > wbs->partime));

    if(wbs->partime)
    {
        V_DrawMenuPatch(
        V_VANILLAWIDTH / 2 + SP_TIMEX + (SP_TIMEX - FREEDOOM * 17 + 3) - 18,
        SP_TIMEY + 1, wipar, false, video.screen_width);
        WI_DrawTime(V_VANILLAWIDTH - SP_TIMEX - 2 - FREEDOOM * 17,
        SP_TIMEY, cnt_par, (cnt_par == wbs->partime), false);
    }
}

void WI_CheckForAccelerate(void)
{
    if((viewplayer->cmd.buttons & BT_ATTACK) || game.keydown[KEY_ENTER])
    {
    ...
    if((viewplayer->cmd.buttons & BT_USE) || game.keydown[KEY_SPACE])
    {
...
void WI_Ticker(void)
{
    ...
    {
        // intermission music
        S_ChangeMusic((game.mode == commercial ? mus_dm2int : mus_inter), true, false, false);
        S_StopSounds();
    }
...
static void WI_LoadData(void)
{
    ...
    else if(game.mode == commercial || (game.mode == retail && wbs->epsd == 3))
...
static void WI_InitVariables(wbstartstruct_t* wbstartstruct)
{
    ...
    exitpic  = P_GetMapExitPic(game.episode, game.map);
    ...
    if(game.mode != retail && wbs->epsd > 2)
        wbs->epsd -= 3;
...
{
    char* enteranim = P_GetMapEnterAnim(game.episode, game.map);
    char* exitanim  = P_GetMapExitAnim(game.episode, game.map);

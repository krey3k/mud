...
{
    char* intertext   = P_GetInterText(game.episode, game.map);
    char* intersecret = P_GetInterSecretText(game.episode, game.map);
    ...
    {
        char* interbackdrop = P_GetInterBackrop(game.episode, game.map);
        int mus             = P_GetInterMusic(game.episode, game.map);
        ...
        if(mus > 0)
            ...
            S_ChangeMusic((game.mode == commercial ? mus_read_m : mus_victor),
            true, false, false);
    }
    else if(P_GetMapEndCast(game.episode, game.map))
    {
        game.action = ga_nothing;
        game.state  = GS_FINALE;
        F_StartCast();
        return;
    }
    else
        ...
        case commercial:
            ...
            case 6:
                finaleflat = bgflat06;
                finaletext = (game.mission == pack_tnt ?
                s_T1TEXT :
                (game.mission == pack_plut ? s_P1TEXT : s_C1TEXT));
                break;
            ...
            case 11:
                ...
                finaletext = (game.mission == pack_tnt ?
                s_T2TEXT :
                (game.mission == pack_plut ? s_P2TEXT : s_C2TEXT));
                break;
            ...
            case 20:
                ...
                finaletext = (game.mission == pack_tnt ?
                s_T3TEXT :
                (game.mission == pack_plut ? s_P3TEXT : s_C3TEXT));
                break;
            ...
            case 30:
                ...
                finaletext = (game.mission == pack_tnt ?
                s_T4TEXT :
                (game.mission == pack_plut ? s_P4TEXT : s_C4TEXT));
                break;
            ...
            case 15:
                ...
                finaletext = (game.mission == pack_tnt ?
                s_T5TEXT :
                (game.mission == pack_plut ? s_P5TEXT : s_C5TEXT));
                break;
            ...
            case 31:
                ...
                finaletext = (game.mission == pack_tnt ?
                s_T6TEXT :
                (game.mission == pack_plut ? s_P6TEXT : s_C6TEXT));
                break;
            }
...
static fixed_t TextSpeed(void)
{
    ...
    return (midstage ?
    NEWTEXTSPEED :
    ((midstage = acceleratestage) ? (acceleratestage = false), NEWTEXTSPEED : TEXTSPEED));
}
...
void F_Ticker(void)
{
    ...
    else if(finalestage == F_STAGE_TEXT)
    {
        ...
        {
            if(P_GetMapEndPic(game.episode, game.map) > 0)
            {
                if(P_GetMapEndCast(game.episode, game.map))
                    F_StartCast();
                else
                {
                    finalecount   = 0;
                    ...
                    if(P_GetMapEndBunny(game.episode, game.map) ||
                    (game.mode != commercial && game.episode == 3))
                        S_StartMusic(mus_bunny);
                }
            }
            else if(P_GetMapNext(game.episode, game.map))
                game.action = ga_worlddone;
            ...
            else if(game.map == 30 || (game.mission == pack_nerve && game.map == 8))
                F_StartCast();
            else
                game.action = ga_worlddone;
        }
...
static void F_TextWrite(void)
{
    ...
    else if(W_CheckNumForName(finaleflat) >= 0)
    {
        ...
        byte* dest         = v_screens[0];

        for(int y = 0; y < video.screen_height; y++)
    ...
    for(int count = MAX(0, FixedDiv((finalecount - 10) * FRACUNIT, TextSpeed()) >> FRACBITS);
    count; count--)
    {
        char letter = *ch++;
        int c;
        ...
        if(cx > V_VANILLAWIDTH - 12)
            continue;

        if(STCFNxxx)
        {
            if(cy + SHORT(hu_font[c]->height) > V_VANILLAHEIGHT)
                break;

            V_DrawMenuPatch(cx + 1, cy + 1, hu_font[c], false, video.screen_width);
...
static castinfo_t castorder[CASTNUMMAX] = { { CC_ZOMBIE, &s_CC_ZOMBIE, MT_POSSESSED, 2 },
    { CC_SHOTGUN, &s_CC_SHOTGUN, MT_SHOTGUY, 2 },
    { CC_HEAVY, &s_CC_HEAVY, MT_CHAINGUY, 2 },
    { CC_IMP, &s_CC_IMP, MT_TROOP, 2 }, { CC_DEMON, &s_CC_DEMON, MT_SERGEANT, 2 },
    { CC_SPECTRE, &s_CC_SPECTRE, MT_SHADOWS, 2 },
    { CC_LOST, &s_CC_LOST, MT_SKULL, 0 }, { CC_CACO, &s_CC_CACO, MT_HEAD, 4 },
    { CC_HELL, &s_CC_HELL, MT_KNIGHT, 4 }, { CC_BARON, &s_CC_BARON, MT_BRUISER, 4 },
    { CC_ARACH, &s_CC_ARACH, MT_BABY, 0 }, { CC_PAIN, &s_CC_PAIN, MT_PAIN, 0 },
    { CC_REVEN, &s_CC_REVEN, MT_UNDEAD, 4 }, { CC_MANCU, &s_CC_MANCU, MT_FATSO, 0 },
    { CC_ARCH, &s_CC_ARCH, MT_VILE, 4 }, { CC_SPIDER, &s_CC_SPIDER, MT_SPIDER, 8 },
    { CC_CYBER, &s_CC_CYBER, MT_CYBORG, 4 }, { "", &s_CC_GHOUL, MT_GHOUL, 0 },
    { "", &s_CC_BANSHEE, MT_BANSHEE, 0 }, { "", &s_CC_SHOCK, MT_SHOCKTROOPER, 0 },
    { "", &s_CC_MIND, MT_MINDWEAVER, 0 }, { "", &s_CC_VASSAGO, MT_VASSAGO, 0 },
    { "", &s_CC_TYRANT, MT_TYRANT, 0 }, { CC_HERO, &s_CC_HERO, MT_PLAYER, 0 } };
...
static void F_CastTicker(void)
{
    ...
    if(caststate->tics == -1 || caststate->nextstate == S_NULL)
    {
        ...
        if(mobjinfo[castorder[castnum].type].seesound)
            S_StartSound(
            NULL, F_RandomizeSound(mobjinfo[castorder[castnum].type].seesound));
    ...
    else
    {
        ...
        st =
        (caststate->action == &A_RandomJump && (M_BigRandom() & 255) < caststate->misc2 ?
        caststate->misc1 :
        caststate->nextstate);
        caststate = &states[st];
        castframes++;
    ...
    if(castattacking &&
    (castframes == 24 || caststate == &states[mobjinfo[castorder[castnum].type].seestate]))
    {
    stopattack:
        castattacking = false;
        castframes    = 0;
    ...
    if(casttics == -1)
    {
        ...
        {
            caststate =
            &states[((M_BigRandom() & 255) < caststate->misc2 ? caststate->misc1 :
                                                                caststate->nextstate)];
            casttics = caststate->tics;
        }
...
static bool F_CastResponder(const event_t* ev)
{
    ...
    ev->data1 != KEY_RIGHTARROW && ev->data1 != KEY_ENTER && ev->data1 != KEY_SPACE &&
    ...
    if(ev->type == ev_controller && !(controllerbuttons & controllerfire) &&
    !(controllerbuttons & controlleruse))
        return false;
...
static void F_CastPrint(const char* text)
{
    ...
    while(ch)
    {
        ...
        V_DrawMenuPatch(cx + 1, 181, hu_font[c], false, video.screen_width);
        cx += SHORT(hu_font[c]->width);
    }
}
...
static void F_CastDrawer(void)
{
    ...
    int rot = 0;
    patch_t* patch;
    int y                 = V_VANILLAHEIGHT - 30;
    const mobjtype_t type = castorder[castnum].type;
    
    patch = (FREEDOOM ? W_CacheLastLumpName(bgcastcall) :
                                W_CacheLumpName(bgcastcall));
    V_DrawPagePatch(0, patch);

    if(type == MT_PLAYER &&
    M_StringCompare(castorder[castnum].name, *castorder[castnum].dehackedname))
    ...
    patch->topoffset =
    (r_fixspriteoffsets ? newspritetopoffset[lump] : spritetopoffset[lump]) >> FRACBITS;
    ...
    if((sprframe->flip & (1 << rot)) || castdeathflip)
    {
        patch->leftoffset =
        (spritewidth[lump] -
        (r_fixspriteoffsets ? newspriteoffset[lump] : spriteoffset[lump])) >>
        FRACBITS;
        ...
        if(r_shadows)
        {
            ...
            {
                if(type == MT_SHADOWS)
                    V_DrawFlippedSpectreShadowPatch(V_VANILLAWIDTH / 2,
                    V_VANILLAHEIGHT - 32 - castorder[castnum].shadowoffset, patch);
                else
                    V_DrawFlippedShadowPatch(V_VANILLAWIDTH / 2,
                    V_VANILLAHEIGHT - 32 - castorder[castnum].shadowoffset, patch);
            }
            else
                V_DrawFlippedSolidShadowPatch(V_VANILLAWIDTH / 2,
                V_VANILLAHEIGHT - 32 - castorder[castnum].shadowoffset, patch);
        }

        if(r_sprites_translucency && (type == MT_SKULL || (type == MT_PAIN && castdeath)))
            V_DrawFlippedTranslucentRedPatch(V_VANILLAWIDTH / 2, y, patch);
        else if(type == MT_SHADOWS)
            V_DrawFlippedFuzzPatch(V_VANILLAWIDTH / 2, y, patch);
        else
            V_DrawFlippedPatch(V_VANILLAWIDTH / 2, y, patch);
    }
    else
    {
        patch->leftoffset =
        (r_fixspriteoffsets ? newspriteoffset[lump] : spriteoffset[lump]) >> FRACBITS;
        ...
        if(r_shadows)
        {
            ...
            {
                if(type == MT_SHADOWS)
                    V_DrawSpectreShadowPatch(V_VANILLAWIDTH / 2,
                    V_VANILLAHEIGHT - 32 - castorder[castnum].shadowoffset, patch);
                else
                    V_DrawShadowPatch(V_VANILLAWIDTH / 2,
                    V_VANILLAHEIGHT - 32 - castorder[castnum].shadowoffset, patch);
            }
            else
                V_DrawSolidShadowPatch(V_VANILLAWIDTH / 2,
                V_VANILLAHEIGHT - 32 - castorder[castnum].shadowoffset, patch);
        }

        if(r_sprites_translucency && (type == MT_SKULL || (type == MT_PAIN && castdeath)))
            V_DrawTranslucentRedPatch(V_VANILLAWIDTH / 2, y, patch);
        else if(type == MT_SHADOWS)
            V_DrawFuzzPatch(V_VANILLAWIDTH / 2, y, patch);
        else
            V_DrawPatch(V_VANILLAWIDTH / 2, y, 0, patch);
    }
}
...
{
    const int scrolled = BETWEEN(0, V_VANILLAWIDTH - (finalecount - 230) / 2, V_VANILLAWIDTH);
    patch_t* p1        = (FREEDOOM ?
           W_CacheLastLumpName("PFUB2") :
           W_CacheLumpName("PFUB2"));
    patch_t* p2        = (FREEDOOM ?
           W_CacheLastLumpName("PFUB1") :
           W_CacheLumpName("PFUB1"));
    const int p1offset = (V_VANILLAWIDTH - SHORT(p1->width)) / 2;
    const int p2offset =
    V_VANILLAWIDTH + (SHORT(p2->width) == V_VANILLAWIDTH ? -p1offset : p1offset);
    const int pillarwidth =
    MAX(0, (video.screen_width - (SHORT(p1->width) << FRACBITS) / DXI) / 2);

    if(pillarwidth && video.screen_width != V_NONWIDEWIDTH)
        memset(v_screens[0], FindDominantEdgeColor(p1), video.screen_area);

    for(int x = pillarwidth; x < video.screen_width - pillarwidth; x++)
    {
        const int x2 = ((x * DXI) >> FRACBITS) - video.widescreen_delta + scrolled;
    ...
    if(finalecount >= 1130)
    {
        ...
        if(finalecount < 1180)
        {
            ...
            V_DrawMenuPatch((V_VANILLAWIDTH - 104) / 2 + 1, (V_VANILLAHEIGHT - 64) / 2 + 1,
            (FREEDOOM ? W_CacheLastLumpName("END0") : W_CacheLumpName("END0")),
            false, video.screen_width);
            laststage = 0;
        }
        else
        {
            ...
            V_DrawMenuPatch((V_VANILLAWIDTH - 104) / 2 + 1, (V_VANILLAHEIGHT - 64) / 2 + 1,
            (FREEDOOM ? W_CacheLastLumpName(name) : W_CacheLumpName(name)),
            false, video.screen_width);
        }
    }
}
...
{
    const int lumpnum = P_GetMapEndPic(game.episode, game.map);
    ...
    else if(P_GetMapEndBunny(game.episode, game.map) || game.episode == 3)
        F_BunnyScroll();
    else
    {
        patch_t* lump;
        ...
        {
        case 1:
            lump = (game.mode == retail || game.mode == commercial ?
            creditlump :
        ...
        case 2:
            lump = (FREEDOOM ?
            W_CacheLastLumpName("VICTORY2") :
            W_CacheLumpName("VICTORY2"));
            break;

        case 4:
            lump = (FREEDOOM ?
            W_CacheLastLumpName("ENDPIC") :
            W_CacheLumpName("ENDPIC"));
            break;

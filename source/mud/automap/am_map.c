...
#define CYMTOF(y) (V_MAPHEIGHT - MTOF((y) - m_y))
...
static void AM_ActivateNewScale(void)
{
    ...
    m_y += m_h / 2;
    m_w = FTOM(V_MAPWIDTH);
    m_h = FTOM(V_MAPHEIGHT);
    m_x -= m_w / 2;
    m_y -= m_h / 2;
    putbigwalldot =
    (scale_mtof >= USEBIGDOTS || r_detail == r_detail_low ? &PUTBIGDOT : &PUTDOT);
}
...
static void AM_RestoreScaleAndLoc(void)
{
    ...
    scale_mtof = FixedDiv(V_MAPWIDTH << FRACBITS, m_w);
...
static void AM_FindMinMaxBoundaries(void)
{
    ...
    a = FixedDiv(V_MAPWIDTH << FRACBITS, (max_x >>= FRACTOMAPBITS) - (min_x >>= FRACTOMAPBITS));
    b = FixedDiv(V_MAPHEIGHT << FRACBITS, (max_y >>= FRACTOMAPBITS) - (min_y >>= FRACTOMAPBITS));

    min_scale_mtof = MIN(scale_mtof, MIN(a, b));
    max_scale_mtof = FixedDiv(V_MAPHEIGHT << FRACBITS, PLAYERRADIUS * 3);
}
...
{
    V_MAPWIDTH  = video.screen_width;
    V_MAPHEIGHT = video.screen_height - V_SBARHEIGHT * (screensize < r_screensize_max);
    V_MAPAREA   = V_MAPWIDTH * V_MAPHEIGHT;
    V_MAPBOTTOM = V_MAPWIDTH * (V_MAPHEIGHT - 1);
...
{
    if(lastlevel != game.map || lastepisode != game.episode || !mainwindow)
    {
        AM_LevelInit();
        lastlevel   = game.map;
...
void AM_AddMark(void)
{
    ...
    M_snprintf(message, sizeof(message), s_AMSTR_MARKEDSPOT, ++nummarks,
    x >> MAPBITS, y >> MAPBITS);
    C_Output(message);
    ...
    if(!menuactive && !paused)
    {
        ...
        if(!automapactive)
        {
            if((ev->type == ev_keydown &&
               (ev->data1 == keyboardautomap || ev->data1 == keyboardautomap2) &&
               keydown != keyboardautomap && (!keyboardautomap2 || keydown != keyboardautomap2)
#ifdef MUD_SOKOL_PORT
               && !(modstate & KMOD_ALT))
#else
               && true)
#endif
            || (ev->type == ev_mouse && mouseautomap >= 0 && (ev->data1 & mouseautomap)) ||
            (ev->type == ev_controller && (controllerbuttons & controllerautomap) && !backbuttondown))
            {
                keydown        = ev->data1;
                backbuttondown = true;
                AM_Start(true);
        ...
        else
        {
            ...
            if(ev->type == ev_keydown)
            {
                ...
                // pan right
                if(key == keyboardright || key == keyboardright2 ||
                ...
                // pan left
                else if(key == keyboardleft || key == keyboardleft2 ||
                ...
                // zoom out
                else if(key == keyboardzoomout && !movement && (keyboardzoomout != '-'))
                {
                    keydown = key;
                    AM_ToggleZoomOut();
                }
                else if(key == keyboardzoomout2 && !movement && (keyboardzoomout2 != '-'))
                {
                    keydown = key;
                    AM_ToggleZoomOut();
                }
                ...
                // zoom in
                else if(key == keyboardzoomin && !movement && (keyboardzoomin != '='))
                {
                    keydown = key;
                    AM_ToggleZoomIn();
                }
                else if(key == keyboardzoomin2 && !movement && (keyboardzoomin2 != '='))
                {
                    keydown = key;
                    AM_ToggleZoomIn();
                }
                ...
                // leave automap
#ifdef MUD_SOKOL_PORT
                else if((key == keyboardautomap || key == keyboardautomap2) &&
                !(modstate & KMOD_ALT)
#else
                else if((key == keyboardautomap || key == keyboardautomap2) && true
#endif
                ...
                else if((key == keyboardmaxzoom || key == keyboardmaxzoom2) && !idclev && !idmus)
                {
                    ...
                    (!keyboardmaxzoom2 || keydown != keyboardmaxzoom2))
                    {
                        keydown = key;
                        AM_ToggleMaxZoom();
                    }
                ...
                else if(key == keyboardfollowmode || key == keyboardfollowmode2)
                {
                    ...
                    (!keyboardfollowmode2 || keydown != keyboardfollowmode2))
                    {
                        keydown = key;
            ...
            else if(ev->type == ev_keyup)
            {
                ...
                if((key == keyboardzoomout || key == keyboardzoomout2 ||
                   key == keyboardzoomin || key == keyboardzoomin2) &&
                !movement)
                {
                    mtof_zoommul = FRACUNIT;
                    ftom_zoommul = FRACUNIT;
                ...
                {
                    if(key == keyboardleft || key == keyboardleft2 ||
                    key == keyboardstrafeleft || key == keyboardstrafeleft2)
                    {
                        speedtoggle = AM_GetSpeedToggle();
                    ...
                    else if(key == keyboardright || key == keyboardright2 ||
                    key == keyboardstraferight || key == keyboardstraferight2)
                    {
                        speedtoggle = AM_GetSpeedToggle();

                        if(keystate(keyboardleft) || keystate(keyboardstrafeleft) ||
            ...
            if((viewplayer->cheats & CF_MYPOS) && !am_followmode &&
...
static void AM_RotatePoint(mpoint_t* point)
{
    ...
    temp = FixedMul(point->x, am_frame.cos) - FixedMul(point->y, am_frame.sin) +
    am_frame.center.x;
    point->y = FixedMul(point->x, am_frame.sin) +
    FixedMul(point->y, am_frame.cos) + am_frame.center.y;
    point->x = temp;
}
...
{
    memset(v_mapscreen, backcolor, V_MAPAREA);
}
...
static bool AM_ClipMline(int* x0, int* y0, int* x1, int* y1)
{
    ...
    *x1 = CXMTOF(*x1);
    ...
    if(*x0 < -1)
        outcode1 = LEFT;
    else if(*x0 >= V_MAPWIDTH)
        outcode1 = RIGHT;

    if(*x1 < -1)
        outcode2 = LEFT;
    else if(*x1 >= V_MAPWIDTH)
        outcode2 = RIGHT;
    ...
    *y1 = CYMTOF(*y1);
    ...
    if(*y0 < -1)
        outcode1 |= TOP;
    else if(*y0 >= V_MAPHEIGHT)
        outcode1 |= BOTTOM;

    if(*y1 < -1)
        outcode2 |= TOP;
    else if(*y1 >= V_MAPHEIGHT)
        outcode2 |= BOTTOM;
...
{
    if(x >= 0 && x < V_MAPWIDTH && y >= 0 && y < V_MAPAREA)
    {
        byte* dot = v_mapscreen + y + x;
...
{
    if(x >= 0 && x < V_MAPWIDTH - 1 && y >= 0 && y < V_MAPAREA)
    {
        byte* dot = v_mapscreen + y + x;

        *dot                  = *color;
        *(dot + 1)            = *color;
        *(dot + V_MAPWIDTH)     = *color;
        *(dot + V_MAPWIDTH + 1) = *color;
    }
}
...
//
static void
AM_DrawFline(int x0, int y0, int x1, int y1, const byte* color, void (*putdot)(int, int, const byte*))
{
    if(AM_ClipMline(&x0, &y0, &x1, &y1))
    {
        ...
        if(!dy)
        {
            ...
            x0 = BETWEEN(-1, x0, V_MAPWIDTH - 1);
            x1 = BETWEEN(-1, x1, V_MAPWIDTH - 1);

            putdot(x0, (y0 *= V_MAPWIDTH), color);
        ...
        {
            // vertical line
            const int sy = SIGN(dy) * V_MAPWIDTH;

            y0 = BETWEEN(-V_MAPWIDTH, y0 * V_MAPWIDTH, V_MAPBOTTOM);
            y1 = BETWEEN(-V_MAPWIDTH, y1 * V_MAPWIDTH, V_MAPBOTTOM);
...
static void AM_DrawGrid(void)
{
    ...
    for(fixed_t x = startx - ((startx - (bmaporgx >> FRACTOMAPBITS)) % gridwidth);
    x < endx; x += gridwidth)
    {
    ...
    for(fixed_t y = starty - ((starty - (bmaporgy >> FRACTOMAPBITS)) % gridheight);
    y < endy; y += gridheight)
    {
...
static byte* AM_DoorColor(unsigned short special)
{
    ...
    {
        if(!(special = ((special - GenLockedBase) & LockedKey) >> LockedKeyShift) ||
        special == AllKeys)
            return cdwallcolor;
        ...
        if(!(flags & ML_DONTDRAW))
        {
            ...
            (lbbox[BOXTOP] >> FRACTOMAPBITS) >= ambbox[BOXBOTTOM])
            {
                ...
                else
                {
                    ...
                    ((flags & ML_TELEPORTTRIGGERED) || isteleport[back->floorpic]))
                        AM_DrawFline(mline.a.x, mline.a.y, mline.b.x, mline.b.y,
                        ((flags & ML_MAPPED) ? teleportercolor : allmapfdwallcolor),
                        putbigdot);
                    else
                    {
                        const sector_t* front = line.frontsector;
                        ...
                        if(back->floorheight != front->floorheight)
                            AM_DrawFline(mline.a.x, mline.a.y, mline.b.x,
                            mline.b.y, ((flags & ML_MAPPED) ? fdwallcolor : allmapfdwallcolor),
                            putbigdot);
                        else if(back->ceilingheight != front->ceilingheight)
                            AM_DrawFline(mline.a.x, mline.a.y, mline.b.x,
                            mline.b.y, ((flags & ML_MAPPED) ? cdwallcolor : allmapcdwallcolor),
                            putbigdot);
                    }
                }
            }
        ...
        (lbbox[BOXTOP] >> FRACTOMAPBITS) >= ambbox[BOXBOTTOM])
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(back->floorheight != front->floorheight)
                        AM_DrawFline(mline.a.x, mline.a.y, mline.b.x, mline.b.y,
                        fdwallcolor, putbigdot);
                    else if(back->ceilingheight != front->ceilingheight)
                        AM_DrawFline(mline.a.x, mline.a.y, mline.b.x, mline.b.y,
                        cdwallcolor, putbigdot);
                    else
                        AM_DrawFline(mline.a.x, mline.a.y, mline.b.x, mline.b.y,
                        tswallcolor, putbigdot);
                }
            }
        }
    }
...
static void AM_DrawPlayerArrow(const mline_t* lineguy,
const int lineguylines,
const angle_t angle,
const fixed_t x,
const fixed_t y,
...
static void AM_DrawThingTriangle(const mline_t* lineguy,
const int lineguylines,
const fixed_t scale,
const angle_t angle,
const fixed_t x,
...
{
    const mline_t playerarrow[PLAYERARROWLINES] = { { { -57275, 0 }, { -39652, 0 } }, //  -
        { { -39652, 0 }, { 74898, 0 } },    //  -------
        { { 74898, 0 }, { 39652, 17623 } }, //  ------>
        { { 74898, 0 }, { 39652, -17623 } }, { { -57275, 0 }, { -74898, 17623 } }, // >------>
        { { -57275, 0 }, { -74898, -17623 } }, { { -39652, 0 }, { -57275, 17623 } }, // >>----->
        { { -39652, 0 }, { -57275, -17623 } } };
    ...
    const mline_t cheatplayerarrow[CHEATPLAYERARROWLINES] = {
        ...
        { { -7343, 0 }, { 74898, 0 } },     //  -------
        { { 74898, 0 }, { 39652, 11748 } }, //  ------>
        { { 74898, 0 }, { 39652, -11748 } }, { { -57275, 0 }, { -74898, 11748 } }, // >------>
        { { -57275, 0 }, { -74898, -11748 } }, { { -39652, 0 }, { -57275, 11748 } }, // >>----->
        { { -39652, 0 }, { -57275, -11748 } }, { { -30840, 0 }, { -30840, -11748 } }, // >>-d--->
        { { -30840, -11748 }, { -19091, -11748 } },
        { { -19091, -11748 }, { -19091, 17623 } }, { { -7343, 0 }, { -7343, -11748 } }, // >>-dd-->
        { { -7343, -11748 }, { 4405, -11748 } }, { { 4405, -11748 }, { 4405, 17623 } },
        { { 16154, 17623 }, { 16154, -10070 } }, // >>-ddt->
        { { 16154, -10070 }, { 18357, -12273 } }, { { 18357, -12273 }, { 23203, -10070 } }
    };
    ...
    if(viewplayer->cheats & (CF_ALLMAP | CF_ALLMAP_THINGS))
    {
        if(invisibility && (invisibility > STARTFLASHING || (invisibility & FLASHONTIC)))
            AM_DrawPlayerArrow(cheatplayerarrow, CHEATPLAYERARROWLINES, angle,
            point.x, point.y, &PUTTRANSLUCENTDOT);
        else
            AM_DrawPlayerArrow(cheatplayerarrow, CHEATPLAYERARROWLINES, angle,
            point.x, point.y, putbigdot2);
    }
    else if(invisibility && (invisibility > STARTFLASHING || (invisibility & FLASHONTIC)))
        AM_DrawPlayerArrow(playerarrow, CHEATPLAYERARROWLINES, angle, point.x,
        point.y, &PUTTRANSLUCENTDOT);
    else
...
const mline_t thingtriangle[THINGTRIANGLELINES] = { { { -32768, -45875 }, { 65536, 0 } },
    { { 65536, 0 }, { -32768, 45875 } }, { { -32768, 45875 }, { -32768, -45875 } } };
        ...
        for(mobj_t* thing = sectors[i].thinglist; thing; thing = thing->snext)
            ...
            !(thing->flags2 & MF2_DONTMAP) && thing->interpolate)
            {
                ...
                else
                {
                    ...
                    point.x =
                    (thing->oldx + FixedMul(thing->x - thing->oldx, fractionaltic)) >> FRACTOMAPBITS;
                    point.y =
                    (thing->oldy + FixedMul(thing->y - thing->oldy, fractionaltic)) >> FRACTOMAPBITS;
                }
                ...
                else
                {
                    ...
                    width = (BETWEEN(65 << FRACBITS,
                             MIN(spritewidth[sprite], spriteheight[sprite]), 80 << FRACBITS) >>
                            FRACTOMAPBITS) /
                    3;
                }

                if((fx = CXMTOF(point.x)) >= -width && fx <= V_MAPWIDTH + width &&
                (fy = CYMTOF(point.y)) >= -width && fy <= V_MAPHEIGHT + width)
...
static void AM_DrawBloodSplats(void)
{
    ...
    for(int i = 0; i < numsectors; i++)
        for(bloodsplat_t* splat = sectors[i].splatlist; splat; splat = splat->next)
        {
            ...
            if((fx = CXMTOF(point.x)) >= -BLOODSPLATWIDTH && fx <= V_MAPWIDTH + BLOODSPLATWIDTH &&
            (fy = CYMTOF(point.y)) >= -BLOODSPLATWIDTH && fy <= V_MAPHEIGHT + BLOODSPLATWIDTH)
                AM_DrawThingTriangle(thingtriangle, THINGTRIANGLELINES,
                BLOODSPLATWIDTH, (splat->angle - angleoffset) >> ANGLETOFINESHIFT,
                point.x, point.y, bloodsplatcolor, putbigdot2);
        }
}
    ...
    for(int i = 0; i < nummarks; i++)
    {
        ...
        do
        {
            ...
            for(int j = 0; j < MARKWIDTH * MARKHEIGHT; j++)
            {
                ...
                if(fx < (unsigned int)V_MAPWIDTH)
                {
                    const unsigned int fy = y + j / MARKWIDTH;

                    if(fy < (unsigned int)V_MAPHEIGHT)
                    {
                        const char src = nums[digit][j];

                        if(src == '1')
                            v_mapscreen[fy * V_MAPWIDTH + fx] = markcolor;
                        else if(src == '2')
                        {
                            byte* dest = &v_mapscreen[fy * V_MAPWIDTH + fx];
...
const int lengths[] = { 500, 2000, 5000, INT_MAX };
...
static void AM_DrawPath(void)
{
    ...
    {
        mpoint_t start = { breadcrumb[i - 1].x >> FRACTOMAPBITS,
            breadcrumb[i - 1].y >> FRACTOMAPBITS };
...
#define CENTERX (video.widescreen_delta + V_VANILLAWIDTH / 2)
#define CENTERY \
    ((V_VANILLAHEIGHT - V_VANILLASBARHEIGHT * (r_screensize < r_screensize_max)) / 2)

static void AM_DrawScaledPixel(const int x, const int y, byte* color)
{
    byte* dest = &v_screens[0][(y * 2 - 1) * V_MAPWIDTH + x * 2 - 1];
...
static void AM_DrawCrosshair(void)
{
    if(r_hud_translucency)
    {
        ...
        {
            byte* dot = &v_mapscreen[(V_MAPHEIGHT - 3) * V_MAPWIDTH / 2 - V_MAPWIDTH];
...
static void AM_StatusBarShadow(void)
{
    for(int i = 24 * 256, y = 1; y < 7; i -= 4 * 256, y++)
    {
        ...
        {
            byte* dot = &v_mapscreen[V_MAPAREA - y * V_MAPWIDTH + x];
...
{
    static byte dest[V_MAXSCREENAREA];

    memcpy(dest, v_mapscreen, V_MAPAREA);

    for(int y = 0; y <= V_MAPAREA - V_MAPWIDTH; y += V_MAPWIDTH)
        for(int x = y; x <= y + V_MAPWIDTH - 2; x++)
            dest[x] = tinttab33[(dest[x + 1] << 8) + dest[x]];

    for(int y = 0; y <= V_MAPAREA - V_MAPWIDTH; y += V_MAPWIDTH)
        for(int x = y + V_MAPWIDTH - 2; x > y; x--)
            dest[x] = tinttab33[(dest[x - 1] << 8) + dest[x]];

    for(int y = 0; y <= V_MAPAREA - V_MAPWIDTH * 2; y += V_MAPWIDTH)
        for(int x = y; x <= y + V_MAPWIDTH - 1; x++)
            dest[x] = tinttab33[(dest[x + V_MAPWIDTH] << 8) + dest[x]];

    for(int y = V_MAPAREA - V_MAPWIDTH; y >= V_MAPWIDTH; y -= V_MAPWIDTH)
        for(int x = y; x <= y + V_MAPWIDTH - 1; x++)
            dest[x] = tinttab33[(dest[x - V_MAPWIDTH] << 8) + dest[x]];

    memcpy(v_mapscreen, dest, V_MAPAREA);

    for(int x = 0; x < V_MAPWIDTH; x++)
        v_mapscreen[x] = tinttab33[v_mapscreen[x]];

    for(int y = 0; y <= V_MAPAREA - V_MAPWIDTH; y += V_MAPWIDTH)
        v_mapscreen[y] = tinttab33[v_mapscreen[y]];

    for(int y = V_MAPWIDTH - 1; y <= V_MAPAREA - 1; y += V_MAPWIDTH)
        v_mapscreen[y] = tinttab33[v_mapscreen[y]];

    if(r_screensize == r_screensize_max)
        for(int x = V_MAPAREA - V_MAPWIDTH; x < V_MAPAREA; x++)
            v_mapscreen[x] = tinttab33[v_mapscreen[x]];
}
...
void AM_Drawer(void)
{
    ...
    if(r_detail == r_detail_low)
    {
        V_LowGraphicDetail_2x2(v_mapscreen, V_MAPWIDTH, 0, 0, V_MAPWIDTH, V_MAPAREA, 2, 2);
